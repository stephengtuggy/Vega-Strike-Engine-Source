...
#define NUM_TITLES 8
static const char *titles[NUM_TITLES] = {"Start New Pilot",
                                         "Play Saved Pilot",
                                         "Continue Last Game",
                                         "Game Settings",
...
DWORD WINAPI DrawStartupDialog(LPVOID lpParameter)
{
    ...
    int pid = spawnl(P_WAIT,
                     "./vegastrike",
                     "./vegastrike",
                     s->num ? s->num : (std::string("\"") + s->my_mission + "\"").c_str(),
                     s->num ? (std::string("\"") + s->my_mission + "\"").c_str() : NULL,
                     NULL);
    if (pid == -1) {
        if (chdir("bin") == 0) {
            spawnl(P_WAIT,
...
void file_ok_auto_sel(GtkWidget *w, GtkFileSelection *fs)
{
    if ((gtk_file_selection_get_filename(GTK_FILE_SELECTION(fs)))[0] != '\0') {
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < NUM_TITLES; i++) {
        ...
        gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(hello), (void *)i);
...
void fileop_destroy(GtkWidget *w, GtkFileSelection *fs)
{
...
void delfile(GtkWidget *w, GtkFileSelection *fs)
{
...
void delfile_conf(GtkWidget *w, GtkFileSelection *fs)
{
    GtkWidget *i = fs->selection_entry;
    if (i) {
        GtkWidget *window;
...
struct dumbstruct {
    GtkWidget *filesel, *entrywin;
};

void fileop_destroy_dumb(GtkWidget *w, dumbstruct *fs)
{
    if (fs->filesel) {
...
void renfile(GtkWidget *w, dumbstruct *dmb)
{
...
void renfile_conf(GtkWidget *w, GtkFileSelection *fs)
{
    GtkWidget *i = fs->selection_entry;
    if (i) {
        GtkWidget *window;
...
void copyfile(GtkWidget *w, dumbstruct *dmb)
{
    GtkFileSelection *fs  = GTK_FILE_SELECTION(dmb->filesel);
    GtkWidget *       ent = dmb->entrywin;
    GtkWidget *       i   = fs->selection_entry;
...
void copyfile_conf(GtkWidget *w, GtkFileSelection *fs)
{
    GtkWidget *i = fs->selection_entry;
    if (i) {
        GtkWidget *window;
...
void copynormal(GtkWidget *w, dumbstruct *dmb)
{
    GtkFileSelection *fs  = GTK_FILE_SELECTION(dmb->filesel);
    GtkWidget *       ent = dmb->entrywin;
    GtkWidget *       i   = fs->selection_entry;
...
void copynormal_conf(GtkWidget *w, GtkFileSelection *fs)
{
    GtkWidget *i = fs->selection_entry;
    if (i) {
        GtkWidget *window;
                      ...
                      bool          usenormalbuttons = false)
{
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(filew)->ok_button), "clicked", (GtkSignalFunc)func, filew);
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(filew)->help_button), "clicked", (GtkSignalFunc)help_func, (void *)i);

    gtk_signal_connect_object(
        GTK_OBJECT(GTK_FILE_SELECTION(filew)->cancel_button), "clicked", (GtkSignalFunc)gtk_widget_destroy, GTK_OBJECT(filew));
