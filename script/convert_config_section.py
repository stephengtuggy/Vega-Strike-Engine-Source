...
setting_declaration_namespaced_parse_key.pattern = re.compile(r'^(?P<initial_space> {4})(?P<static_or_const>static|const)\s*(?P<type>std::string)\s+(?P<name>\w+)(?P<init_braces>\{})?\s*; *(?://[^\r\n]*?)?\r?\n',
                                                              regex_flags_multiline)
...
setting_declaration_namespaced_parse_key_1a.pattern = re.compile(r'^(?P<initial_space> {4})(?P<type>std::string)\s+(?P<name>\w+)(?P<init_braces>\{})?\s*; *(?://[^\r\n]*?)?\r?\n',
                                                                 regex_flags_multiline)
...
setting_declaration_namespaced_parse_key_1b.pattern = re.compile(r'^(?P<initial_space> {4})(?P<type>std::string)\s+(?P<name>\w+)\s*=\s*\"(?P<default_value>[^\"]*)\"\s*; *(?://[^\r\n]*?)?\r?\n',
                                                                 regex_flags_multiline)
...
setting_declaration_namespaced_parse_key_1c.pattern = re.compile(r'^(?P<initial_space> {4})(?P<static_or_const>static|const)\s*(?P<type>std::string)\s+(?P<name>\w+)\s*=\s*\"(?P<default_value>[^\"]*)\"\s*; *(?://[^\r\n]*?)?\r?\n',
                                                                 regex_flags_multiline)
...
setting_declaration_simple_parse_key.pattern = re.compile(r'^(?P<initial_space> {4})(?P<static_or_const>static|const)\s*(?P<type>bool|double|float|int)\s+(?P<name>\w+)(?P<init_braces>\{})?\s*; *(?://[^\r\n]*?)?\r?\n',
                                                          regex_flags_multiline)
...
setting_declaration_simple_parse_key_1a.pattern = re.compile(r'^(?P<initial_space> {4})(?P<type>bool|double|float|int)\s+(?P<name>\w+)(?P<init_braces>\{})?\s*; *(?://[^\r\n]*?)?\r?\n',
                                                             regex_flags_multiline)
...
setting_declaration_simple_parse_key_1b.pattern = re.compile(r'^(?P<initial_space> {4})(?P<type>bool|double|float|int)\s+(?P<name>\w+)\s*=\s*\"(?P<default_value>[^\"]*)\"\s*; *(?://[^\r\n]*?)?\r?\n',
                                                             regex_flags_multiline)
...
setting_declaration_simple_parse_key_1c.pattern = re.compile(r'^(?P<initial_space> {4})(?P<static_or_const>static|const)\s*(?P<type>bool|double|float|int)\s+(?P<name>\w+)\s*=\s*\"(?P<default_value>[^\"]*)\"\s*; *(?://[^\r\n]*?)?\r?\n',
                                                             regex_flags_multiline)
...
equals_configuration_parse_key_1.pattern = re.compile(r'^(?P<initial_space> *)(?P<name>\w+)\s*=\s*('
                                                      r'?P<config_expr>configuration\(\)->\w+(?:\.\w+)+); *(?://[^\r\n]*?)?\r?\n',
                                                      regex_flags_multiline)
...
def build_parse_keys(section_name: str) -> list[ParseKey]:
    parse_key_0 = ParseKey()
    parse_key_0.pattern = re.compile(r'vs_config->getVariable\s*\(\s*\"' + section_name +
                                     r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"(?P<default_value>true)\"\s*\)', regex_flags_multiline)
    parse_key_0.substitution = r'configuration().' + section_name + r'.\g<subsection>.\g<name2>'
    parse_key_0.leave_as_is = ''
    ...
    parse_key_0c = ParseKey()
    parse_key_0c.pattern = re.compile(r'vs_config->getVariable\s*\(\s*\"' + section_name +
                                      r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"(?P<default_value>[^\"]*)\"\s*\)', regex_flags_multiline)
    parse_key_0c.substitution = r'configuration().' + section_name + r'.\g<subsection>.\g<name2>'
    parse_key_0c.leave_as_is = ''
    ...
    parse_key_1.pattern = re.compile(
        ...
        r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_2.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<type>bool|double|float|int|std::string)\s+=(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name +
        r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_2a.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<type>bool|double|float|int|std::string)\s+=(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"'
        + section_name +
        r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_3.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_3a.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_4.pattern = re.compile(
        r'\bXMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_4a.pattern = re.compile(
        r'\bXMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_5.pattern = re.compile(
        r'\bparse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_5a.pattern = re.compile(
        r'\bparse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>\w+)\"\s*,\s*\"(?P<name2>\w+)\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_6.pattern = re.compile(
        r'\bvs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_6a.pattern = re.compile(
        r'\bvs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<subsection>w+)\"\s*,\s*\"(?P<name2>w+)\"\s*,\s*\"[^\"]*\"\s*\)',
        regex_flags_multiline)
    ...
    parse_key_7.pattern = re.compile(
        r'^(?P<initial_space> *)static\s+(?P<type>bool|double|float|int|std::string)\s+(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name +
        r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_9.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_9a.pattern = re.compile(
        r'^(?P<initial_space> *)(?P<name>\w+)\s+=\s*XMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_10.pattern = re.compile(
        r'\bXMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_10a.pattern = re.compile(
        r'\bXMLSupport::parse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_11.pattern = re.compile(
        r'\bparse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)\s*;\r?\n',
        regex_flags_multiline)
    ...
    parse_key_11a.pattern = re.compile(
        r'\bparse_[a-z0-9_]+\s*\(\s*vs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)\s*\)',
        regex_flags_multiline)
    ...
    parse_key_12.pattern = re.compile(
        r'\bvs_config->getVariable\s*\(\s*\"' + section_name + r'\"\s*,\s*\"(?P<name2>\w+)\"\s*,\s*\"[^\"]*\"\s*\)',
        regex_flags_multiline)
