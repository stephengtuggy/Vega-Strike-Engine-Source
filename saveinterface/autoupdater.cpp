...
void main2()
{
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(hello), (void *)0);
...
int main(int argc, char **argv)
{
    ...
    *c++ = '\0'; /* cut off last part (binary name) */
     ...
     * function hello() passing it NULL as its argument.  The hello()
     * function is defined above. */
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(bCvsUpdateList), vbox);
     ...
     * function hello() passing it NULL as its argument.  The hello()
     * function is defined above. */
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(bmain2), vbox);
    gtk_container_add(GTK_CONTAINER(hbox), button);
    gtk_widget_show(button);
