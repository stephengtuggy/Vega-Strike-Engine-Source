...
static const char *s_testEndianess(void)
{
    ...
    if (s_testBigEndian() != IS_BIG_ENDIAN)
        return "*ERROR: POSH compile time endianess does not match run-time endianess verification.  Please report this to "
               "poshlib@poshlib.org!\n";
    /* make sure our endian swap routines work */
    if ((NATIVE32(0x11223344L) != 0x11223344L) || (FOREIGN32(0x11223344L) != 0x44332211L) || (NATIVE16(0x1234) != 0x1234) ||
