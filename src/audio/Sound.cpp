...
Sound::Sound(const std::string &_name, bool streaming) throw() : name(_name)
{
    flags.loaded    = false;
    flags.loading   = false;
...
void Sound::waitLoad() throw(Exception)
{
    while (isLoading())
        Audio::sleep(10);
}
...
void Sound::onLoaded(bool success) throw()
{
    flags.loaded  = success;
    flags.loading = false;
}

void Sound::abortLoad() throw()
{
