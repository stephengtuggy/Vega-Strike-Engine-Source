...
RenderableSource::RenderableSource(Source *_source) throw() : source(_source)
{
}
...
void RenderableSource::startPlaying(Timestamp start) throw(Exception)
{
    try {
...
void RenderableSource::stopPlaying() throw()
{
...
bool RenderableSource::isPlaying() const throw()
{
    try {
...
Timestamp RenderableSource::getPlayingTime() const throw(Exception)
{
    return getPlayingTimeImpl();
}

void RenderableSource::update(int flags, const Listener &sceneListener) throw()
{
    try {
...
void RenderableSource::seek(Timestamp time) throw(Exception)
{
    seekImpl(time);
}
