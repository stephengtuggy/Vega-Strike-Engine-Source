...
OpenALStreamingSound::OpenALStreamingSound(const std::string &name, VSFileSystem::VSFileType type, unsigned int _bufferSamples) throw()
...
void OpenALStreamingSound::loadImpl(bool wait) throw(Exception)
{
    // just in case
    unloadImpl();
...
void OpenALStreamingSound::flushBuffers() throw()
{
...
void OpenALStreamingSound::unloadImpl() throw()
{
    if (isStreamLoaded())
        closeStream();
...
ALBufferHandle OpenALStreamingSound::readAndFlip() throw(Exception)
{
    if (!isLoaded())
    ...
    clearAlError();
    alBufferData(bufferHandle, asALFormat(targetFormat), buffer.getBuffer(), buffer.getUsedBytes(), targetFormat.sampleFrequency);
    checkAlError();
...
void OpenALStreamingSound::unqueueBuffer(ALBufferHandle buffer) throw(Exception)
{
...
void OpenALStreamingSound::seek(double position) throw(Exception)
{
    if (!isLoaded())
        throw ResourceNotLoadedException(getName());
...
Timestamp OpenALStreamingSound::getTimeBase() const throw()
{
