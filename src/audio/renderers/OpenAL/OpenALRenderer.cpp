        ...
        struct RendererData {
            ...
            class SoundKey
            {
              ...
              public:
                ...
                SoundKey(VSFileSystem::VSFileType _type, const std::string &_name) : type(_type), nameLen(_name.length()), name(_name)
                {
                }
                ...
                bool operator<(const SoundKey &other) const
                {
                    return (type < other.type) ||
                           ((type == other.type) && ((nameLen < other.nameLen) || ((nameLen == other.nameLen) && (name < other.name))));
                }
            ...
            void openDevice(const char *deviceSpecifier) throw(Exception)
            {
                if (alDevice)
            ...
            void closeDevice() throw(Exception)
            {
                if (alContext)
            ...
            void openContext(const Format &format) throw(Exception)
            {
                if (alContext)
                ...
                ALCint params[] = {ALC_FREQUENCY, format.sampleFrequency, 0};
            ...
            void closeContext() throw(Exception)
            {
                if (alContext) {
            ...
            RendererData() throw() : alDevice(NULL), alContext(NULL)
            {
            }
...
OpenALRenderer::OpenALRenderer() throw(Exception) : data(new RendererData)
{
}
...
SharedPtr<Sound> OpenALRenderer::getSound(const std::string &name, VSFileSystem::VSFileType type, bool streaming) throw(Exception)
{
    checkContext();
    ...
    if (!sound.get() || streaming) {
        if (streaming) {
            ...
            sound.reset();
            data->addSound(type, name, sound = SharedPtr<Sound>(new OpenALStreamingSound(name, type)));
        } else {
            data->addSound(type, name, sound = SharedPtr<Sound>(new OpenALSimpleSound(name, type)));
        }
    }
    return sound;
}
...
void OpenALRenderer::attach(SharedPtr<Source> source) throw(Exception)
{
    checkContext();
    source->setRenderable(SharedPtr<RenderableSource>(source->getSound()->isStreaming()
                                                          ? (RenderableSource *)new OpenALRenderableStreamingSource(source.get())
                                                          : (RenderableSource *)new OpenALRenderableSource(source.get())));
}

void OpenALRenderer::attach(SharedPtr<Listener> listener) throw(Exception)
{
    checkContext();
    listener->setRenderable(SharedPtr<RenderableListener>(new OpenALRenderableListener(listener.get())));
}

void OpenALRenderer::detach(SharedPtr<Source> source) throw()
{
    // Just clear it... RenderableListener's destructor will handle everything fine.
    source->setRenderable(SharedPtr<RenderableSource>());
}

void OpenALRenderer::detach(SharedPtr<Listener> listener) throw()
{
    // Just clear it... RenderableListener's destructor will handle everything fine.
    listener->setRenderable(SharedPtr<RenderableListener>());
}

void OpenALRenderer::setMeterDistance(Scalar distance) throw()
{
    // ToDo
...
void OpenALRenderer::setDopplerFactor(Scalar factor) throw()
{
...
void OpenALRenderer::setOutputFormat(const Format &format) throw(Exception)
{
    if (!data->alDevice)
...
void OpenALRenderer::checkContext() throw(Exception)
{
    if (!data->alDevice)
        data->openDevice(NULL);
    if (!data->alContext) {
...
void OpenALRenderer::beginTransaction() throw(Exception)
{
    data->suspend();
...
void OpenALRenderer::commitTransaction() throw(Exception)
{
    data->commit();
}

void OpenALRenderer::initContext() throw(Exception)
{
...
void OpenALRenderer::setupDopplerEffect() throw(Exception)
{
    clearAlError();
...
BorrowedOpenALRenderer::BorrowedOpenALRenderer(ALCdevice *device, ALCcontext *context) throw(Exception) : OpenALRenderer()
{
    if (device)
...
void BorrowedOpenALRenderer::setOutputFormat(const Format &format) throw(Exception)
{
    // No-op... format is given by the borrowed context
    Renderer::setOutputFormat(format);
}

void BorrowedOpenALRenderer::checkContext() throw(Exception)
{
