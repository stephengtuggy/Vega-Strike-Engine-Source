...
OpenALRenderableSource::OpenALRenderableSource(Source *source) : RenderableSource(source), alSource(0), alBuffersAttached(false)
{
...
void OpenALRenderableSource::startPlayingImpl(Timestamp start) throw(Exception)
{
    if (!isPlayingImpl()) {
...
void OpenALRenderableSource::stopPlayingImpl() throw(Exception)
{
    alSourceStop(alSource);
}

bool OpenALRenderableSource::isPlayingImpl() const throw(Exception)
{
    ALint state = 0;
...
Timestamp OpenALRenderableSource::getPlayingTimeImpl() const throw(Exception)
{
    ALfloat offs = -1.f;
...
void OpenALRenderableSource::updateImpl(int flags, const Listener &sceneListener) throw(Exception)
{
    ...
    if (flags & UPDATE_LOCATION) {
        if (source->isRelative()) {
            ...
            alSource3f(als, AL_VELOCITY, sceneListener.toLocalDirection(source->getVelocity() - sceneListener.getVelocity()));
...
void OpenALRenderableSource::attachALBuffers() throw(Exception)
{
...
void OpenALRenderableSource::seekImpl(Timestamp time) throw(Exception)
{
