...
namespace __impl
{
    ...
    struct SceneManagerData {
        struct SourceRef {
            ...
            SourceRef(const SharedPtr<Source> &src, const SharedPtr<Scene> &scn) : source(src), scene(scn), needsActivation(0)
            {
            }
...
SceneManager::SceneManager() throw() : data(new SceneManagerData)
{
}
...
SharedPtr<Source> SceneManager::createSource(SharedPtr<Sound> sound, bool looping) throw(Exception)
{
...
SharedPtr<Source> SceneManager::createSource(SharedPtr<SourceTemplate> tpl) throw(Exception)
{
    return createSource(tpl, tpl->getSoundName());
}

SharedPtr<Source> SceneManager::createSource(SharedPtr<SourceTemplate> tpl, const std::string &name) throw(Exception)
{
    SharedPtr<Source> source = createSource(internalRenderer()->getSound(name, tpl->getSoundType(), tpl->isStreaming()), tpl->isLooping());
...
void SceneManager::destroySource(SharedPtr<Source> source) throw()
{
...
void SceneManager::addScene(SharedPtr<Scene> scene) throw(DuplicateObjectException)
{
...
SharedPtr<Scene> SceneManager::createScene(const std::string &name) throw(DuplicateObjectException)
{
...
SharedPtr<Scene> SceneManager::getScene(const std::string &name) const throw(NotFoundException)
{
...
void SceneManager::destroyScene(const std::string &name) throw(NotFoundException)
{
    // By simply unreferencing, it should get destroyed when all references are released.
    // Which is good for multithreading - never destroy something that is being referenced.
...
void SceneManager::setSceneActive(const std::string &name, bool active) throw(NotFoundException)
{
...
bool SceneManager::getSceneActive(const std::string &name) throw(NotFoundException)
{
    return data->activeScenes.count(name) > 0;
}

void SceneManager::setRenderer(SharedPtr<Renderer> renderer) throw(Exception)
{
    if (data->renderer.get()) {
...
SharedPtr<Renderer> SceneManager::getRenderer() const throw()
{
    return data->renderer;
}

unsigned int SceneManager::getMaxSources() const throw()
{
    return data->maxSources;
}

void SceneManager::setMaxSources(unsigned int n) throw(Exception)
{
    data->maxSources = n;
}

void SceneManager::playSource(SharedPtr<SourceTemplate> tpl,
...
float SceneManager::getMinGain() const throw()
{
    return data->minGain;
}

void SceneManager::setMinGain(float gain) throw(Exception)
{
    assert(gain >= 0.f);
    data->minGain = gain;
}

double SceneManager::getMaxDistance() const throw()
{
    return data->maxDistance;
}

void SceneManager::setMaxDistance(double distance) throw(Exception)
{
    assert(distance >= 0.f);
    data->maxDistance = distance;
}

SharedPtr<SceneManager::SceneIterator> SceneManager::getSceneIterator() const throw()
{
    return SharedPtr<SceneIterator>(new ChainingIterator<VirtualValuesIterator<SceneManagerData::SceneMap::iterator>>(
        VirtualValuesIterator<SceneManagerData::SceneMap::iterator>(data->activeScenes.begin(), data->activeScenes.end()),
        VirtualValuesIterator<SceneManagerData::SceneMap::iterator>(data->inactiveScenes.begin(), data->inactiveScenes.end())));
}

SharedPtr<SceneManager::SceneIterator> SceneManager::getActiveSceneIterator() const throw()
{
    return SharedPtr<SceneIterator>(
        new VirtualValuesIterator<SceneManagerData::SceneMap::iterator>(data->activeScenes.begin(), data->activeScenes.end()));
}
...
void SceneManager::activationPhaseImpl() throw(Exception)
{
    ...
    for (SceneManagerData::SceneMap::iterator it = data->activeScenes.begin(); it != data->activeScenes.end(); ++it) {
        ...
        for (SimpleScene::SourceIterator sit = scene->getActiveSources(), send = scene->getActiveSourcesEnd(); sit != send; ++sit) {
    ...
    for (std::vector<SourcePriorityRef>::const_iterator it = selection.begin(); it != selection.end(); ++it) {
        newSources.insert(SceneManagerData::SourceRef(*(it->iter), dynamic_cast<SimpleScene *>(it->scene)->shared_from_this()));
    }
...
void SceneManager::updateSourcesImpl(bool withAttributes) throw(Exception)
{
    // Two-pass stuff.

    // First, update attributes (mostly location)
    RenderableSource::UpdateFlags updateFlags = withAttributes ? RenderableSource::UPDATE_ALL : RenderableSource::UPDATE_LOCATION;
    for (SceneManagerData::SourceRefSet::const_iterator it = data->activeSources.begin(); it != data->activeSources.end(); ++it) {
        // Update the renderable (attributes)
        it->source->updateRenderable(updateFlags | (it->needsActivation ? RenderableSource::UPDATE_ALL : 0), it->scene->getListener());
...
void SceneManager::updateListenerImpl(bool withAttributes) throw(Exception)
{
    // Update root listener
    RenderableListener::UpdateFlags updateFlags = withAttributes ? RenderableListener::UPDATE_ALL : RenderableListener::UPDATE_LOCATION;
...
Duration SceneManager::getPositionUpdateFrequency() const throw()
{
    return data->positionUpdateFrequency;
}

Duration SceneManager::getListenerUpdateFrequency() const throw()
{
    return data->listenerUpdateFrequency;
}

Duration SceneManager::getAttributeUpdateFrequency() const throw()
{
    return data->attributeUpdateFrequency;
}

Duration SceneManager::getActivationFrequency() const throw()
{
    return data->activationFrequency;
}

void SceneManager::setPositionUpdateFrequency(Duration interval) const throw()
{
    data->positionUpdateFrequency = interval;
}

void SceneManager::setListenerUpdateFrequency(Duration interval) const throw()
{
    data->listenerUpdateFrequency = interval;
}

void SceneManager::setAttributeUpdateFrequency(Duration interval) const throw()
{
    data->attributeUpdateFrequency = interval;
}

void SceneManager::setActivationFrequency(Duration interval) const throw()
{
    data->activationFrequency = interval;
}

SharedPtr<Listener> SceneManager::getRootListener() const throw()
{
    return data->rootListener;
}

void SceneManager::notifySourcePlaying(SharedPtr<Source> source, SharedPtr<Scene> scene, bool playing) throw(Exception)
{
