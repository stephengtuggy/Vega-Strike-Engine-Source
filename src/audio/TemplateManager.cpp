...
namespace __impl
{
    ...
    struct TemplateManagerData {
        ...
        SharedPtr<XMLDOM::XMLDocument> getDefinitionFile(const std::string &path) const throw(Exception)
        {
...
TemplateManager::TemplateManager() throw() : data(new TemplateManagerData)
{
}
...
SharedPtr<XMLDOM::XMLDocument> TemplateManager::getDefinitionFile(const std::string &path) const throw(ResourceNotLoadedException)
{
    return ((const TemplateManagerData &)*data).getDefinitionFile(path);
}

SharedPtr<XMLDOM::XMLDocument> TemplateManager::getDefinitionFile(const std::string &path) throw(Exception)
{
    try {
...
void TemplateManager::setDefaultDefinitionFile(const std::string &x) throw()
{
    data->defaultDefinitionFile = x;
}

const std::string &TemplateManager::getDefaultDefinitionFile() const throw()
{
    return data->defaultDefinitionFile;
}

SharedPtr<SourceTemplate> TemplateManager::getSourceTemplate(const std::string &name) throw(Exception)
{
...
SharedPtr<SourceTemplate> TemplateManager::loadSourceTemplate(const std::string &name) throw(Exception)
{
    ...
    SharedPtr<SourceTemplate> rv(new SourceTemplate(src, type, looping));
    ...
    rv->setPerFrequencyRadiusRatios(PerFrequency<Scalar>(parseFloat(tdef->getAttributeValue("lfReferenceFreq", "250")),
...
void TemplateManager::addSourceTemplate(const string &name, SharedPtr<SourceTemplate> tpl, bool perm) throw(ResourceAlreadyLoadedException)
{
    static string empty;
    addSourceTemplate(empty, name, tpl, perm);
}

void TemplateManager::addSourceTemplate(const string &            path,
                                        const string &            name,
                                        SharedPtr<SourceTemplate> tpl,
                                        bool                      perm) throw(ResourceAlreadyLoadedException)
{
