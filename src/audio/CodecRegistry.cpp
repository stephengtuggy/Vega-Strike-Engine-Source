...
Codec *CodecRegistry::findByFile(const std::string &path, VSFileSystem::VSFileType type) const throw(CodecNotFoundException)
{
    ...
    if (candidates.begin() != candidates.end()) {
        ...
        for (std::vector<Codec *>::const_reverse_iterator it = candidates.rbegin();
             it != reinterpret_cast<const std::vector<Codec *> &>(candidates).rend();
             ++it)
...
Stream *CodecRegistry::open(const std::string &path, VSFileSystem::VSFileType type) const throw(Exception)
{
    Codec *codec = findByFile(path, type);
    return codec->open(path, type);
}

CodecRegistration::CodecRegistration(Codec *_codec, int priority) throw() : codec(_codec)
{
