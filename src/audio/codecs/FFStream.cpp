...
// Read the next packet, skipping all packets that aren't for this stream
        ...
        void decodeFrame() throw(PacketDecodeException)
        {
            ...
            int used =
#if (LIBAVCODEC_VERSION_MAJOR >= 53)
                avcodec_decode_audio3(pCodecCtx, (int16_t *)sampleBufferAligned, &dataSize, &packet);
#else
                avcodec_decode_audio2(pCodecCtx, (int16_t *)sampleBufferAligned, &dataSize, packetBuffer, packetBufferSize);
#endif
...
FFStream::FFStream(const std::string &path, int streamIndex, VSFileSystem::VSFileType type) throw(Exception) : Stream(path)
{
...
void FFStream::seekImpl(double position) throw(Exception)
{
    ...
    if ((targetSample >= ffData->sampleBufferStart) && (targetSample < ffData->sampleBufferStart + ffData->sampleBufferSize)) {
        ...
        if (ffData->saneTimeStamps()) {
            ...
            av_seek_frame(ffData->pFormatCtx,
                          ffData->streamIndex,
                          ffData->timeToPts(position),
