...
bool QuadTree::GetGroundPos(QVector &Location, Vector &norm, const Matrix &transf, float TotalTerrainSizeX, float TotalTerrainSizeZ) const
{
...
float QuadTree::GetHeight(Vector Location, Vector &normal, const Matrix &transf, float TotalTerrainSizeX, float TotalTerrainSizeZ) const
{
...
{
    return Vector(
        1.0F / (Vector(trans.getP()).Magnitude()), 1.0F / (Vector(trans.getQ()).Magnitude()), 1.0F / (Vector(trans.getR()).Magnitude()));
}
...
void QuadTree::Render()
{
    ...
    quadsquare::SetCurrentTerrain(
        &VertexAllocated, &VertexCount, &vertices, &unusedvertices, nonlinear_transform, &textures, Scales, neighbors);
