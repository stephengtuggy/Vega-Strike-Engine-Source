  ...
  private:
    ...
    void convertFrame()
    {
        if (frameReady) {
            ...
            img_convert(
                (AVPicture *)pFrameRGB, PIX_FMT_RGB24, (AVPicture *)pNextFrameYUV, pCodecCtx->pix_fmt, pCodecCtx->width, pCodecCtx->height);
#else
            sws_scale(pSWSCtx, pNextFrameYUV->data, pNextFrameYUV->linesize, 0, pCodecCtx->height, pFrameRGB->data, pFrameRGB->linesize);
#endif
            prevPTS = fbPTS;
...
// Decode the next chunk of data
                    ...
                    bytesDecoded = avcodec_decode_video2(pCodecCtx, pNextFrameYUV, &frameFinished, &packet);
#else
                    bytesDecoded = avcodec_decode_video(pCodecCtx, pNextFrameYUV, &frameFinished, packetBuffer, packetBufferSize);
#endif
                    VSFileSystem::vs_dbg(3) << boost::format("dts %1%: Decoded %2% bytes %3%") % int64_t(packet.dts) % int(bytesDecoded) %
...
// Dump format info in case we want to know...
        ...
        for (unsigned int i = 0; i < pFormatCtx->nb_streams; ++i) {
            VSFileSystem::vs_dbg(3) << boost::format("  Stream %1%: type %2% (%3%) first dts %4%") % i %
        ...
        VSFileSystem::vs_dbg(3) << boost::format("  Codec Timebase: %1%/%2%") % pCodecCtx->time_base.num % pCodecCtx->time_base.den
                                << std::endl;
        ...
        if ((pFrameYUV == 0) || (pNextFrameYUV == 0))
            throw VidFile::Exception("Problem during YUV framebuffer initialization");
        // Get some info
        ...
        VSFileSystem::vs_dbg(3) << boost::format("  Framerate: %1%/%2%") % pStream->r_frame_rate.num % pStream->r_frame_rate.den
                                << std::endl;
        VSFileSystem::vs_dbg(3) << boost::format("  Stream timebase: %1%/%2%") % pStream->time_base.num % pStream->time_base.den
                                << std::endl;
        ...
        VSFileSystem::vs_dbg(2) << boost::format("  playing at %1%x%2%") % width % height << std::endl;
        ...
        prevPTS = fbPTS = pFrameYUV->pts = pNextFrameYUV->pts = 0;
    ...
    bool seek(float time)
    {
        ...
        VSFileSystem::vs_dbg(3) << boost::format("Seeking to %1$.3fs pts %2%") % time % targetPTS << std::endl;
        if ((targetPTS >= prevPTS) && (targetPTS < pNextFrameYUV->pts)) {
            ...
            if (targetPTS < fbPTS) {
                ...
                VSFileSystem::vs_dbg(3) << boost::format("backseeking to %1% (at %2%)") % backPTS % int64_t(pNextFrameYUV->pts)
                                        << std::endl;
            ...
            try {
                ...
                if (pNextFrameYUV->pts < targetPTS) {
                    ...
                    nextFrame();
                    VSFileSystem::vs_dbg(3) << boost::format("decoding to %1% (at %2%-%3%)") % targetPTS % prevPTS %
                ...
                while (packet.dts < targetPTS) {
                    prevPTS = packet.dts;
                    nextFrame(true);
                    VSFileSystem::vs_dbg(3) << boost::format("skipping to %1% (at %2%-%3%)") % targetPTS % prevPTS % int64_t(packet.dts)
                                            << std::endl;
                }
...
VidFile::VidFile() throw() : impl(NULL)
{
}
