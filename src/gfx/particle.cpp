...
static inline void SetPointVertex(const QVector &                                loc,
                                  const GFXColor &                               col,
                                  const float                                    psize,
                                  const float                                    grow,
                                  const float                                    trans,
...
void ParticleTrail::DrawAndUpdate()
{
    ...
    if (!config.initialized) {
        ...
        VSFileSystem::vs_dbg(1) << boost::format("Configured particle system %1% with %2% particles") % config.prefix % maxparticles
                                << std::endl;
    }
    ...
    if (use_points) {
        ...
        if (dosort) {
            ...
            {
                for (std::vector<QVector, aligned_allocator<QVector>>::const_iterator it = particleLoc.begin(); it != particleLoc.end();
                     ++it) {
        ...
        if (dosort) {
            VSFileSystem::vs_dbg(3) << boost::format("Drawing %1%/%2% sorted particles") % nparticles % maxparticles << std::endl;
            GFXDrawElements(GFXQUAD, &particleVert[0], nparticles * vertsPerParticle, &indices[0], indices.size(), 3, 4, 2);
        } else {
            VSFileSystem::vs_dbg(3) << boost::format("Drawing %1%/%2% unsorted particles") % nparticles % maxparticles << std::endl;
    ...
    if (nparticles) {
        VSFileSystem::vs_dbg(3) << boost::format("Drawn %1% particles, minalpha %2%") % nparticles % minalpha << std::endl;
    }
...
void ParticleEmitter::doParticles(
    const QVector &pos, float rSize, float percent, const Vector &basevelocity, const Vector &velocity, float pSize, const GFXColor &color)
{
    percent = 1 - percent;
    int i   = rand();
