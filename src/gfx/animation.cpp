...
Animation::Animation(
    const char *FileName, bool Rep, float priority, enum FILTER ismipmapped, bool camorient, bool appear_near_by_radius, const GFXColor &c)
    : mycolor(c)
{
...
void Animation::ProcessDrawQueue(std::vector<Animation *> &animationdrawqueue, float limit)
{
    ...
    for (i = animationdrawqueue.size() - 1; i >= 0; i--) {
        ...
        if ((limit <= -FLT_MAX) || (animationdrawqueue[i]->Position() - campos).Magnitude() - animationdrawqueue[i]->height > limit) {
            // other way was inconsistent about what was far and what was not--need to use the same test for putting to far queueu and
            // drawing it--otherwise graphical glitches
            GFXFogMode(FOG_OFF);
...
bool Animation::CalculateOrientation(Matrix &result)
{
    ...
    bool         retval     = ::CalculateOrientation(
        pos, camp, camq, camr, wid, hei, (options & ani_close) ? HaloOffset : 0, false, (options & ani_up) ? NULL : &local_transformation);
