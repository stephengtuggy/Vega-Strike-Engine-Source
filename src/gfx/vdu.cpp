...
bool VDU::staticable() const
{
    ...
    if (thismode != COMM && thismode != TARGETMANIFEST && thismode != TARGET && thismode != NAV && thismode != VIEW && thismode != WEBCAM &&
                       ...
                       GFXColor innershield)
{
    ...
    static float shthresh[3] = {
        XMLSupport::parse_floatf(vs_config->getVariable("graphics", "hud", "shield_vdu_thresh0", do_shield_fade ? "0" : ".25")),
        XMLSupport::parse_floatf(vs_config->getVariable("graphics", "hud", "shield_vdu_thresh1", do_shield_fade ? ".33" : ".50")),
        XMLSupport::parse_floatf(vs_config->getVariable(
            "graphics", "hud", "shield_vdu_thresh2", do_shield_fade ? ".66" : ".75"))}; // PM me if you don't know why I did this.
...
void VDU::DrawTarget(GameCockpit *cp, std::shared_ptr<Unit> parent, std::shared_ptr<Unit> target)
{
    ...
    DrawHUDSprite(this,
                  ((target->isUnit() != PLANETPTR || target->getHudImage() != NULL)
                       ? target->getHudImage()
    ...
    if (inrange) {
        ...
        if (automatte)
            tp->bgcol = GFXColor(0, 0, 0, background_alpha);
        if (draw_auto_message)
            tp->Draw(MangleString(std::string("\n") + cp->autoMessage, _Universe->AccessCamera()->GetNebula() != NULL ? .4 : 0),
                     0,
                     true,
                     false,
                     automatte);
...
void VDU::DrawMessages(GameCockpit *parentcp, std::shared_ptr<Unit> target)
{
    ...
    for (int i = scrolloffset < 0 ? -scrolloffset - 1 : 0; rows_used < row_lim && mc->last(i, lastmsg, message_people, whoNOT); i++) {
        ...
        if (sendtime <= nowtime && (sendtime > nowtime - oldtime || scrolloffset < 0)) {
            if (showStardate) {
                string stardate =
                    _Universe->current_stardate.ConvertFullTrekDate(sendtime + _Universe->current_stardate.GetElapsedStarTime());
...
void VDU::DrawNav(GameCockpit *cp, std::shared_ptr<Unit> you, std::shared_ptr<Unit> targ, const Vector &nav)
{
    ...
    if (automatte)
        tp->bgcol = GFXColor(0, 0, 0, background_alpha);
    tp->Draw(MangleString(navdata + (draw_auto_message ? msg : std::string()), _Universe->AccessCamera()->GetNebula() != NULL ? .4 : 0),
             scrolloffset,
             true,
             true,
             automatte);
...
static void DrawGun(Vector pos, float w, float h, weapon_info::MOUNT_SIZE sz)
{
    ...
    if (sz == weapon_info::NOWEAP) {
        ...
        const float verts[8 * 3] = {
            pos.i,           pos.j - h / 6,           0, pos.i,       pos.j + h / 6, 0, pos.i + 3 * oox, pos.j - h / 6 + 2 * ooy, 0,
            pos.i - 3 * oox, pos.j - h / 6 + 2 * ooy, 0, pos.i + oox, pos.j - h / 6, 0, pos.i + oox,     pos.j + h / 9,           0,
...
void VDU::DrawDamage(std::shared_ptr<Unit> parent)
{
    ...
    static string damage_report_heading =
        XMLSupport::escaped_string(vs_config->getVariable("graphics", "hud", "damage_report_heading", "#00ff00DAMAGE REPORT\n\n"));
    string retval(damage_report_heading);
    retval += fpstring.str;
    unsigned int       numCargo        = parent->numCargo();
    double             percent_working = 0.88;
    static std::string non_repair_screen_cargo =
        vs_config->getVariable("graphics", "hud", "not_included_in_damage_report", "plasteel_hull tungsten_hull isometal_hull");
...
#define REPORTINTEGRATED(which, which_key, which_name_default)                                                                             \
    do {                                                                                                                                   \
        ...
        if (!name.empty()) {                                                                                                               \
            REPORTITEM(parent->pImage->which##Functionality, parent->pImage->which##FunctionalityMax, print_percent_working, name);        \
        }                                                                                                                                  \
    } while (0)
...
#define REPORTINTEGRATEDFLAG(which, which_key, which_name_default)                                                                         \
    ...
    for (unsigned int i = 0; i < numCargo; i++) {
        ...
        if (damaged ||
            (the_cargo.GetCategory().find("upgrades/") == 0 && the_cargo.installed && the_cargo.GetContent().find("mult_") != 0 &&
             the_cargo.GetContent().find("add_") != 0 && non_repair_screen_cargo.find(the_cargo.GetContent()) == std::string::npos)) {
            percent_working = UnitUtil::PercentOperational(parent, the_cargo.content, the_cargo.category, false);
            // retval+=parent->GetManifest (i,parent,parent->GetVelocity())+string (" (")+tostring (int(percent_working*100))+string ("%)"
            // +the_cargo.GetCategory()+"\n");
...
GFXColor MountColor(Mount *mnt)
{
    ...
    case 127:
        ...
        break;
    default:
        // already set default color before switch; other than that, "default" should not happen with the current code, ever, so maybe we
        // have to do something when it does
        break;
    }
    return mountcolor;
}
...
void VDU::DrawWeapon(std::shared_ptr<Unit> parent)
{
    ...
    static string list_empty_mounts_as = vs_config->getVariable(
        "graphics", "hud", "mounts_list_empty", ""); // empty string skips; " ", "n/a", "(empty)" or "-" will show an empty mount
