...
class Texture : public ::VSImage
{
  ...
  public:
    ...
    /// The minimum/maximum texturing coordinates for this texture (have in mind that it is not always enforce, ie. for meshes) (i=s, j=t,
    /// k=u)
  ...
  public:
    ...
    {
        return 1;
    } // Number of layers in the texture. If more than 1, makeActive() sets up multitexturing units, and other use of multitexture units,
      // other than [stage..stage+numLayers), should be avoided. If not enough multitexturing units are available, special fall-back
      // mechanisms should be implmemented by MakeActive() so that it still renders as correctly as possible. In that case, canMultiPass(),
      // numPasses() and MakeActive(stage,pass) could be used to process in multipass mode (one stage at a time, or a stage group at a time,
      // depending on fallback implementation), if possible.
    ...
    {
        return true;
    } // True, if full functionality can be achieved after multipass rendering. If false, fallback rendering must be possible by multipass
      // (or singlepass) rendering. True, also, if texturing units are sufficient for singlepass rendering.
