        ...
        if (!vidMode) {
            ...
            try {
                ...
                if (vidSource->seek(curtime)) {
                    ...
                    VSFileSystem::vs_dbg(1) << "Transferring video frame" << std::endl;
                ...
                VSFileSystem::vs_dbg(1) << boost::format("\nVidFile exception: %1%") % e.what() << std::endl;
            }
    ...
    for (; i < static_cast<int>(numframes); i++)
        if (loadall || (i == midframe)) { // FIXME midframe used without guaranteed initialization
            ...
            if (vidMode) {
                ...
                frames_mintc.push_back(Vector(XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "mins", defms)),
                ...
                frames_maxtc.push_back(Vector(XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "maxs", defMs)),
                ...
                if (Decal[j - 1]) {
                    Decal[j - 1]->mintcoord = Vector(XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "mins", defms)),
                                                     XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "mint", defmt)),
                                                     XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "minr", defmr)));
                    Decal[j - 1]->maxtcoord = Vector(XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "maxs", defMs)),
                                                     XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "maxt", defMt)),
                                                     XMLSupport::parse_float(XMLSupport::parse_option_value(opt, "maxr", defMr)));
                }
            }
