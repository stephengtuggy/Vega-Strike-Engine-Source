...
float quadsquare::RecomputeErrorAndLighting(const quadcornerdata &cd)
{
    ...
    V->SetNormal(MakeLightness((Vertex[1].Y - Vertex[3].Y) * OneOverSize, (Vertex[4].Y - Vertex[2].Y) * OneOverSize, V->GetConstVertex()));
    float       v;
    quadsquare *s = GetFarNeighbor(0, cd);
    if (s)
        v = s->Vertex[0].Y;
    ...
    V->SetNormal(MakeLightness((v - Vertex[0].Y) * OneOverSize, (cd.Verts[3].Y - cd.Verts[0].Y) * OneOverSize, V->GetConstVertex()));
    s = GetFarNeighbor(1, cd);
    if (s)
        v = s->Vertex[0].Y;
    else
    ...
    V->SetNormal(MakeLightness((cd.Verts[0].Y - cd.Verts[1].Y) * OneOverSize, (Vertex[0].Y - v) * OneOverSize, V->GetConstVertex()));
    s = GetFarNeighbor(2, cd);
    if (s)
        v = s->Vertex[0].Y;
    else
    ...
    V->SetNormal(MakeLightness((Vertex[0].Y - v) * OneOverSize, (cd.Verts[2].Y - cd.Verts[1].Y) * OneOverSize, V->GetConstVertex()));
    ...
    V->SetNormal(MakeLightness((cd.Verts[3].Y - cd.Verts[2].Y) * OneOverSize, (v - Vertex[0].Y) * OneOverSize, V->GetConstVertex()));
    vertices->EndMutate();
