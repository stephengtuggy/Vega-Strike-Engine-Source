...
PipelinedTexture::PipelinedTexture(unsigned int width, unsigned int height, unsigned char *current, unsigned char *last) : Texture()
{
    ...
    GFXCreateTexture(
        sizeX, sizeY, (mode == _24BITRGBA) ? RGBA32 : ((mode == _8BIT) ? PALETTE8 : RGB32), &this->current, NULL, (stage == 1) ? 0 : 1);
    GFXCreateTexture(
        sizeX, sizeY, (mode == _24BITRGBA) ? RGBA32 : ((mode == _8BIT) ? PALETTE8 : RGB32), &this->last, NULL, (stage == 1) ? 0 : 1);
    if (current)
        GFXTransferTexture(
            current, this->current, sizeX, sizeY, (mode == _24BITRGBA) ? RGBA32 : ((mode == _8BIT) ? PALETTE8 : RGB32), TEXTURE_2D);
    if (last)
        GFXTransferTexture(
            last, this->last, sizeX, sizeY, (mode == _24BITRGBA) ? RGBA32 : ((mode == _8BIT) ? PALETTE8 : RGB32), TEXTURE_2D);
}
...
void PipelinedTexture::Swap()
{
    ...
    if (clone) {
        clone--;
        GFXCreateTexture(
            sizeX, sizeY, (mode == _24BITRGBA) ? RGBA32 : ((mode == _8BIT) ? PALETTE8 : RGB32), &name, NULL, (stage == 1) ? 0 : 1);
    }
}
