...
const EnumMap::Pair MeshXML::attribute_names[] = {EnumMap::Pair("UNKNOWN", MeshXML::UNKNOWN),
...
void Mesh::beginElement(MeshXML *xml, const string &name, const AttributeList &attributes)
{
    ...
    case MeshXML::LOCATION:
        ...
        assert((xml->point_state & (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z)) == (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z));
        break;
    case MeshXML::NORMAL:
        ...
        if ((xml->point_state & (MeshXML::P_I | MeshXML::P_J | MeshXML::P_K)) != (MeshXML::P_I | MeshXML::P_J | MeshXML::P_K)) {
            if (!xml->recalc_norm) {
    ...
    case MeshXML::LOD:
        ...
        break;
    case MeshXML::VERTEX: {
        assert(top == MeshXML::TRI || top == MeshXML::QUAD || top == MeshXML::LINE || top == MeshXML::TRISTRIP || top == MeshXML::TRIFAN ||
               top == MeshXML::QUADSTRIP || top == MeshXML::LINESTRIP);
        // assert(xml->load_stage==4);
        ...
        assert((xml->vertex_state & (MeshXML::V_POINT | MeshXML::V_S | MeshXML::V_T)) == (MeshXML::V_POINT | MeshXML::V_S | MeshXML::V_T));
...
void Mesh::endElement(MeshXML *xml, const string &name)
{
    ...
    case MeshXML::UNKNOWN:
        ...
        break;
    case MeshXML::POINT:
        assert((xml->point_state & (MeshXML::P_X | MeshXML::P_Y | MeshXML::P_Z | MeshXML::P_I | MeshXML::P_J | MeshXML::P_K)) ==
...
Mesh *
Mesh::LoadMesh(const char *filename, const Vector &scale, int faction, Flightgroup *fg, const std::vector<std::string> &overridetextures)
{
...
vector<Mesh *>
Mesh::LoadMeshes(const char *filename, const Vector &scale, int faction, Flightgroup *fg, const std::vector<std::string> &overrideTextures)
{
    /*
...
void Mesh::LoadXML(
    const char *filename, const Vector &scale, int faction, Flightgroup *fg, bool origthis, const vector<string> &textureOverride)
{
    VSFile  f;
    VSError err = f.OpenReadOnly(filename, MeshFile);
    if (err > Ok) {
...
void Mesh::LoadXML(
    VSFileSystem::VSFile &f, const Vector &scale, int faction, Flightgroup *fg, bool origthis, const vector<string> &textureOverride)
{
...
static void
SumNormals(vector<GFXVertex> &vertices, vector<int> &indices, size_t begin, size_t end, POLYTYPE polytype, vector<float> &weights)
{
    int    flip = 0;
    size_t i;
    switch (polytype) {
...
static void
SumTangents(vector<GFXVertex> &vertices, vector<int> &indices, size_t begin, size_t end, POLYTYPE polytype, vector<float> &weights)
{
    int    flip = 0;
    size_t i;
    switch (polytype) {
...
void Mesh::PostProcessLoading(MeshXML *xml, const vector<string> &textureOverride)
{
    ...
    if (xml->sharevert) {
        vlist = new GFXVertexList((polytypes.size() ? &polytypes[0] : 0),
                                  xml->vertices.size(),
                                  (xml->vertices.size() ? &xml->vertices[0] : 0),
                                  o_index,
