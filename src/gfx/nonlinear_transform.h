...
class SphericalTransform : public IdentityTransform
{
  ...
  public:
    ...
    CLIPSTATE BoxInFrustum(Vector &min, Vector &max, const Vector &campos) const
    {
         ...
         *  if (tmpx>.25*GetZ()&&tmpx<.75*GetZ()&&maxx>.25*GetZ()&&maxx<.75*GetZ()) {      return GFX_NOT_VISIBLE;//i/f it's on the other
 ...
 *  Vector Origin;
 *  public:
 *  PlanetaryTransform (Vector loc, float r, float scalex, float scaley): SphericalTransform (float r,float scalex, float scaley),
 * origin(loc) {} void SetOrigin (const Vector &t) {Origin = t;} ~PlanetaryTransform () {while (1);} Vector Transform (const Vector & v)
 * {return Origin+SphericalTransform::Transform(v);} Vector TransformNormal (const Vector &p, const vector & n){return
 * SphericalTransform::TransformNormal (p,n);} Vector InvTransform (const Vector &v) {return SphericalTransform::InvTransform (v-Origin);}
