...
class OrigMeshContainer
{
  public:
    ...
    OrigMeshContainer(Mesh *orig, float d, int passno)
    {
        ...
        this->transparent           = ((pass.blendMode == Technique::Pass::Decal) ||
                             ((pass.blendMode == Technique::Pass::Default) && (orig->blendDst == ZERO) && (orig->blendSrc != DESTCOLOR)))
                                ? 0
                                : 1;
        this->zsort =
            (transparent && ((pass.blendMode != Technique::Pass::Default) || (orig->blendDst != ONE) || (orig->blendSrc != ONE))) ? 1 : 0;
            ...
            for (size_t i = 0, n = apass.getNumTextureUnits(); i < n; ++i) {
                ...
                if (atu.sourceType == btu.sourceType) {
                    if (atu.sourceType == Technique::Pass::TextureUnit::File) {
                        ...
                        const Texture *tb =
                            (btu.sourceIndex < static_cast<int>(b.orig->Decal.size())) ? b.orig->Decal[btu.sourceIndex] : NULL;
                        PLESSX(ta, tb);
                    }
                } else {
...
Texture *Mesh::TempGetTexture(MeshXML *xml, int index, std::string factionname) const
{
    ...
    if (zt->animated_name.length()) {
        ...
        if (zt->alpha_name.length() == 0) {
            ...
            tex            = new Texture(
                temptex.c_str(), 0, MIPMAP, TEXTURE2D, TEXTURE_2D, (g_game.use_ship_textures || xml->force_texture) ? GFXTRUE : GFXFALSE);
            if (!tex->LoadSuccess()) {
                delete tex;
                tex = new Texture(zt->decal_name.c_str(),
                                  0,
                                  MIPMAP,
                                  TEXTURE2D,
                                  TEXTURE_2D,
...
void SetupCloakState(char cloaked, const GFXColor &CloakFX, vector<int> &specialfxlight, unsigned char hulldamage, unsigned int matnum)
{
    if (cloaked & MeshDrawContext::CLOAK) {
        ...
        if (cloaked & MeshDrawContext::GLASSCLOAK) {
            ...
            int ligh;
            GFXCreateLight(
                ligh, GFXLight(true, GFXColor(0, 0, 0, 1), GFXColor(0, 0, 0, 1), GFXColor(0, 0, 0, 1), CloakFX, GFXColor(1, 0, 0)), true);
            specialfxlight.push_back(ligh);
            GFXBlendMode(ONE, ONE);
            GFXSelectMaterialHighlights(matnum, GFXColor(1, 1, 1, 1), GFXColor(1, 1, 1, 1), GFXColor(1, 1, 1, 1), CloakFX);
        } else {
            GFXEnable(TEXTURE1);
                           ...
                           bool                  nomultienv)
{
    ...
    if (nomultienv) {
        GFXSelectMaterialHighlights(mat, GFXColor(1, 1, 1, 1), GFXColor(1, 1, 1, 1), GFXColor(0, 0, 0, 0), GFXColor(0, 0, 0, 0));
        GFXBlendMode(ONE, ONE);
        GFXDepthFunc(LEQUAL);
    }
    bool retval         = false;
...
void SetupSpecMapSecondPass(
    Texture *decal, unsigned int mat, BLENDFUNC blendsrc, bool envMap, const GFXColor &cloakFX, float polygon_offset)
{
    GFXPushBlendMode();
    GFXSelectMaterialHighlights(
        mat, GFXColor(0, 0, 0, 0), GFXColor(0, 0, 0, 0), cloakFX, (envMap ? GFXColor(1, 1, 1, 1) : GFXColor(0, 0, 0, 0)));
    GFXBlendMode(ONE, ONE);
...
void Mesh::activateTextureUnit(const Technique::Pass::TextureUnit &tu, bool deflt)
{
...
#ifdef NV_CUBE_MAP
        if (tu.texKind != Technique::Pass::TextureUnit::TexDefault && tu.texKind != Technique::Pass::TextureUnit::TexCube &&
    ...
    case Technique::Pass::TextureUnit::Detail:
        ...
        if (detailTexture)
            ...
            throw MissingTexture("Texture Unit for technique requested a missing texture (detail default given that cannot be found)");
        break;
...
void Mesh::ProcessShaderDrawQueue(size_t whichpass, int whichdrawqueue, bool zsort, const QVector &sortctr)
{
    if (!technique->isCompiled(GFXGetProgramVersion())) {
        try {
            ...
            VSFileSystem::vs_dbg(1) << boost::format("Technique recompilation failed: %1%") % e.what() << std::endl;
        }
    }
    ...
    for (int i = 0, n = cur_draw_queue.size(); i < n; ++i) {
        ...
        if (c.mesh_seq == whichdrawqueue) {
            ...
            for (size_t iter = 0, lightnum = 0, nlights = lights.size();
                ...
                GFXUploadLightState(numLightsParam,
                ...
                for (unsigned int spi = 0; spi < pass.getNumShaderParams(); ++spi) {
                    ...
                    if (sp.id >= 0) {
                        ...
                        case Technique::Pass::ShaderParam::Damage4:
                            GFXShaderConstant(sp.id, c.damage / 255.f, c.damage / 255.f, c.damage / 255.f, c.damage / 255.f);
                            break;
...
void Mesh::ProcessFixedDrawQueue(size_t techpass, int whichdrawqueue, bool zsort, const QVector &sortctr)
{
    ...
    if (cur_draw_queue.empty()) {
        ...
        if (!thiserrdone) {
            VSFileSystem::vs_fprintf(
                stderr, "cloaking queues issue! Report to hellcatv@hotmail.com\nn%d\n%s", whichdrawqueue, hash_name.c_str());
        }
        thiserrdone = true;
        return;
    }
    ...
    bool               splitpass1     = (wantsplitpass1 && getEnvMap() && ((mat.sr != 0) || (mat.sg != 0) || (mat.sb != 0))) ||
                      (getEnvMap() && !gl_options.Multitexture && (ENVSPEC_PASS < DecalSize) &&
                       (!HASDECAL(ENVSPEC_TEX))); // For now, no PPL reflections with no multitexturing - There is a way, however, doing it
        ...
        if ((nomultienv && whichpass == ENVSPEC_PASS) || !whichpass || HASDECAL(whichpass)) {
            ...
            case BASE_PASS:
                SetupSpecMapFirstPass(Decal,
                                      ...
                                      detailPlanes,
                                      skipglowpass,
                                      nomultienv && HASDECAL(ENVSPEC_TEX));
                break;
            case ENVSPEC_PASS:
                if (!nomultienv)
        ...
        for (unsigned int ind = 0; ind < xml->logos.size(); ind++)
            if (xml->logos[ind].type == index) {
                ...
                if (xml->logos[ind].refpnt.size() > 2) {
                    if (xml->logos[ind].refpnt[0] < static_cast<int>(xml->vertices.size()) && xml->logos[ind].refpnt[0] >= 0 &&
                        xml->logos[ind].refpnt[1] < static_cast<int>(xml->vertices.size()) && xml->logos[ind].refpnt[1] >= 0 &&
                        xml->logos[ind].refpnt[2] < static_cast<int>(xml->vertices.size()) && xml->logos[ind].refpnt[2] >= 0) {
