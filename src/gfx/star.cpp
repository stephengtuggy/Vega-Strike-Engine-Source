...
static GFXColorVertex *AllocVerticesForSystem(std::string our_system_name, float spread, int *num, int repetition)
{
    ...
    VSFileSystem::vs_fprintf(stderr,
                             "Min (%f, %f, %f) Max(%f, %f, %f) MinLumin %f, MaxLumin %f",
                             starmin.i,
                             starmin.j,
                             starmin.k,
                             starmax.i,
    ...
    for (int y = 0; y < *num; ++y) {
        ...
        if (our_system_name.size() > 0 && !si.Done()) {
            ...
            float distance =
                Vector(tmpvertex[j + repetition - 1].x, tmpvertex[j + repetition - 1].y, tmpvertex[j + repetition - 1].z).Magnitude();
...
bool PointStarVlist::BeginDrawState(
    const QVector &center, const Vector &velocity, const Vector &torque, bool roll, bool yawpitch, int whichTexture)
{
    UpdateGraphics();
    static bool StarStreaks = XMLSupport::parse_bool(vs_config->getVariable("graphics", "star_streaks", "false"));
...
void Stars::Draw()
{
    ...
    if (fade) {
        ...
        GFXLight fadeLight(true,
                           GFXColor(cp.i, cp.j, cp.k),
                           GFXColor(0, 0, 0, 1),
                           GFXColor(0, 0, 0, 1),
                           GFXColor(1, 1, 1, 1),
    ...
    for (LC = 0; LC < LN; ++LC) {
        bool stretch = vlist->BeginDrawState(_Universe->AccessCamera()->GetR().Scale(-spread).Cast(),
                                             ...
                                             _Universe->AccessCamera()->GetAngularVelocity(),
                                             false,
                                             false,
                                             LC);
        int  i;
...
static void
upd(double &a, double &b, double &c, double &d, double &e, double &f, double &g, double &h, double &i, const double cp, const float spread)
{
...
bool SpriteStarVlist::BeginDrawState(
    const QVector &center, const Vector &velocity, const Vector &torque, bool roll, bool yawpitch, int whichTex)
{
    UpdateGraphics();
    GFXEnable(TEXTURE0);
