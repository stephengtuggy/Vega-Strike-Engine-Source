...
template <typename L, typename R, typename RV> vector_binop<L, R, std::plus<RV>> operator+(const L &l, const R &r)
{
    return vector_binop<L, R, std::plus<RV>>(l, r);
}

template <typename L, typename R, typename RV> vector_binop<L, R, std::minus<RV>> operator-(const L &l, const R &r)
{
    return vector_binop<L, R, std::minus<RV>>(l, r);
}

template <typename L, typename R, typename RV> vector_binop<L, R, std::multiplies<RV>> operator*(const L &l, const R &r)
{
    return vector_binop<L, R, std::multiplies<RV>>(l, r);
}

template <typename L, typename R, typename RV> vector_binop<L, R, std::divides<RV>> operator/(const L &l, const R &r)
{
    return vector_binop<L, R, std::divides<RV>>(l, r);
}

template <typename L, typename RV> vector_unop<L, std::negate<RV>> operator-(const L &l)
{
...
{
    return vector_unop<L, std::binder2nd<std::plus<typename L::component_type>>>(l,
                                                                                 std::bind2nd(std::plus<typename L::component_type>(), r));
}

template <typename L>
vector_unop<L, std::binder2nd<std::minus<typename L::component_type>>> operator-(const L &l, typename L::component_type r)
{
    return vector_unop<L, std::binder2nd<std::minus<typename L::component_type>>>(
        l, std::bind2nd(std::minus<typename L::component_type>(), r));
}

template <typename L>
vector_unop<L, std::binder2nd<std::multiplies<typename L::component_type>>> operator*(const L &l, typename L::component_type r)
{
    return vector_unop<L, std::binder2nd<std::multiplies<typename L::component_type>>>(
        l, std::bind2nd(std::multiplies<typename L::component_type>(), r));
}
...
{
    return vector_unop<L, std::binder2nd<std::multiplies<typename L::component_type>>>(
        l, std::bind2nd(std::multiplies<typename L::component_type>(), typename L::component_type(1) / r));
}

template <typename R>
vector_unop<std::binder1st<std::plus<typename R::component_type>>, R> operator+(typename R::component_type l, const R &r)
{
    return vector_unop<std::binder1st<std::plus<typename R::component_type>>, R>(r,
                                                                                 std::bind1st(std::plus<typename R::component_type>(), l));
}

template <typename R>
vector_unop<std::binder1st<std::minus<typename R::component_type>>, R> operator-(typename R::component_type l, const R &r)
{
    return vector_unop<std::binder1st<std::minus<typename R::component_type>>, R>(
        r, std::bind1st(std::minus<typename R::component_type>(), l));
}

template <typename R>
vector_unop<std::binder1st<std::multiplies<typename R::component_type>>, R> operator*(typename R::component_type l, const R &r)
{
    return vector_unop<std::binder1st<std::multiplies<typename R::component_type>>, R>(
        r, std::bind1st(std::multiplies<typename R::component_type>(), l));
}

template <typename R>
vector_unop<std::binder1st<std::divides<typename R::component_type>>, R> operator/(typename R::component_type l, const R &r)
{
    return vector_unop<std::binder1st<std::divides<typename R::component_type>>, R>(
        r, std::bind1st(std::divides<typename R::component_type>(), l));
}

namespace vectorized
{

template <typename L, typename OP> vector_unop<L, OP> map(const L &l, OP op = OP())
{
...
template <typename T> class vector_output
{
  ...
  public:
    ...
    template <typename R> vector_output<T> &operator=(const R &r)
    {
        assert(size() == r.size());
...
template <typename T> vector_input<T> vectorize(const T &v)
{
    return vector_input<T>(v);
}

template <typename T> vector_output<T> vectorize(T &v)
{
