                                ...
                                const std::vector<std::string> &overrideTextures)
{
    ...
    uint32bit NUMFIELDSPERREFERENCEDANIMATION =
        VSSwapHostIntToLittle(inmemfile[word32index + 4].i32val); // Number of fields per referenced animation: integer (1)
    ...
    for (uint32bit recordindex = 0; recordindex < numrecords; recordindex++) {
        ...
        for (uint32bit meshindex = 0; meshindex < nummeshes; meshindex++) {
            ...
            bxmfprintf(Outputfile,
                       "<Mesh scale=\"%f\" reverse=\"%d\" forcetexture=\"%d\" sharevert=\"%d\" polygonoffset=\"%f\" blendmode=\"%s %s\" "
                       "alphatest=\"%f\" ",
                       scale,
                       reverse,
                       forcetexture,
            ...
            if (detailtexturename.size() != 0) {
                ...
                mesh->detailTexture = mesh->TempGetTexture(
                    &xml, detailtexturename, FactionUtil::GetFaction(xml.faction), GFXTRUE); // LoadTexture(detailtexturename);
            } else {
            ...
            for (uint32bit detplane = 0; (unsigned int)detplane < Detailplanes.size(); detplane++) {
                ...
                mesh->detailPlanes.push_back(Vector(Detailplanes[detplane].x, Detailplanes[detplane].y, Detailplanes[detplane].z));
            }
            // Logos
