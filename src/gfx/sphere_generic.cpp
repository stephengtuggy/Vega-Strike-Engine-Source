                            ...
                            bool               subclass)
{
    ...
    hash_name = string("@@Sphere") + "#" + texture + "#" + technique + "#" + XMLSupport::tostring(stacks) + "#" +
                XMLSupport::tostring(slices) + ab + "#" + XMLSupport::tostring(rho_min) + "#" + XMLSupport::tostring(rho_max);
...
#undef g_theta
        ...
        while (inputtex.length()) {
            ...
            if (thistex.find(".ani") != string::npos) {
                ...
                if (alpha) {
                    Decal[count] = createTexture(thistex.c_str(),
                                                 alpha,
                                                 0,
                                                 mipmap,
                                                 TEXTURE2D,
                    ...
                    Decal[count] = createTexture(
                        thistex.c_str(), 0, mipmap, TEXTURE2D, TEXTURE_2D, (Insideout || g_game.use_planet_textures) ? GFXTRUE : GFXFALSE);
                }
            }
            count++;
        }
    ...
    : SphereMesh()
{
    ...
    wrapy = zzwrapy;
    FILTER filter =
        (FILTER)XMLSupport::parse_int(vs_config->getVariable("graphics", "CityLightFilter", XMLSupport::tostring(((int)TRILINEAR))));
    InitSphere(radius,
               stacks,
               slices,
               texture,
