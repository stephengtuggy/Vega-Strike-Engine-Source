...
unsigned int quadsquare::SetVertices(GFXVertex *vertexs, const quadcornerdata &pcd)
{
    ...
    for (unsigned int i = 0; i < 5; i++) {
        v[i].j = Vertex[i].Y;
        vertexs[Vertex[i].vertindex].SetTexCoord(nonlinear_trans->TransformS(v[i].i, (*textures)[Vertex[i].GetTex()].scales),
...
void quadsquare::AddHeightMapAux(const quadcornerdata &cd, const HeightMapInfo &hm)
{
    ...
    if (cd.xorg > hm.XOrigin + ((int)(hm.XSize + 2) << hm.Scale) || cd.xorg + BlockSize < hm.XOrigin - (1 << hm.Scale) ||
    ...
    if (cd.Parent && cd.Parent->Square)
        cd.Parent->Square->EnableChild(
            cd.ChildIndex, *cd.Parent); // causes parent edge verts to be enabled, possibly causing neighbor blocks to be created.
    int i;
    int half = 1 << cd.Level;
