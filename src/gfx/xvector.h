...
class XVector
{
  ...
  public:
    ...
    {
        return XVector(this->j * v.k - this->k * v.j, this->k * v.i - this->i * v.k, this->i * v.j - this->j * v.i);
    }
    ...
    {
        return XVector(i >= 0 ? i : -i, j >= 0 ? j : -j, k >= 0 ? k : -k);
    }
    inline const XVector Transform(const class Matrix &m1) const;

    inline XVector Min(const XVector &other) const
    {
        return XVector((i < other.i) ? i : other.i, (j < other.j) ? j : other.j, (k < other.k) ? k : other.k);
    }
...
{
    return XVector(p.i * v.i + q.i * v.j + r.i * v.k, p.j * v.i + q.j * v.j + r.j * v.k, p.k * v.i + q.k * v.j + r.k * v.k);
}
