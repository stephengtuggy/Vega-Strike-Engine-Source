 ...
 *                        CommandInterpretor->addCommand(leftCommand, "left"); //adding the second left command will automagically override
 *the first
 *        }
 *        ~walkOnPlanet() {
 *                CommandInterpretor->remCommand(leftCommand); //by passing it by pointer we can be assured the right one will be removed,
 *in case commands are added/removed out of order
 *        }
 ...
 *       If you need a quote to pass through the command processor to a function, use \" the same way you'd pass a quote to a variable in
 * c++ ;)
 *
 *
 */
 ...
 *       Example abstract usage: Imbedded HTTP servers, objects which can be placed in game that can execute their own commands (then
 *"taken-over" or "possessed", or given somehow to the player to be used)
 *
 *       Example with psuedo:
 ...
 *                                               buffer += CDTS[counter];
 *                                       }
 *                               } if(buffer.size() > 0) { //POSTS don't end post data with anything, so if we want to process it, we do it
 *now. Headers all terminate with \r\n. commandI::execute(buffer, true, socket);
 *                               }
 *
...
coms *commandI::findCommand(const char *comm, int &sock_in)
{
    ...
    for (iter = findme->rc.begin(); iter < findme->rc.end(); iter++) {
        ...
        if (testCom.Name.length() >= name.length()) {
            ...
            transform(temp.begin(), temp.end(), temp.begin(), static_cast<int (*)(int)>(tolower));
            transform(commandentered2.begin(), commandentered2.end(), commandentered2.begin(), static_cast<int (*)(int)>(tolower));
 ...
 *
 *
 *         Possible optimization for the main execute function ( medium optimization, less unneeded allocated variables in the execute
 *function when not needed) Move findCommand higher up, before the string vector and 1str array are built, and build those depending on the
 *argument type, to avoid excessive string copying when it's not needed, such as when not in console mode but in game mode, when 1 bool is
 *enough to tell the function being called wether the key is pressed or not.
