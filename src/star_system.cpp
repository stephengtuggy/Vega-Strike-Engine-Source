  ...
  public:
    ...
    bool draw(std::shared_ptr<Unit> unit)
    {
        ...
        interpolation_blend_factor =
            calc_blend_factor(saved_interpolation_blend_factor, unit->sim_atom_multiplier, unit->cur_sim_queue_slot, cur_sim_frame);
...
void GameStarSystem::createBackground(StarSystem::StarXML *xml)
{
    ...
    LightMap[0] = new Texture(
        (xml->backgroundname + "_light.cube").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_POSITIVE_X, GFXFALSE, game_options.max_cubemap_size);
    if (LightMap[0]->LoadSuccess() && LightMap[0]->isCube()) {
        LightMap[1] = LightMap[2] = LightMap[3] = LightMap[4] = LightMap[5] = 0;
    } else {
        delete LightMap[0];
        LightMap[0] = new Texture((xml->backgroundname + "_right.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_POSITIVE_X,
                                  GFXFALSE,
                                  game_options.max_cubemap_size);
        LightMap[1] = new Texture((xml->backgroundname + "_left.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_NEGATIVE_X,
                                  ...
                                  LightMap[0]);
        LightMap[2] = new Texture((xml->backgroundname + "_up.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_POSITIVE_Y,
                                  ...
                                  DEFAULT_ADDRESS_MODE,
                                  LightMap[0]);
        LightMap[3] = new Texture((xml->backgroundname + "_down.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_NEGATIVE_Y,
                                  ...
                                  DEFAULT_ADDRESS_MODE,
                                  LightMap[0]);
        LightMap[4] = new Texture((xml->backgroundname + "_front.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_POSITIVE_Z,
                                  ...
                                  DEFAULT_ADDRESS_MODE,
                                  LightMap[0]);
        LightMap[5] = new Texture((xml->backgroundname + "_back.image").c_str(),
                                  1,
                                  TRILINEAR,
                                  CUBEMAP,
                                  CUBEMAP_NEGATIVE_Z,
    ...
    bg = new Background(
        xml->backgroundname.c_str(), xml->numstars, g_game.zfar * .9, filename, xml->backgroundColor, xml->backgroundDegamma);
