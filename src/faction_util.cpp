    ...
    if (fac->secondaryLogo == 0) {
        if (!fac->secondaryLogoName.empty()) {
            if (!fac->secondaryLogoAlphaName.empty())
                fac->secondaryLogo = FactionUtil::createTexture(fac->secondaryLogoName.c_str(), fac->secondaryLogoAlphaName.c_str(), true);
...
std::vector<Animation *> *FactionUtil::GetRandCommAnimation(int faction, Unit *un, unsigned char &sex)
{
    ...
    if (siz > 0) {
        for (int i = 0; i < 8 + siz; ++i) {
            ...
            if (tmp->dockable == Faction::comm_face_t::CEITHER || (tmp->dockable == Faction::comm_face_t::CYES && dockable) ||
