...
static GLint
appleBuild2DMipmaps(GLenum target, GLint components, GLsizei width, GLsizei height, GLenum format, GLenum type, const void *data)
{
    GLint     w, h, maxsize;
    void *    image, *newimage;
                                       ...
                                       enum ADDRESSMODE    address_mode)
{
    ...
    if ((mipmap & (MIPMAP | TRILINEAR)) && !isPowerOfTwo(width, dummy)) {
        VSFileSystem::vs_dbg(1) << boost::format("Width %1% not a power of two") % width << std::endl;
    }
    if ((mipmap & (MIPMAP | TRILINEAR)) && !isPowerOfTwo(height, dummy)) {
        VSFileSystem::vs_dbg(1) << boost::format("Height %1% not a power of two") % height << std::endl;
    }
        ...
        *newbuf                      = (unsigned char *)malloc(newheight * newwidth * pixsize * sizeof(unsigned char));
        ...
        for (i = 0; i < newheight; i++, irow[0] += 2 * istride, irow[1] += 2 * istride, orow += ostride)
            for (j = k = 0; j < newwidth; j++, k += pixsize) {
                ...
                (temp[0] += irow[0][(k << 1) + pixsize + 0]), (pixsize > 1) && (temp[1] += irow[0][(k << 1) + pixsize + 1]),
                ...
                (temp[0] += irow[1][(k << 1) + pixsize + 0]), (pixsize > 1) && (temp[1] += irow[1][(k << 1) + pixsize + 1]),
                    (pixsize > 2) && (temp[2] += irow[1][(k << 1) + pixsize + 2]),
                    // Unrolled loop
                    (pixsize > 3) && (temp[3] += irow[1][(k << 1) + pixsize + 3]);

                (orow[k + 0] = (unsigned char)((temp[0] + 3) >> 2)), (pixsize > 1) && (orow[k + 1] = (unsigned char)((temp[1] + 3) >> 2)),
...
GFXBOOL /*GFXDRVAPI*/ GFXTransferSubTexture(
    unsigned char *buffer, int handle, int x, int y, unsigned int width, unsigned int height, enum TEXTURE_IMAGE_TARGET imagetarget)
{
                                         ...
                                         unsigned int              pageIndex)
{
    ...
    if ((textures[handle].mipmapped & (TRILINEAR | MIPMAP)) &&
        ...
        static unsigned char NONPOWEROFTWO[1024] = {255, 127, 127, 255, 255, 255, 0, 255, 255, 255, 0, 255, 255, 127, 127, 255};
    ...
    if (internformat != PALETTE8 && internformat != PNGPALETTE8) {
...
#endif
        {
            ...
            GFXTransferTexture(
                tbuf, handle, textures[handle].iwidth, textures[handle].iheight, RGBA32, imagetarget, maxdimension, detail_texture);
            free(tbuf);
        }
    }
    if (tempbuf)
