...
bool vsVendorMatch(const char *vendor)
{
    ...
    if (_glvendor == NULL) {
        ...
        if (_glvendor != NULL) {
            VSFileSystem::vs_dbg(1) << boost::format("OpenGL Vendor: %1%") % (const char *)_glvendor << std::endl;
        }
    }
...
void init_opengl_extensions()
{
    ...
    VSFileSystem::vs_dbg(3) << boost::format("OpenGL Extensions supported: %1%") % extensions << std::endl;
    ...
    if ((glMultiTexCoord2fARB_p && glMultiTexCoord4fARB_p && glClientActiveTextureARB_p && glActiveTextureARB_p) &&
        ...
        if (gl_options.Multitexture) {
            VSFileSystem::vs_dbg(3) << boost::format("OpenGL::Multitexture supported (%1% units)") % gl_options.Multitexture << std::endl;
        }
    } else {
        gl_options.Multitexture = 0;
    ...
    if (vsExtensionSupported("GL_ARB_framebuffer_sRGB") || vsExtensionSupported("GL_EXT_framebuffer_sRGB")) {
        ...
        if (srgbCapable) {
            ...
            VSFileSystem::vs_dbg(1) << "OpenGL::sRGB Framebuffer unsupported by visual" << std::endl;
    ...
    VSFileSystem::vs_dbg(1) << boost::format("Max vertex array indices: %1%") % gl_options.max_array_indices << std::endl;
    VSFileSystem::vs_dbg(1) << boost::format("Max vertex array vertices: %1%") % gl_options.max_array_vertices << std::endl;
}
...
void GFXInit(int argc, char **argv)
{
    ...
    bool textsupported = (vsExtensionSupported("GL_ARB_texture_non_power_of_two") || vsExtensionSupported("GL_ARB_texture_rectangle") ||
    ...
    if (gl_options.rect_textures) {
        ...
        VSFileSystem::vs_dbg(3) << boost::format("RECT max texture dimension: %1%") % max_rect_dimension << std::endl;
    }
