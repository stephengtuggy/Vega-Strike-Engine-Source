...
static bool setup_sdl_video_mode()
{
    ...
    if ((screen = SDL_SetVideoMode(width, height, bpp, video_flags)) == NULL) {
        VSFileSystem::vs_dbg(1) << boost::format("Couldn't initialize video: %1%") % SDL_GetError() << std::endl;
        for (int counter = 0; screen == NULL && counter < 2; ++counter) {
            for (int bpd = 4; bpd > 1; --bpd) {
                ...
                if ((screen = SDL_SetVideoMode(width, height, bpp, video_flags | SDL_ANYFORMAT)) == NULL)
                    VSFileSystem::vs_dbg(1) << boost::format("Couldn't initialize video bpp %1% depth %2%: %3%") % bpp % (bpd * 8) %
                                                   SDL_GetError()
                                            << std::endl;
                else
                    break;
    ...
    VSFileSystem::vs_dbg(3) << boost::format("Setting Screen to w %1% h %2% and pitch of %3% and %4% bpp %5% bytes per pix mode") %
        ...
        while (SDL_PollEvent(&event)) {
            ...
            case SDL_KEYDOWN:
                if (keyboard_func) {
                    ...
                    VSFileSystem::vs_dbg(2) << boost::format("Kbd: %s mod:%x sym:%x unicode:%x sh:%c u:%c mu:%c") %
                    ...
                    key = is_unicode ? ((shifton) ? shiftdown(event.key.keysym.unicode) : event.key.keysym.unicode) : event.key.keysym.sym;
                    // Send the event
...
void winsys_atexit(winsys_atexit_func_t func)
{
    ...
    if (called != false) {
        VSFileSystem::vs_dbg(1) << "winsys_atexit called twice" << std::endl;
    }
    called = true;
}
...
static void glut_keyboard_cb(unsigned char ch, int x, int y)
{
    if (keyboard_func) {
        ...
        if (gm) {
            VSFileSystem::vs_dbg(3) << boost::format("Down Modifier %d for char %d %c") % gm % (int)ch % ch << std::endl;
        }
...
static void glut_keyboard_up_cb(unsigned char ch, int x, int y)
{
    if (keyboard_func) {
        int gm = glutGetModifiers();
        if (gm) {
            VSFileSystem::vs_dbg(3) << boost::format("Up Modifier %d for char %d %c") % gm % (int)ch % ch << std::endl;
        }
        if (gm & GLUT_ACTIVE_CTRL)
            ch = AdjustKeyCtrl(ch);
...
void winsys_init(int *argc, char **argv, char const *window_title, char const *icon_title)
{
    ...
    VSFileSystem::vs_dbg(3) << boost::format("Game Mode Params %1%x%2% at depth %3% @ %4% Hz") % glutGameModeGet(GLUT_GAME_MODE_WIDTH) %
