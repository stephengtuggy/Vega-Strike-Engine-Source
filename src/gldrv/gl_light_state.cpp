...
{
    error               = false;
    float tot_intensity = ((specular[0] + specular[1] + specular[2]) * specular[3] + (diffuse[0] + diffuse[1] + diffuse[2]) * diffuse[3] +
    ...
    // - (double)attenuate[0])))/(2*(double)attenuate[2]); simplistic calculation above causes floating point inaccuracies
