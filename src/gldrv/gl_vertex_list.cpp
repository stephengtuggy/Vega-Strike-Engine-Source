...
void GFXOptimizeList(GFXVertex *old, int numV, GFXVertex **nw, int *nnewV, unsigned int **ind)
{
    ...
    *nnewV = _nnewV;
    ...
    VSFileSystem::vs_dbg(3) << boost::format("Optimized vertex list - vertices: %1% -> %2%") % numV % *nnewV << std::endl;
}
...
void GFXVertexList::ColVtxCopy(GFXVertexList *thus, GFXVertex *dst, int offset, int howmany)
{
    for (int i = 0; i < howmany; i++)
        dst[i]
            ...
            .SetTangent(Vector(thus->data.colors[i + offset].tx, thus->data.colors[i + offset].ty, thus->data.colors[i + offset].tz),
...
void GFXVertexList::GetPolys(GFXVertex **vert, int *numpolys, int *numtris)
{
    ...
    vtxcpy = (changed & HAS_COLOR) ? ((changed & HAS_INDEX) ? ColIndVtxCopy : ColVtxCopy) : ((changed & HAS_INDEX) ? IndVtxCopy : VtxCopy);
