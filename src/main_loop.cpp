        ...
        for (int s = 0; s < fg->nr_ships; s++) {
            ...
            if (fg_terrain == -1 || (fg_terrain == -2 && myterrain == NULL)) {
                            ...
                            for (un_iter ui = cp->activeStarSystem->gravitationalUnits().createIterator(); (grav = *ui) != NULL; ++ui) {
                                ...
                                for (unsigned int i = 0; i < siz; ++i)
                                    if (srcsys == grav->GetDestinations()[i]) {
                                        QVector newpos =
                                            grav->LocalPosition() + QVector(vsrandom.uniformExc(-grav->rSize() / 4, grav->rSize() / 4),
                ...
                if (s == 0 && squadnum < (int)fighter0name.size()) {
                    ...
                    _Universe->AccessCockpit(squadnum)->SetParent(
                        fighters[a], fighter0name[squadnum].c_str(), fighter0mods[squadnum].c_str(), pox);
                }
                if (squadnum < (int)fighter0name.size()) {
...
void AddUnitToSystem(const SavedUnits *su)
{
    ...
    default:
        ...
        un->SetTurretAI();
        if (_Universe->AccessCockpit()->GetParent())
            un->SetPosition(_Universe->AccessCockpit()->GetParent()->Position() + QVector(rand() * 10000. / RAND_MAX - 5000,
...
#ifndef NO_GFX
    VSFileSystem::vs_dbg(3) << boost::format("Drawn %1% vertices in %2% batches") % gl_vertices_this_frame % gl_batches_this_frame
                            << std::endl;
