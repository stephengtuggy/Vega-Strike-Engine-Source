...
void StarDate::InitTrek(string date)
{
    ...
    VSFileSystem::vs_dbg(3) << boost::format("Initializing stardate from a Trek date for %1% factions") % factions.size() << std::endl;
    for (unsigned int i = 0; i < factions.size(); i++)
...
double StarDate::ConvertTrekDate(string date)
{
    ...
    if ((nb = sscanf(date.c_str(), "%da%4d:%3d", &days, &tmphrs, &seconds)) != 3) {
        VSFileSystem::vs_dbg(3) << "!!! ERROR reading date" << std::endl;
    }
    ...
    VSFileSystem::vs_dbg(3) << boost::format("Converted date to %1%, which stardate is %2%") % long(res) % formatted << std::endl;
    return res;
}
