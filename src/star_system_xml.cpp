...
void StarSystem::beginElement(const string &name, const AttributeList &attributes)
{
    ...
    xml->cursun.k                       = 0;
    static GFXColor planet_mat_ambient  = vs_config->getColor("default", "planet_mat_ambient", GFXColor(1.0, 1.0, 1.0, 1.0));
    static GFXColor planet_mat_diffuse  = vs_config->getColor("default", "planet_mat_diffuse", GFXColor(1.0, 1.0, 1.0, 1.0));
    static GFXColor planet_mat_specular = vs_config->getColor("default", "planet_mat_specular", GFXColor(0.0, 0.0, 0.0, 1.0));
    static GFXColor planet_mat_emissive = vs_config->getColor("default", "planet_mat_emmissive", GFXColor(0.0, 0.0, 0.0, 1.0));
    GFXMaterial     ourmat;
    ...
    case SYSTEM:
        ...
        for (iter = attributes.begin(); iter != attributes.end(); ++iter) {
            ...
            case BACKGROUND_COLOR:
                parse_floatfv(
                    (*iter).value, 4, &xml->backgroundColor.r, &xml->backgroundColor.g, &xml->backgroundColor.b, &xml->backgroundColor.a);
                break;
    ...
    case PLANET:
        ...
        for (iter = attributes.begin(); iter != attributes.end(); ++iter) {
            ...
            case SPOWER:
                ...
                break;
            case REFLECTNOLIGHT:
                ourmat.sr = ourmat.sg = ourmat.sb = ourmat.dr = ourmat.dg = ourmat.db = ourmat.ar = ourmat.ag = ourmat.ab = 0;
                break;
            case RI:
        ...
        if (xml->unitlevel > 2) {
            ...
            fprintf(stderr,
                    ...
                    filename.c_str(),
                    technique.c_str());
            std::shared_ptr<Unit> un = xml->moons[xml->moons.size() - 1]->beginElement(
                R,
                S,
                velocity,
    ...
    case CONDITION:
        ...
        xml->conditionStack.push_back((!xml->conditionStack.size() || xml->conditionStack.back()) && ConfigCondition(condition));
        break;
    case UNIT:
    case BUILDING:
    case VEHICLE:
        ...
        if ((!xml->conditionStack.size() || xml->conditionStack.back()) && ConfigAllows(varname, varvalue) && ConfigCondition(condition)) {
            if (((elem == UNIT || elem == NEBULA || elem == ENHANCEMENT || elem == ASTEROID) ||
                ...
                if (elem == UNIT) {
                    ...
                    plan->AddSatellite(
                        un = UnitFactory::createAsteroid(filename.c_str(), faction, fg, fg->nr_ships - 1, scalex < 0 ? -scalex : scalex));
                    un->SetSerial(serial); // FIXME un de-referenced before allocation
                ...
                if ((elem == BUILDING || elem == VEHICLE) && xml->ct == NULL && xml->parentterrain != NULL) {
                    std::shared_ptr<Unit> b =
                        UnitFactory::createBuilding(xml->parentterrain, elem == VEHICLE, filename.c_str(), false, faction, string(""));
                    b->SetSerial(serial);
                    ...
                    std::shared_ptr<Unit> b = UnitFactory::createBuilding(xml->ct, elem == VEHICLE, filename.c_str(), false, faction);
                    b->SetSerial(serial);
                    b->SetPosAndCumPos(xml->cursun.Cast() + xml->systemcentroid.Cast());
                    b->EnqueueAI(new Orders::AggressiveAI("default.agg.xml"));
                    b->SetTurretAI();
                    ...
                    {
                        std::shared_ptr<Unit> un = xml->moons.back();
                        for (unsigned int i = 0; i < dest.size(); ++i)
                            un->AddDestination(dest[i]);
                        dest.clear();
                    }
                    xml->moons.back()->SetAI(new PlanetaryOrbit(xml->moons[xml->moons.size() - 1],
                                                                velocity,
                                                                position,
                                                                R,
                                                                S,
...
void StarSystem::LoadXML(const char *filename, const Vector &centroid, const float timeofyear)
{
    ...
    XML_SetElementHandler(parser, &StarSystem::beginElement, &StarSystem::endElement);
    {
        ...
        VSFileSystem::vs_dbg(2) << "Contents of star system:" << std::endl;
        VSFileSystem::vs_dbg(2) << fcontents << std::endl;
