...
XMLElement::XMLElement()
    : mType(XET_CDATA), mAttributes(), mParent(0), mDocument(0), mIdAttribute(mAttributes.end()), mNameAttribute(mAttributes.end())
{
}
...
namespace ExpatHandlers
{
    ...
    static void StartElement(void *userData, const XML_Char *name, const XML_Char **atts)
    {
        ...
        internals->current =
            internals->current->getChild(internals->current->appendChild(new XMLElement((const char *)name, (const char *const *)atts)));
    }
