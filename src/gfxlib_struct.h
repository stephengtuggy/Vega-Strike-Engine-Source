...
inline GFXColor colLerp(GFXColor a, GFXColor b, float bweight)
{
    ...
    return GFXColor(
        a.r * aweight + b.r * bweight, a.g * aweight + b.g * bweight, a.b * aweight + b.b * bweight, a.a * aweight + b.a * bweight);
}
...
enum LIGHT_TARGET { ATTENUATE = 1, DIFFUSE = 2, SPECULAR = 4, AMBIENT = 8, POSITION = 16, EMISSION = 32 };
...
enum POLYTYPE { GFXTRI, GFXQUAD, GFXLINE, GFXTRISTRIP, GFXQUADSTRIP, GFXTRIFAN, GFXLINESTRIP, GFXPOLY, GFXPOINT };
...
class /*GFXDRVAPI*/ GFXVertexList
{
  ...
  protected:
     ...
     */
    int *offsets;
    /// If vertex list has been mutated since last draw.  Low 3 bits store the stride of the index list (if avail). another bit for if color
    /// is presnet
    char changed;
  ...
  public:
    ...
    inline GFXVertexList(
        enum POLYTYPE poly, int numVertices, const GFXVertex *vertices, int numindices, bool Mutable = false, unsigned int *index = 0)
    {
    ...
    inline GFXVertexList(
        enum POLYTYPE poly, int numVertices, const GFXColorVertex *colors, int numindices, bool Mutable = false, unsigned int *index = 0)
    {
    ...
    /// returns a packed vertex list with number of polys and number of tries to passed in arguments. Useful for getting vertex info from a
    /// mesh
...
enum TEXTURE_TARGET { TEXTURE1D, TEXTURE2D, TEXTURE3D, CUBEMAP, TEXTURERECT };
/// Textures may only be cube maps, Texture1d, Texture2d or Texture3d
 ...
 */
enum STATE { LIGHTING, DEPTHTEST, DEPTHWRITE, COLORWRITE, TEXTURE0, TEXTURE1, CULLFACE, SMOOTH, STENCIL };
 ...
 */
enum ADDRESSMODE { WRAP, MIRROR, CLAMP, BORDER, DEFAULT_ADDRESS_MODE };
 ...
 */
enum DEPTHFUNC { NEVER, LESS, EQUAL, LEQUAL, GREATER, NEQUAL, GEQUAL, ALWAYS };

/**
 * Used for stencil writes
 */
enum STENCILOP { KEEP, CLEAR, REPLACE, INCR, DECR, INVERT };
