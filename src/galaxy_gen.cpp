...
void MakeBigUnit(int callingentitytype, string name = string(), float orbitalradius = 0)
{
    ...
    for (unsigned int i = 0; i < fullname.size(); i++) {
        ...
        } else if (1 == sscanf(fullname[i].c_str(), "planet%f", &size) || 1 == sscanf(fullname[i].c_str(), "moon%f", &size) ||
                   1 == sscanf(fullname[i].c_str(), "gas%f", &size)) {
                ...
                int    numberofstarbases)
{
    ...
    if ((entitytype == PLANET && temprandom < game_options.AtmosphereProbability) && (!atmosphere.empty())) {
        string NAME = thisname + " Atmosphere";
        {
            ...
            xmllevel++;
            Tab();
            f.Fprintf("<FogElement file=\"atmXatm.bfxm\" ScaleAtmosphereHeight=\"1.0\"  red=\"%f\" blue=\"%f\" green=\"%f\" alpha=\"%f\" "
                      "dired=\"%f\" diblue=\"%f\" digreen=\"%f\" dialpha=\"%f\" concavity=\".3\" focus=\".6\" minalpha=\"0\" "
                      "maxalpha=\"0.7\"/>\n",
                      r,
                      g,
                      b,
            ...
            Tab();
            f.Fprintf(
                "<FogElement file=\"atmXhalo.bfxm\" ScaleAtmosphereHeight=\"1.0\"  red=\"%f\" blue=\"%f\" green=\"%f\" alpha=\"%f\" "
                "dired=\"%f\" diblue=\"%f\" digreen=\"%f\" dialpha=\"%f\" concavity=\"1\" focus=\".6\" minalpha=\"0\" maxalpha=\"0.7\"/>\n",
                r,
                g,
                b,
                a,
    ...
    if (entitytype == PLANET) {
        ...
        if (ringrand < game_options.RingProbability) {
            ...
            if (ringrand < (1 - game_options.DoubleRingProbability)) {
                Tab();
                f.Fprintf("<Ring file=\"%s\" ri=\"%f\" rj=\"%f\" rk=\"%f\" si=\"%f\" sj=\"%f\" sk=\"%f\" innerradius=\"%f\" "
                          "outerradius=\"%f\" wrapx=\"%d\" wrapy=\"%d\" />\n",
                          ringname.c_str(),
                          r.i,
                          r.j,
                          r.k,
...
void MakeMoons(float callingradius, int callingentitytype)
{
    while (planetoffset < stars[staroffset].planets.size() && stars[staroffset].planets[planetoffset].moonlevel == moonlevel) {
        ...
        MakePlanet((.5 + .5 * grand()) * callingradius,
                   callingentitytype == STAR ? PLANET : MOON,
                   infos.name,
                   infos.unitname,
                   infos.technique,
                   infos.num,
...
void beginStar()
{
    ...
    unsigned int i;
    Vector       center =
        generateAndUpdateRS(r, s, radius, false); // WTF why was this commented out--that means all stars start on top of each other
    planetoffset = 0;
    ...
    f.Fprintf(" Red=\"%f\" Green=\"%f\" Blue=\"%f\" ReflectNoLight=\"true\" light=\"%d\">\n",
              ...
              lights[staroffset].b,
              staroffset);
    f.Fprintf("<fog>\n\t<FogElement file=\"atmXatm.bfxm\" ScaleAtmosphereHeight=\".900\" red=\"%f\" blue=\"%f\" green=\"%f\" alpha=\"1.0\" "
              "dired=\"%f\" diblue=\"%f\" digreen=\"%f\" dialpha=\"1\" concavity=\".3\" focus=\".6\" minalpha=\".7\" "
              "maxalpha=\"1\"/>\n\t<FogElement file=\"atmXhalo.bfxm\" ScaleAtmosphereHeight=\".9000\" red=\"%f\" blue=\"%f\" green=\"%f\" "
              "alpha=\"1.0\" dired=\"%f\" diblue=\"%f\" digreen=\"%f\" dialpha=\"1\" concavity=\".3\" focus=\".6\" minalpha=\".7\" "
              "maxalpha=\"1\"/>\n</fog>\n",
              lights[staroffset].r,
              lights[staroffset].g,
              lights[staroffset].b,
...
void generateStarSystem(SystemInfo &si)
{
    ...
    // numstarbases    = (int) (si.numun2*game_options.SmallUnitsMultiplier); This yields 0  in addition to making the preceding statement
    // pointless.   Probably not intended
...
int main(int argc, char **argv)
{
    if (argc < 9) {
        VSFileSystem::vs_fprintf(stderr,
                                 "Usage: starsysgen <seed> <sector>/<system> <sunradius>/<compactness> <numstars> "
    ...
    if (1 == sscanf(argv[8], "AN%d", &numbigthings) || 1 == sscanf(argv[8], "NA%d", &numun[0]))
        ...
        return -1;
    generateStarSystem("./", seed, sectorname, systemname, filen, srad, comp; strtol(argv[4], NULL, 10),
