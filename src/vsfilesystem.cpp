...
ObjSerial getUniqueSerial()
{
    ...
    } while ((iter = usedSerials.find(ret)) != usedSerials.end() && (*iter).second);
    usedSerial(ret, true);
    return ret;
}
    ...
    if (subdir != "") {
        ...
        if (DirectoryExists(moddir + "/" + subdir)) {
            if (FileExists(moddir + "/" + subdir, config_file) >= 0) {
                if (!found)
                    cout << "CONFIGFILE - Found a config file in mods directory, using : " << (moddir + "/" + subdir + "/" + config_file)
                         << endl;
...
void InitPaths(string conf, string subdir)
{
    ...
    /*
      ...
      sharedunits/faction/unitname/ Meshes are in sharedmeshes/ or in current unit that is being loaded Textures are in sharedtextures/ or
      in current unit dir that is being loaded or in the current animation dir that is being loaded or in the current sprite dir that is
      being loeded Sounds are in sharedsounds/ Universes are in universe/ Systems are in "sectors"/ config variable Cockpits are in
      cockpits/ (also a config var) Animations are in animations/ VSSprite are in sprites/ or in ./ (when full subpath is provided) or in
      the current cockpit dir that is being loaded First allocate an empty directory list for each file type
    */
    for (i = 0; i < UnknownFile; i++) {
        vector<string> vec;
...
void VSFile::checkExtracted()
{
    if (q_volume_format == vfmtPK3) {
        if (!pk3_extracted_file) {
            ...
            this->size = pk3size;
            cerr << "EXTRACTING " << (this->subdirectoryname + "/" + this->filename) << " WITH INDEX=" << this->file_index
                 << " SIZE=" << pk3size << endl;
        }
    }
}
...
{
    if (this->Size() < 0) {
        cerr << "Attempt to call ReadFull on a bad file " << this->filename << " " << this->Size() << " " << this->GetFullPath().c_str()
             << endl;
        return string();
    }
    if (!UseVolumes[alt_type] || this->volume_type == VSFSNone) {
