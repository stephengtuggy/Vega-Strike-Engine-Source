...
void /*GFXDRVAPI*/
GFXPickLights(const Vector &center, const float radius, vector<int> &lights, const int maxlights, const bool pickglobals);
...
void /*GFXDRVAPI*/ GFXSelectMaterialHighlights(
    const unsigned int number, const GFXColor &ambient, const GFXColor &diffuse, const GFXColor &specular, const GFXColor &emmissive);
...
/// Sets the Projection matrix to have fov and aspect as follows (fov is field of view in radians, aspect is width/height znear and zfar are
/// clip planes
...
void /*GFXDRVAPI*/ GFXDraw(POLYTYPE type, const float data[], int vnum, int vsize = 3, int csize = 0, int tsize0 = 0, int tsize1 = 0);
