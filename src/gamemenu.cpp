...
void GameMenu::createNetworkControls(GroupControl *serverConnGroup, std::vector<unsigned int> *inputqueue)
{
    ...
    StaticDisplay *portInputText = new TextInputDisplay(inputqueue,
                                                        "\x1b\n \t\r*?\\/|:<>\"!@#$%^&*()[]{},.=_-+`~"
...
void GameMenu::readJoinGameControls(Window *window, string &user, string &pass)
{
    ...
    if (window->findControlById("MultiPlayerAccountServer")->hidden()) {
        ...
        vs_config->setVariable("network", "server_ip", static_cast<TextInputDisplay *>(window->findControlById("VegaserverHost"))->text());
        vs_config->setVariable(
            "network", "server_port", static_cast<TextInputDisplay *>(window->findControlById("VegaserverPort"))->text());
    } else {
        vs_config->setVariable("network", "use_account_server", "true");
        vs_config->setVariable(
            "network", "account_server_url", static_cast<TextInputDisplay *>(window->findControlById("AccountServer"))->text());
    }
    enableNetwork(true);
    if (Network != NULL)
...
bool NetActionConfirm::confirmedJoinGame()
{
    ...
    if (numships) {
        ...
        if (shipList.size() > 1) {
            ...
            showListQuestion(
                err + "  Select a ship to fly, or hit cancel  ", shipList, new ShipSelectorCallback(this, false), "ShipSelected");
        } else {
            if (err.empty())
                finalizeJoinGame(0);
            else
                showYesNoQuestion(
                    "Warning: " + err + "\n\nDo you want to join to this server?", new ShipSelectorCallback(this, true), "ServerWarning");
        }
    } else {
        UniverseUtil::hideSplashScreen();
        if (window())
