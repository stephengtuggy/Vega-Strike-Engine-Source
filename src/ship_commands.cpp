    ...
    ShipCommands()
    {
        ...
        // a test menu {{{
        {
            ...
            }
            {
                ...
                mi->func2call.append(
                    "python"); // call this function when this menuitem is called and input is all recieved, user input is appened with a
                               // space, along with the action string if there is one. (string generated: "func2call action userinput")
                mi->inputbit2 = true; // set single-line input mode
                mi->selectstring.append(
                    "Type multiple lines of python input. Use <ENTER> on a line ALONE to finish"); // Call function "Display" with this
                                                                                                   // string
                CommandInterpretor->addMenuItem(mi);
            }
        }
...
void ShipCommands::setkps(const char *in)
{
    ...
    std::shared_ptr<Unit> player = UniverseUtil::getPlayer();
    if (player) {
