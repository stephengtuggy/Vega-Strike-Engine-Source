...
void StarSystem::Statistics::CheckVitals(StarSystem *ss)
{
    ...
    if (checkIter >= sortedsize && sortedsize > (unsigned int)(enemycount + neutralcount + friendlycount + citizencount) /
    ...
    std::shared_ptr<Unit> un, std::shared_ptr<Unit> jumppoint, const std::string &system, bool force, bool save_coordinates)
{
    ...
    if (Network == NULL || force) {
        ...
        if (ss && !isJumping(pendingjump, un)) {
            ...
            pendingjump.push_back(
                new unorigdest(un,
                               jumppoint,
                               this,
                               ss,
                               un->GetJumpStatus().delay,
                               ani,
                               justloaded,
                               save_coordinates ? ComputeJumpPointArrival(un->Position(), this->getFileName(), system) : QVector(0, 0, 0)));
        } else {
#ifdef JUMP_DEBUG
