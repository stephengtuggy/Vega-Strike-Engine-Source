...
void bootstrap_draw(const std::string &message, Animation *newSplashScreen)
{
    ...
    if (ani) {
        if (GetElapsedTime() < 10)
            ani->UpdateAllFrame();
        {
            ani->DrawNow(tmp); // VSFileSystem::vs_fprintf( stderr, "(new?) splash screen ('animation'?) %d.  ", (long long)ani );
                               // //temporary, by chuck
        }
    }
    bs_tp->Draw(game_options.default_boot_message.length() > 0 ? game_options.default_boot_message
...
void SetStartupView(std::shared_ptr<Cockpit> cp)
{
    cp->SetView(game_options.startup_cockpit_view == "view_target"
                    ? CP_TARGET
                    : (game_options.startup_cockpit_view == "back" ? CP_BACK
...
void bootstrap_main_loop()
{
    ...
    if (LoadMission) {
        ...
        for (k = 0, it = playername.begin(), jt = playerpasswd.begin(); k < (unsigned int)_Universe->numPlayers(); k++, it++, jt++) {
            ...
            if (Network != NULL) {
                ...
                _Universe->AccessCockpit(k)->savegame->ParseSaveGame(
                    "", mysystem, mysystem, pos, setplayerXloc, credits, packedInfo, k, savefiles[k][0], false);
                _Universe->AccessCockpit(k)->TimeOfLastCollision = getNewTime();
            } else {
                if (game_options.load_last_savegame) {
                    _Universe->AccessCockpit(k)->savegame->ParseSaveGame(
                        savegamefile, mysystem, mysystem, pos, setplayerXloc, credits, packedInfo, k);
                } else {
        ...
        if (Network == NULL && mission->getVariable("savegame", "").length() != 0 && game_options.dockOnLoad) {
            for (size_t i = 0; i < _Universe->numPlayers(); i++) {
                QVector vec;
