    ...
    if (button == WS_LEFT_BUTTON) {
        ...
        if (state == WS_MOUSE_UP) {
            ...
            if (button == WS_RIGHT_BUTTON)
                ...
                str = "Please create a file named stdin.txt and type\nin the type of room followed by arguments for the room followed by "
              ...
              vs_config->getColor("FixerTextColor_Background", GFXColor(0, 0, 0, 1)))
{
    ...
    if (!rooms.size()) {
        ...
        BaseUtil::Comp(
            0, "default room", 0, -1, 1, 2, "ERROR: No rooms specified... - Computer", "Cargo Upgrade Info ShipDealer News Missions");
#endif
    }
    GotoLink(0);
    {
...
void BaseInterface::Room::Launch::Click(BaseInterface *base, float x, float y, int button, int state)
{
    if (state == WS_MOUSE_UP) {
        ...
        if (playa && bas) {
            if (((playa->name == "eject") || (playa->name == "ejecting") || (playa->name == "pilot") || (playa->name == "Pilot") ||
                 (playa->name == "Eject")) &&
                (bas->faction == playa->faction))
