...
Terrain::Terrain(const char *filename, const Vector &scales, const float mass, const float radius, updateparity *updatetransform)
...
void Terrain::ApplyForce(Unit *un, const Vector &normal, float dist)
{
    ...
    un->ApplyDamage(un->Position().Cast() - normal * un->rSize(),
                    -normal,
                    .5 * fabs(normal.Dot(un->GetVelocity())) * mass * SIMULATION_ATOM,
                    un,
                    GFXColor(1, 1, 1, 1),
                    NULL);
}
