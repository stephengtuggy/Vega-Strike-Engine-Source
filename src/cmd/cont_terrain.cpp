        ...
        for (i = 0; i < width; i++)
            for (int j = 0; j < width; j++) {
                ...
                if (data[j + width * i] && data[(j + 1) % width + width * i] && data[j + width * ((i + 1) % width)] &&
                    ...
                    data[j + width * i]->SetNeighbors(
                        data[(j + 1) % width + width * i], data[j + width * ((i + 1) % width)], data[nj + width * i], data[j + width * ni]);
...
QVector ContinuousTerrain::GetGroundPosIdentTrans(QVector ShipPos, Vector &norm)
{
    ...
    for (int i = 0; i < numcontterr; i++) {
        QVector tmploc = ShipPos - location[i] +
                         QVector((data[i])->getminX() + .5 * (data[i])->getSizeX(), 0, (data[i])->getminZ() + .5 * (data[i])->getSizeZ());
...
void ContinuousTerrain::Collide(Unit *un, Matrix t)
{
    ...
    for (int i = 0; i < numcontterr; i++) {
        QVector tmp(Transform(
            t,
            location[i] -
                QVector((data[i] ? (data[i])->getminX() : md[i].mesh->corner_min().i) +
                            .5 * (data[i] ? (data[i])->getSizeX() : (md[i].mesh->corner_max().i - md[i].mesh->corner_min().i)),
                        0,
                        (data[i] ? (data[i])->getminZ() : md[i].mesh->corner_min().k) +
                            (.5 * (data[i] ? (data[i])->getSizeZ() : (md[i].mesh->corner_max().i - md[i].mesh->corner_min().i))))));
        ...
        if (data[i]) {
            ...
            if (autocol) {
                ...
                un->ApplyDamage(un->Position().Cast() - bigNormal * un->rSize(),
                                -bigNormal,
                                .5 * fabs(bigNormal.Dot(un->GetVelocity())) * mass * SIMULATION_ATOM,
                                un,
                                GFXColor(1, 1, 1, 1),
                                NULL);
            }
