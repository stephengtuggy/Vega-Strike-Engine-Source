...
void UnitFactory::addUnitBuffer(NetBuffer &netbuf, const Unit *un, string *netxml)
{
    addUnitBuffer(netbuf,
                  ...
                  un->getFlightgroup(),
                  un->getFgSubnumber(),
                  netxml /*For ENTERCLIENT, will generate a saved game netxml*/,
                  un->GetSerial());
}
...
Planet *UnitFactory::parsePlanetBuffer(NetBuffer &netbuf)
{
    ...
    return UnitFactory::createPlanet(x,
                                     y,
                                     vely,
                                     rotvel,
                                     pos,
...
void UnitFactory::addNebulaBuffer(
    NetBuffer &netbuf, const char *unitfile, bool SubU, int faction, Flightgroup *fg, int fg_snumber, ObjSerial netcreate)
{
...
{
    addNebulaBuffer(
        netbuf, neb->getFilename().c_str(), neb->isSubUnit(), neb->faction, neb->getFlightgroup(), neb->getFgSubnumber(), neb->GetSerial());
}
...
{
    addMissileBuffer(netbuf,
                     mis->getFilename().c_str(),
                     mis->name,
                     mis->getFullname(),
                     mis->faction,
                     "" /* modifications */,
                     ...
                     mis->time,
                     mis->radial_effect,
                     mis->radial_multiplier,
                     mis->detonation_radius,
                     mis->GetSerial());
}
...
Missile *UnitFactory::parseMissileBuffer(NetBuffer &netbuf)
{
    ...
    Missile *mis =
        createMissile(file.c_str(), faction, modifs, damage, phasedamage, time, radialeffect, radmult, detonation_radius, serial);
    if (netbuf.version() <= 4951)
...
void UnitFactory::addAsteroidBuffer(
    NetBuffer &netbuf, const char *filename, int faction, Flightgroup *fg, int fg_snumber, float difficulty, ObjSerial netcreate)
{
...
{
    addAsteroidBuffer(netbuf,
                      aster->getFilename().c_str(),
                      aster->faction,
                      aster->getFlightgroup(),
                      aster->getFgSubnumber(),
