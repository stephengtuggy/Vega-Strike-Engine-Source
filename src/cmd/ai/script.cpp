...
void AIScript::endElement(const string &name)
{
    ...
    case MATCHLIN:
        xml->unitlevel--;
        xml->orders.push_back(new Orders::MatchLinearVelocity(
            parent->ClampVelocity(topv(), xml->afterburn), ((bool)xml->acc), xml->afterburn, xml->terminate));
        popv();
        break;
    case MATCHVEL:
        xml->unitlevel--;
...
void AIScript::LoadXML()
{
    ...
    if (iter == hard_coded_scripts.end() && full_filename.length() > 5 && full_filename[0] == 'r' && full_filename[1] == 'o' &&
        full_filename[2] == 'l' && full_filename[3] == 'l' && full_filename[4] == ' ') {
        doroll        = true;
