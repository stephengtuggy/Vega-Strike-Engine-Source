...
const EnumMap::Pair element_names[] = {EnumMap::Pair("AggressiveAI", AggressiveAI::AGGAI),
...
static AIEvents::ElemAttrMap *
getLogicOrInterrupt(string name, int faction, string unittype, vsUMap<string, AIEvents::ElemAttrMap *> &mymap, int personalityseed)
{
    string                           append   = "agg";
...
static AIEvents::ElemAttrMap *
getProperLogicOrInterruptScript(string name, int faction, string unittype, bool interrupt, int personalityseed)
{
...
static AIEvents::ElemAttrMap *getProperScript(std::shared_ptr<Unit> me, std::shared_ptr<Unit> targ, bool interrupt, int personalityseed)
{
    ...
    return getProperLogicOrInterruptScript(
        ROLES::getRoleEvents(me->attackPreference(), targ->unitRole()), me->faction, me->name, interrupt, personalityseed);
}
                    ...
                    if (targ->InCorrectStarSystem(_Universe->activeStarSystem())) {
                        ...
                        if (parent->InRange(targ, true, false)) {
                            ...
                            if (parent == leaderowner) {
                                ...
                                Order *ord =
                                    new Orders::MatchLinearVelocity(parent->ClampVelocity(Vector(0, 0, 0), true), true, false, true);
                ...
                if (leader != NULL) {
                    if (leader->InCorrectStarSystem(_Universe->activeStarSystem())) {
                        ...
                        if (fg->directive != last_directive || (!last_time_insys)) {
                            ...
                            if (o)
                                o->Communicate(c);
                            static float esc_percent =
                                XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "EscortDistance", "10.0"));
                            static float turn_leader =
                                XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "TurnLeaderDist", "5.0"));
                        ...
                        if (fg->directive != last_directive || (!last_time_insys)) {
                            ...
                            // if i am a cargo wingman and so is the player, get into a dockable position with the leader
                                ...
                                Order *ord   = new Orders::MoveTo(
                                    leaderownerun->Position() + Vector(0.5 * Xpos * psize, 0.5 * Ypos * psize, 0.5 * qdist), true, 4);
                                ord->SetParent(parent);
                                ReplaceOrder(ord);
                                // facing it
                            ...
                            // if i am the capship, go into defensive mode
                                ...
                                parentowner = parent;
                                Order *ord  = new Orders::FormUp(
                                    QVector(5 * Xpos * psize, 5 * Ypos * psize, -fabs(formdist) + Ypos * psize + Xpos * psize));
                                ord->SetParent(parent);
                                ReplaceOrder(ord);
                                ord = new Orders::FaceDirection(dist * turn_leader);
                                ord->SetParent(parent);
                    ...
                    if (LeaderPosition.Magnitude() > 0 || leader->InCorrectStarSystem(_Universe->activeStarSystem())) {
                        ...
                        if (fg->directive != last_directive || (!last_time_insys)) {
                            ...
                            if ((parent->owner == leader->owner) || parent->owner == leader) {
                                ...
                                Order *ord   = new Orders::MoveTo(
                                    LeaderPosition + Vector(0.5 * Xpos * psize, 0.5 * Ypos * psize, 0.5 * qdist), true, 4);
                                ord->SetParent(parent);
                                ReplaceOrder(ord);
                                // facing it
                                ord = new Orders::FaceDirection(-qdist * turn_leader);
...
ChooseNearNavPoint(std::shared_ptr<Unit> parent, std::shared_ptr<Unit> suggestion, QVector location, float locradius)
{
    if (suggestion)
        return suggestion;
...
class FlyTo : public Orders::MoveTo
{
  ...
  public:
    FlyTo(const QVector &       target,
          ...
          int                   leniency     = 6,
          std::shared_ptr<Unit> destUnit     = NULL)
        ...
        if (dest) {
            ...
            if (!otherdest) {
                ...
                if (dest->isUnit() == PLANETPTR) {
                    ...
                    dir += ((randVector() * randspacingfactor / 4) + (unitdir * randspacingfactor)) *
