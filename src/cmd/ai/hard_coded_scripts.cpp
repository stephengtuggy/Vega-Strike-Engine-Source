...
class LoopAround : public Orders::FaceTargetITTS
{
  ...
  public:
        ...
        : FaceTargetITTS(false, 3), m(Vector(0, 0, 1000), true, afterburn, false)
    {
        ...
        qq = pp               = 0;
        static float loopdisd = XMLSupport::parse_float(vs_config->getVariable("AI", "loop_around_destination_distance", "20.0"));
    ...
    void Execute()
    {
        ...
        if (targ) {
            ...
            m.SetDesiredVelocity(
                Vector(0, 0, afterburn && ab_needed ? parent->GetComputerData().max_ab_speed() : parent->GetComputerData().max_speed()),
                true);
            ...
            if (r.Dot(relloc) < 0) {
                ...
                if (afterburn)
                    ...
                    m.SetAfterburn(0);
                Vector scala = targ->cumulative_transformation_matrix.getQ().Scale(qq * (parent->rSize() + targ->rSize())) +
    ...
    void Execute()
    {
        ...
        if (targ) {
            ...
            if (r.Dot(relloc) < 0) {
                ...
                m.SetAfterburn(afterburn && ab_needed);
                m.Execute(parent,
                          targ->Position() - r.Scale(rr.k * parent->rSize() + targ->rSize()) +
