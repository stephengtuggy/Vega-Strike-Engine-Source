...
/**
 * The moveto order attempts to calculate the best way to apply thrust (within the computer bound limits) to get a starship to place B and
 * stopped. It uses an integral of acceleration and velocity over time to solve for time when to decelerate.  Is  inaccurate within 1
 * physics frame, and must use switchbacks and then once they have been met sets terminating X,Y, and Z to figure out how many switchbacks
 * it has made , missing the target and coming back over it.
 */
...
class MoveTo : public Order
{
  ...
  public:
    ...
    /// takes in the destination target, whether afterburners should be applied, and the ammount of accuracy (how many times it shoudl miss
    /// destination and come back) should be used
...
class ChangeHeading : public Order
{
  ...
  public:
    /// takes in the destination target, and the ammount of accuracy (how many times it should miss destination and come back) should be
    /// used
    ChangeHeading(const QVector &final_heading, int switchback, float turning_speed = 1, bool term = false)
        : Order(FACING, SLOCATION),
