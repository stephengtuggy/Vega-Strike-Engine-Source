...
namespace AIEvents
{
AIEvresult::AIEvresult(
    int type, float const min, const float max, float timetofinish, float timeuntilinterrupts, float priority, const std::string &aiscript)
{
    ...
    if (!validateHardCodedScript(this->script)) {
        ...
        printf("SERIOUS WARNING in AI script: no fast method to perform %s when type %d is at least %f and at most %f with priority %f for "
               "%f time\n",
               this->script.c_str(),
               type,
               min,
...
const XMLSupport::EnumMap::Pair AIattribute_names[] = {EnumMap::Pair("UNKNOWN", AIUNKNOWN),
...
void LoadAI(const char *filename, ElemAttrMap &result, const string &faction)
{
    ...
    static float cfg_obedience = XMLSupport::parse_float(vs_config->getVariable("AI", "Targetting", "obedience", ".99"));
