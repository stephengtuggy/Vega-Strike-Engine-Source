...
class Order
{
  ...
  protected:
    ...
    /// a bunch of communications that have not been answered CommunicationMessages are actually containing reference to a nice Finite State
    /// Machine that can allow a player to have a reasonable conversation with an AI
  ...
  public:
    ...
    void ClearMessages();
    /// The varieties of order types  MOVEMENT,FACING, and WEAPON orders may not be mutually executed (lest one engine goes left, the other
    /// right)
    ...
    Order()
        : parent(NULL),
          ...
          targetlocation(0, 0, 0){VSCONSTRUCT1('O')} /// The constructor that specifies what order dependencies this order has
          Order(int type, int subtype)
        : parent(NULL), type(type), subtype(subtype), done(false), targetlocation(0, 0, 0)
    {
        VSCONSTRUCT1('O')
    }
