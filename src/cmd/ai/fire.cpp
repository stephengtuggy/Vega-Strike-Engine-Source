...
void FireAt::ChooseTargets(int numtargs, bool force)
{
    ...
    static int minnumpollers = float_to_int(XMLSupport::parse_float(
        vs_config->getVariable("AI",
                               "Targetting",
                               "MinNumberofpollersperframe",
                               "5"))); // maximum number of vessels allowed to search for a target in a given physics frame
    static int maxnumpollers = float_to_int(XMLSupport::parse_float(
        vs_config->getVariable("AI",
                               "Targetting",
    ...
    pollindex[hastarg]++; // count number of craft touched - will use in the next physics frame to spread out the vessels actually chosen to
                          // be processed among all of the vessels being touched
    ...
    lastchangedtarg =
        0 + targrand.uniformInc(0, 1) * mintimetoswitch; // spread out next valid time to switch targets - helps to ease per-frame loads.
    if (fg) {
    ...
    static int   maxtargets      = XMLSupport::parse_int(vs_config->getVariable(
        "AI",
        "Targetting",
    ...
    if (curtarg) {
        if (gcounter++ < min_rechoose_interval || rand() / 8 < RAND_MAX / 9) {
            // in this case only look at potentially *interesting* units rather than huge swaths of nearby units...including target, threat,
            // players, and leader's target
