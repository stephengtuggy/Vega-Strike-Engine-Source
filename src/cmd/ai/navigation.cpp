 ...
 *
 * 8/15/05 Patched Calulate BalancedDecel time: our previous quantization factor ignored the quantization during ACCEL phase and also
 * ignored the fact that we overestimated the integral rather than underestimated new quantization factor is
 * .5*accel*SIMULATION_ATOM*SIMULATION_ATOM-.5*initialVelocity*SIMULATION_ATOM also this threshold idea is silly--accelerate if t>SIM_ATOM
 * decel if t<0  still havent fixed t between 0 and SIM_ATOM...have decent approx for now. 3/2/02  Patched CalculateBalancedDecel time with
 * the fact that length should be more by a quantity of .5*initialVelocity*SIMULATION_ATOM
 *
 */
 ...
 * L = k^2 (.5*a*S^2) +.5*a*k*R*S^2 - .5*d*S^2 (k^2 a^2 / d^2 + 2*(k*a/d)*v/(dS) + v^2/(d^2 s^2) + Pk*a/d + P * v/(d*S)) + v^2/d + S*v*k
 * (a/d + 1) 0 = k^2 (.5*a*S*S*(1-a/d)) + k*S*( .5*R*a*S - .5*P*a*S- .5*2*a*v/d+v(1+a/d)) + v*v/d-.5*v*v/d- P*v*S*.5 - L 0 = (k*S)^2
 * *(.5*a*(1-a/d)) + k*S*v + v^2/(2d) - vS/2 -L kS = -v (+/-) sqrtf (v*v-a*(1-a/d)*(v*v/d-vS-L))/(a*(1-a/d))
 */
// end failed attempt
...
void AutoLongHaul::MakeLinearVelocityOrder()
{
    ...
    float speed = parent->GetComputerData().combat_mode
...
void AutoLongHaul::Execute()
{
    ...
    if ((parent->graphicOptions.WarpFieldStrength < enough_warp_for_cruise) && (parent->graphicOptions.RampCounter == 0)) {
        ...
        if (obstacle != NULL && obstacle != target) {
            ...
            if ((obstacledistance - obstacle->rSize() < destinationdistance - target->rSize()) && (angle > warp_behind_angle)) {
                ...
                double finaldetourdistance =
                    mymax(obstacle->rSize() * warp_orbit_multiplier,
                ...
                QVector newdestination =
                    NewDestination(obstacle->LocalPosition() + detourvector, finaldetourdistance); // add to our position
                float weight =
                    (maxmultiplier - go_perpendicular_speed) /
                    (enough_warp_for_cruise -
                ...
                if (maxmultiplier < go_perpendicular_speed) {
                    ...
                    destination =
                        newdestination * (1 - weight) + olddestination * weight; // use the weight to combine our direction and the dest
                }
            }
        }
    }
    ...
    static float min_warpfield_to_enter_warp = XMLSupport::parse_float(vs_config->getVariable("AI", "min_warp_to_try", "1.5"));
