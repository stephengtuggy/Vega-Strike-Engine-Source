...
void Unit::beginElement(const string &name, const AttributeList &attributes)
{
    ...
    case MESHFILE: {
        ...
        default:
            pushMesh(xml->meshes,
                     xml->randomstartframe,
                     xml->randomstartseconds,
                     file.c_str(),
                     xml->unitscale,
                     faction,
                     flightgroup,
    ...
    case SUBUNIT:
        ...
        indx = xml->units.size();
        xml->units.push_back(
            UnitFactory::createUnit(filename.c_str(), true, faction, xml->unitModifications, NULL)); // I set here the fg arg to NULL
    ...
    *myhudim   = xml->hudimage;
    ...
    for (a = 0; a < xml->mountz.size(); a++) {
        ...
        if (a % 2 == parity) {
            ...
            mounts[a].sound = AUDCreateSound(mounts[a].type->sound,
