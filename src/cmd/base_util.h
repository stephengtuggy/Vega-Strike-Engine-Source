...
void Python(
    int room, std::string index, float x, float y, float wid, float hei, std::string text, std::string pythonfile, bool front = false);
...
void  LoadBaseInterfaceAtDock(std::string name, std::shared_ptr<Unit> dockat, std::shared_ptr<Unit> dockee);
