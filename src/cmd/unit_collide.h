...
template <class CTSIZ, class CTACCURACY, class CTHUGE> class UnitHash3d
{
  ...
  public:
    ...
    {
        return ((int)(((aye < 0) ? (aye - COLLIDETABLEACCURACY) : aye) / COLLIDETABLEACCURACY)) % (COLLIDETABLESIZE / 2) +
               (COLLIDETABLESIZE / 2);
    }
    /// clears entire table
    void Clear()
    ...
    void Put(LineCollide *target, std::shared_ptr<Unit> objectToPut)
    {
        ...
        if (fabs((maxx - minx) * (maxy - miny) * (maxz - minz)) >
            ((double)COLLIDETABLEACCURACY) * ((double)COLLIDETABLEACCURACY) * ((double)COLLIDETABLEACCURACY) * ((double)HUGEOBJECT)) {
            target->hhuge = true;
