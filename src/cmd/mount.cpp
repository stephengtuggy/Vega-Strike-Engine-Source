...
Mount::Mount(const string &filename, int am, int vol, float xyscale, float zscale, float func, float maxfunc, bool banked) : bank(banked)
{
    // short fix
    functionality    = func;
    ...
    if (processed == FIRED) {
        ...
        case weapon_info::PROJECTILE:
            if (Network == NULL || SERVER) {
                ...
                if (err <= Ok) {
                    temp = UnitFactory::createMissile(type->file.c_str(),
                                                      caller->faction,
                                                      "",
                                                      type->Damage,
                                                      type->PhaseDamage,
                                                      ...
                                                      type->Radius,
                                                      type->RadialSpeed,
                                                      type->PulseSpeed /*detonation_radius*/,
                                                      this->serial);
                    ...
                    if (testfg->name == "Base") {
                        int          fgsnumber = 0;
                        Flightgroup *fg        = Flightgroup::newFlightgroup(
                            "Base_Patrol", type->file, FactionUtil::GetFactionName(caller->faction), "deafult", 1, 1, "", "", mission);
                        if (fg != NULL) {
                        ...
                        temp = UnitFactory::createUnit(type->file.c_str(), false, caller->faction, "", fg, fgsnumber, NULL, this->serial);
                    } else {
                ...
                VSFileSystem::vs_dbg(3) << boost::format("Creating missile with SERIAL ID %1%") % this->serial << std::endl;
        ...
        static float exterior_weapon_gain    = XMLSupport::parse_float(vs_config->getVariable("audio", "exterior_weapon_gain", ".35"));
        static float min_weapon_sound_refire = XMLSupport::parse_float(vs_config->getVariable("audio", "min_weapon_sound_refire", ".2"));
