    ...
    if (!dischargedMissiles.empty()) {
        if (dischargedMissiles.back()->GetRadius() >
            ...
            for (un_iter ui = getUnitList().createIterator(); NULL != (un = (*ui)); ++ui) {
                ...
                if (collideroids ||
                    type != ASTEROIDPTR) // could check for more, unless someone wants planet-killer missiles, but what it would change?
...
void MissileEffect::ApplyDamage(Unit *smaller)
{
    ...
    if (distance < radius) {                                // "smaller->isUnit() != MISSILEPTR &&" was removed - why disable antimissiles?
        ...
        norm.Normalize();
        smaller->ApplyDamage(
            pos.Cast(), norm, damage * damage_mul, smaller, GFXColor(1, 1, 1, 1), ownerDoNotDereference, phasedamage * damage_mul);
    }
}
