...
int get_alpha(int _query, int _maxrez_x, int _min, int _max, double _focus, double _concavity, int _tail_mode_start, int _tail_mode_end)
{
    ...
    // double vx1                              = double(x2-x0);				//	mid point slope (flat if at top/bottom, so there is
    // no clipping, due to this, range capped to .2 -> .8 instead of 0 -> 1)
    if (concavity < 0)
    ...
    if (query <= half) {
        ...
        yt =
            int(y0 + (vy0 * _t0) + (((3 * (y1 - y0)) - ((2 * vy0) + vy1)) * pow(_t0, 2)) + (((2 * (y0 - y1)) + (vy0 + vy1)) * pow(_t0, 3)));
    } else {
        double _t1 = _t;
        // xt = x1	+	(vx1 * _t1)
        ...
        yt =
            int(y1 + (vy1 * _t1) + (((3 * (y2 - y1)) - ((2 * vy1) + vy2)) * pow(_t1, 2)) + (((2 * (y1 - y2)) + (vy1 + vy2)) * pow(_t1, 3)));
    }
    int return_alpha = yt;
    if (return_alpha < min)
