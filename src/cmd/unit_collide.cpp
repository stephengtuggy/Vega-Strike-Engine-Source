        ...
        if (!is_null(this->location[locind])) {
            ...
            if (collidemap_sanity_check) {
                if (0) {
                    ...
                    bool found = false;
                    for (i = activeStarSystem->collidemap[locind]->begin(); i != activeStarSystem->collidemap[locind]->end(); ++i) {
...
Matrix WarpMatrixForCollisions(std::shared_ptr<Unit> un, const Matrix &ctm)
{
    if (un->GetWarpVelocity().MagnitudeSquared() * SIMULATION_ATOM * SIMULATION_ATOM < un->rSize() * un->rSize()) {
        ...
        const Vector v(Vector(1, 1, 1) +
                       Vabs(ctm.getR() * ctm.getR().Dot(un->GetWarpVelocity().Scale(100 * SIMULATION_ATOM / un->rSize()))));
        k.r[0] *= v.i;
        k.r[1] *= v.j;
        k.r[2] *= v.k;
                             ...
                             bool                  smallasteroid)
{
    ...
    if (tmpCol && (tmpCol->Collide(*bigger->colTrees->colTree(bigger, smaller->GetWarpVelocity()), &smalltransform, &bigtransform))) {
    ...
    if (bigger->SubUnits.empty() == false && (bigger->graphicOptions.RecurseIntoSubUnitsOnCollision == true || bigtype == ASTEROIDPTR)) {
        ...
        for (std::shared_ptr<Unit> un; (un = *i); ++i) {
            ...
            if ((un->Position() - smaller->Position()).Magnitude() <= subrad + rad) {
                if ((un->InsideCollideTree(
                        smaller, bigpos, bigNormal, smallpos, smallNormal, bigtype == ASTEROIDPTR, smalltype == ASTEROIDPTR)))
                    return true;
            }
        }
    }
    if (smaller->SubUnits.empty() == false &&
        ...
        for (std::shared_ptr<Unit> un; (un = *i); ++i) {
            ...
            if ((un->Position() - bigger->Position()).Magnitude() <= subrad + rad) {
                if ((bigger->InsideCollideTree(
                        un, bigpos, bigNormal, smallpos, smallNormal, bigtype == ASTEROIDPTR, smalltype == ASTEROIDPTR)))
                    return true;
            }
        }
    }
...
bool Unit::Collide(std::shared_ptr<Unit> target)
{
    ...
    if (target == this ||
        ((targetisUnit != NEBULAPTR && thisisUnit != NEBULAPTR) &&
         (owner == target || target->owner == this || (owner != NULL && target->owner == owner))) ||
    ...
    if (distance > 0.0f || (this->colTrees && this->colTrees->colTree(this, this->GetWarpVelocity()) && !sphere_test)) {
        ...
        if (this->colTrees) {
            ...
            if (tmpCol->rayCollide(boltbeam, norm, distance)) {
                ...
                // NOTE:   Here is where we need to retrieve the point on the ray that we collided with the mesh, and set it to end, create
                // the normal and set distance
                VSFileSystem::vs_dbg(3) << boost::format("Beam collide with %1$p, distance %2%") % this % distance << std::endl;
                return (this);
            }
