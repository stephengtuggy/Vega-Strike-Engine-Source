...
const string BASIC_REPAIR_DESC =
    "Hire starship mechanics to examine and assess any wear and tear on your craft. They will replace any damaged components on your "
    "vessel with the standard components of the vessel you initially purchased.  Further upgrades above and beyond the original will not "
    "be replaced free of charge.  The total assessment and repair cost applies if any components are damaged or need servicing (fuel, wear "
    "and tear on jump drive, etc...). If such components are damaged you may save money by repairing them on your own. Your vessel will "
    "also be refuelled.";
...
void BaseComputer::constructControls(void)
{
    ...
    }
    {
        ...
        if (Network) {
            ...
            } else if (serverip.substr(0, 3) == "10." || serverip.substr(0, 8) == "192.168." || serverip.substr(0, 8) == "169.254." ||
                       serverip.find('.') == std::string::npos) {
    ...
    case CARGO: {
        ...
        if (playerUnit) {
            ...
            const float volumeLeft =
                emptyVolume - (m_currentDisplay == CARGO ? playerUnit->getCargoVolume() : playerUnit->getUpgradeVolume());
            ...
            if (showStardate) {
                ...
                sprintf(playerTitle,
                        "Credits: %.2f      Space left: %.6g of %.6g cubic meters   Mass: %.0f%% (base)",
                        playerCredits,
                        volumeLeft,
                        emptyVolume,
                        massEffect);
    ...
    if (tlist->transaction != ACCEPT_MISSION) {
        ...
        case SELL_CARGO:
            ...
            if (item.mission)
                ...
                sprintf(tempString, "Value: #b#%.2f#-b, purchased for %.2f#n#", baseUnit->PriceCargo(item.content), item.price);
            descString += tempString;
            sprintf(tempString, "Cargo volume: %.2f cubic meters;  Mass: %.2f metric tons#n1.5#", item.volume, item.mass);
            descString += tempString;
            if (!item.mission)
        ...
        case SELL_UPGRADE:
            ...
            //********************************************************************************************
            {
                double percent_working =
                    m_player.GetUnit() ? UnitUtil::PercentOperational(m_player.GetUnit(), item.content, item.category, false) : 0.0;
                if (percent_working < 1) {
                    ...
                    sprintf(tempString,
                            ...
                            usedValue(baseUnit->PriceCargo(item.content)),
                            item.price);
                    descString += tempString;
                }
                if (damaged_mode)
                    descString +=
                        "#c1:0:0#Warning: #b#Because pieces of your ship are damaged, you will not be able to sell this item until you fix "
                        "those damaged items in this column in order to allow the mechanics to remove this item.#-c#-b#n1.5#";
                //********************************************************************************************
                if (item.GetDescription() == "" || item.GetDescription()[0] != '#')
                    item.description = buildUpgradeDescription(item);
                break;
...
SimplePickerCell *BaseComputer::createCategoryCell(SimplePickerCells &cells, const string &origCategory, bool skipFirstCategory)
{
...
void BaseComputer::loadListPicker(TransactionList &tlist, SimplePicker &picker, TransactionType transType, bool skipFirstCategory)
{
...
bool BaseComputer::sellSelectedCargo(int requestedQuantity)
{
    ...
    if (item) {
        ...
        if (item->mission) {
            vector<Cargo>::iterator mycargo = std::find(playerUnit->pImage->cargo.begin(), playerUnit->pImage->cargo.end(), *item);
...
#if (defined(__FREEBSD__)) || (defined(_WIN32) && !defined(__CYGWIN__)) || (defined(__GLIBC_MINOR__) && __GLIBC_MINOR__ >= 10) ||          \
    defined(__HAIKU__)
...
bool BaseComputer::BuyUpgradeOperation::checkTransaction(void)
{
    ...
    if (playerUnit->canUpgrade(m_newPart, m_selectedMount, m_selectedTurret, m_addMultMode, false, percent, m_theTemplate)) {
        ...
        showYesNoQuestion("The item cannot fit the frame of your starship.  Do you want to buy it anyway?", this, CONFIRM_ID);
        return true;
    }
}
...
void BaseComputer::SellUpgradeOperation::selectMount(void)
{
    ...
    for (int i = 0; i < playerUnit->GetNumMounts(); i++) {
        ...
        if (playerUnit->mounts[i].status == Mount::ACTIVE || playerUnit->mounts[i].status == Mount::INACTIVE) {
            ...
            const std::shared_ptr<Unit> partUnit =
...
Cargo CreateCargoForOwnerStarship(const std::shared_ptr<Cockpit> cockpit, const std::shared_ptr<Unit> base, int i)
{
    ...
    static const float shipping_price_base  = XMLSupport::parse_float(vs_config->getVariable("physics", "shipping_price_base", "0"));
    static const float shipping_price_insys = XMLSupport::parse_float(vs_config->getVariable("physics", "shipping_price_insys", "1000"));
    ...
    if (needsJumpTransport) {
        ...
        VSFileSystem::vs_dbg(3) << boost::format("Player ship needs transport from %1% to %2% across %3% systems") % locationBaseName %
                                       destinationSystemName % jumps.size()
                                << std::endl;
        cargo.price += shipping_price_perjump * (jumps.size() - 1);
    } else if (needsInsysTransport) {
        VSFileSystem::vs_dbg(3) << boost::format("Player ship needs insys transport from %1% to %2%") % locationBaseName %
                                       destinationBaseName
                                << std::endl;
...
void trackPrice(int                     whichplayer,
                ...
                /*out*/ vector<string> &highest,
                /*out*/ vector<string> &lowest)
{
    static size_t toprank = (size_t)XMLSupport::parse_int(vs_config->getVariable("general", "trade_interface_tracks_prices_toprank", "10"));

    VSFileSystem::vs_dbg(1) << boost::format("Ranking item %1%/%2% at %3%/%4%") % item.category.get() % item.content.get() % systemName %
                                   baseName
                            << std::endl;
    ...
    // First record the given item's price and update the ranking lists
    {
...

        {
            bool resort = false;
            {
                ...
                vector<string>::const_iterator prev = std::find(recordedHighestLocs.begin(), recordedHighestLocs.end(), locname);
...

        {
            bool resort = false;
            {
                ...
                vector<string>::const_iterator prev = std::find(recordedLowestLocs.begin(), recordedLowestLocs.end(), locname);
    ...
    // Now build the top-ranking descriptions
    {
        ...
        VSFileSystem::vs_dbg(1) << boost::format("  highest locs: (%1%)") % recordedHighestLocs.size() << std::endl;
        {
            for (size_t i = 0; i < recordedHighestLocs.size(); ++i) {
                VSFileSystem::vs_dbg(1) << boost::format("    %1% : %2%") % i % recordedHighestLocs[i] << std::endl;
            }
        }

        VSFileSystem::vs_dbg(1) << boost::format("  highest prices: (%1%)") % recordedHighestPrices.size() << std::endl;
        {
            for (size_t i = 0; i < recordedHighestPrices.size(); ++i) {
                VSFileSystem::vs_dbg(1) << boost::format("    %1% : %2$.2f") % i % recordedHighestPrices[i] << std::endl;
            }
        }

        VSFileSystem::vs_dbg(1) << boost::format("  lowest locs: (%1%)") % recordedLowestLocs.size() << std::endl;
        {
            for (size_t i = 0; i < recordedLowestLocs.size(); ++i) {
                VSFileSystem::vs_dbg(1) << boost::format("    %1% : %2%") % i % recordedLowestLocs[i] << std::endl;
            }
        }

        VSFileSystem::vs_dbg(1) << boost::format("  lowest prices: (%1%)") % recordedLowestPrices.size() << std::endl;
        {
            for (size_t i = 0; i < recordedLowestPrices.size(); ++i) {
                VSFileSystem::vs_dbg(1) << boost::format("    %1% : %2$.2f") % i % recordedLowestPrices[i] << std::endl;
            }
        }
...
string buildCargoDescription(const Cargo &item, BaseComputer &computer, float price)
{
    static bool trackBestPrices = XMLSupport::parse_bool(vs_config->getVariable("general", "trade_interface_tracks_prices", "true"));
...
bool buyShip(std::shared_ptr<Unit> baseUnit,
             ...
             BaseComputer *        bcomputer)
{
    ...
    if (shipCargo) {
        if (shipCargo->price < _Universe->AccessCockpit()->credits) {
            ...
            std::shared_ptr<Unit> newPart =
                UnitFactory::createUnit(content.c_str(), false, baseUnit->faction, newModifications, flightGroup, fgsNumber);
            ...
            if (newPart->name != LOAD_FAILED) {
                if (newPart->nummesh() > 0) {
                    ...
                    static bool persistent_missions_across_ship_switch =
                        XMLSupport::parse_bool(vs_config->getVariable("general", "persistent_mission_across_ship_switch", "true"));
...
bool BaseComputer::buyShip(const EventCommandId &command, Control *control)
{
    ...
    if (!(playerUnit && baseUnit && item))
        return true;
    ::buyShip(
        baseUnit, playerUnit, item->GetContent(), item->GetCategory().find("My_Fleet") != string::npos, true, this); // last was false BUCO
    return true;
}
...
void showUnitStats(std::shared_ptr<Unit> playerUnit, string &text, int subunitlevel, int mode, Cargo &item)
{
    ...
    if (!mode) {
        ...
        case 1: // Additive
            PRETTY_ADDU(
                statcolor + "Adds #-c", playerUnit->FuelData(), 2, "metric tons of Lithium-6 " /*+statcolor+"to Fuel Capacity #-c"*/);
            break;
        case 2: // multiplicative
            break;
        default: // Failure
        ...
        if (!mode && (playerUnit->GetMass() != 0)) {
            ...
            if (playerUnit->limits.lateral == playerUnit->limits.vertical) {
                PRETTY_ADDU(statcolor + "Orthogonal acceleration: #-c",
                            playerUnit->limits.vertical / (9.8 * playerUnit->GetMass()),
                            2,
                            "gravities");
            ...
            PRETTY_ADDU(statcolor + "Forward acceleration with overthrust: #-c",
                        playerUnit->limits.afterburn / (9.8 * playerUnit->GetMass()),
                        2,
                        "gravities");
            ...
            case 0: // Replacement or new Module
                ...
                if (playerUnit->limits.vertical != blankUnit->limits.vertical) {
                    PRETTY_ADDU(
                        statcolor + "Provides vertical thrust rated at: #-c", playerUnit->limits.vertical / 1000.0, 2, "MegaNewtons");
                }
            ...
            case 1: // Additive
                if (playerUnit->limits.forward != blankUnit->limits.forward) {
                    PRETTY_ADDU(
                        statcolor + "Increases forward thrust rating by: #-c", playerUnit->limits.forward / 1000.0, 2, "MegaNewtons");
                }
                if (playerUnit->limits.retro != blankUnit->limits.retro) {
                ...
                if (playerUnit->limits.vertical != blankUnit->limits.vertical) {
                    PRETTY_ADDU(
                        statcolor + "Increases vertical thrust rating by: #-c", playerUnit->limits.vertical / 1000.0, 2, "MegaNewtons");
                }
                if (playerUnit->limits.lateral != blankUnit->limits.lateral) {
                    PRETTY_ADDU(
                        statcolor + "Increases lateral thrust rating by: #-c", playerUnit->limits.lateral / 1000.0, 2, "MegaNewtons");
                }
                if (playerUnit->limits.afterburn != blankUnit->limits.afterburn) {
                    PRETTY_ADDU(
                        statcolor + "Increases overdrive thrust rating by: #-c", playerUnit->limits.afterburn / 1000.0, 2, "MegaNewtons");
                }
                break;
            case 2: // multiplicative
                if (playerUnit->limits.forward != blankUnit->limits.forward) {
        ...
        if (!mode) {
            ...
            case 1: // Additive
                if (uc.max_speed() != buc.max_speed()) {
                    ...
                    PRETTY_ADDU(statcolor + "Increases max non-combat speed governor setting by: #-c",
                                uc.max_speed() * non_combat_mode_mult,
                                0,
                                "m/s");
                }
                if (uc.max_ab_speed() != buc.max_ab_speed())
            ...
            case 2: // multiplicative
                if (uc.max_speed() != buc.max_speed()) {
                    ...
                    PRETTY_ADDU(
                        statcolor + "Increases max non-combat speed governor settings by: #-c", 100.0 * (uc.max_speed() - 1), 0, "%");
                }
                if (uc.max_ab_speed() != buc.max_ab_speed())
                    PRETTY_ADDU(statcolor + "Increases max overdrive combat speed governor settings by: #-c",
                                (uc.max_ab_speed() - 1) * 100,
                                0,
                                "%");
                break;
            default: // Failure
    ...
    } else if (uc.max_yaw_right != buc.max_yaw_right || uc.max_pitch_up != buc.max_pitch_up || uc.max_roll_right != buc.max_roll_right) {
        switch (replacement_mode) {
        case 0: // Replacement or new Module
    ...
    if (!mode) {
        ...
        if (!subunitlevel) {
            ...
            if (uj.drive == -2) {
                ...
                if (playerUnit->WarpCapData() < uj.energy) {
                    text += "#n##c1:.3:.3#" + prefix + "WARNING: Warp capacitor banks under capacity for jump: upgrade warp capacitance#-c";
                }
            }
        }
    } else {
        ...
        case 0: // Replacement or new Module
            if (playerUnit->EnergyRechargeData() != blankUnit->EnergyRechargeData())
                PRETTY_ADDU(
                    statcolor + "Installs reactor with recharge rate: #-c", playerUnit->EnergyRechargeData() * RSconverter, 0, "MJ/s");
        ...
        case 1: // Additive
            ...
            if (playerUnit->MaxEnergyData() != blankUnit->MaxEnergyData())
                PRETTY_ADDU(
                    statcolor + "Adds #-c", (playerUnit->MaxEnergyData() * RSconverter), 0, "MJ of storage to main capacitor banks");
            if (playerUnit->GetWarpEnergy() != blankUnit->GetWarpEnergy())
                PRETTY_ADDU(
                    statcolor + "Adds #-c", playerUnit->GetWarpEnergy() * RSconverter * Wconv, 0, "MJ of storage to warp capacitor bank");
            break;
        case 2: // multiplicative
            if (playerUnit->EnergyRechargeData() != blankUnit->EnergyRechargeData())
    ...
    if (!mode) {
        ...
        case 0: // Replacement or new Module
            PRETTY_ADDU(statcolor + "New Sustained Hull Damage Rating: #-c",
                        playerUnit->GetHull() / (playerUnit->GetHullPercent()) * VSDM,
                        0,
                        "MJ");
            break;
        case 1: // Additive
        ...
        if (anyweapons) {
            for (int i = 0; i < playerUnit->GetNumMounts(); i++) {
                ...
                if ((!wi) || (wi->weapon_name == "")) {
                    ...
                    text += wi->weapon_name + ": #c0:1:1#" + lookupMountSize(wi->size) + "#-c#c.9:.9:.5#" + WeaponTypeStrings[wi->type] +
                            " #-c";
                    if (wi->Damage < 0) {
                    ...
                    if ((mode != 0) && (wi->type != weapon_info::PROJECTILE) && (wi->Refire() > 0) &&
                        ...
                        float shot_cycle_mul =
                            wi->type == weapon_info::BEAM ? wi->Stability / (wi->Refire() + wi->Stability) : 1 / wi->Refire();
                        if (wi->Damage != 0)
                            PRETTY_ADDU(statcolor + "   Damage: #-c", wi->Damage * VSDM * shot_cycle_mul, 2, "MJ/s");
                        if (wi->PhaseDamage != 0)
                            PRETTY_ADDU(statcolor + "   Phase damage: #-c", wi->PhaseDamage * VSDM * shot_cycle_mul, 2, "MJ/s");
                        if (wi->EnergyRate != 0)
                            PRETTY_ADDU(statcolor + "   Energy usage: #-c", wi->EnergyRate * RSconverter * shot_cycle_mul, 2, "MJ/s");
                    }
                    text += "#n#";
                }
            }
    ...
    if (subunitlevel == 0 && mode == 0) {
        ...
        PRETTY_ADDU(statcolor + "Minimum time to reach full overthrust speed: #-c",
                    playerUnit->GetMass() * uc.max_ab_speed() / playerUnit->limits.afterburn,
                    2,
                    "seconds");
        // reactor
        float avail    = (playerUnit->MaxEnergyData() * RSconverter - maxshield * VSDM);
        float overhead = (shields_require_power)
                             ? (playerUnit->shield.recharge / shieldenergycap * shield_maintenance_cost * playerUnit->shield.number * VSDM)
                             : 0;
            ...
            if (shields_require_power) {
                ...
                if (maint_draw_percent > 60) {
                    text += "#n##c1:1:.1#" + prefix +
                            "WARNING: Reactor power is heavily consumed by passive shield maintenance: consider downgrading shield or "
                            "upgrading reactor.#-c";
                } else if (maint_draw_percent > 95) {
                    text += "#n##c1:.3:.3#" + prefix +
                            "SEVERE WARNING: Reactor power is overdrawn! Unsustainable power is being consumed by passive shield "
                            "maintenance: downgrade shield or upgrade reactor immediately!#-c";
                }
            }
        }
        ...
        float maint_draw =
            (shields_require_power && playerUnit->shield.number)
                ? (playerUnit->shield.recharge * VSDM / shieldenergycap * shield_maintenance_cost * playerUnit->shield.number)
                : 0;
