...
unsigned int pushSaveData(int whichcp, const string &key, float val)
{
    ...
    vector<float> *ans = &((_Universe->AccessCockpit(whichcp)->savegame->getMissionData(key)));
    if (SERVER)
        VSServer->sendSaveData(whichcp, Subcmd::FloatValue | Subcmd::SetValue, ans->size(), &key, NULL, NULL, &val);
    ans->push_back(val);
    return ans->size() - 1;
}
...
unsigned int eraseSaveData(int whichcp, const string &key, unsigned int index)
{
    ...
    if (index < ans->size()) {
        if (SERVER)
            VSServer->sendSaveData(whichcp, Subcmd::FloatValue | Subcmd::EraseValue, index, &key, NULL, NULL, NULL);
...
unsigned int clearSaveData(int whichcp, const string &key)
{
    ...
    if (!ret)
        return 0;
    if (SERVER)
        VSServer->sendSaveData(whichcp, Subcmd::FloatValue | Subcmd::EraseValue, -1, &key, NULL, NULL, NULL);
    ans->clear();
    return ret;
}
...
unsigned int pushSaveString(int whichcp, const string &key, const string &value)
{
    ...
    vector<std::string> *ans = &((_Universe->AccessCockpit(whichcp)->savegame->getMissionStringData(key)));
    if (SERVER)
        VSServer->sendSaveData(whichcp, Subcmd::StringValue | Subcmd::SetValue, ans->size(), &key, NULL, &value, NULL);
...
void putSaveString(int whichcp, const string &key, unsigned int num, const string &val)
{
    ...
    if (num < ans->size()) {
        if (SERVER)
            VSServer->sendSaveData(whichcp, Subcmd::StringValue | Subcmd::SetValue, num, &key, NULL, &val, NULL);
        (*ans)[num] = val;
    }
}
...
void putSaveData(int whichcp, const string &key, unsigned int num, float val)
{
    ...
    if (num < ans->size()) {
        if (SERVER)
            VSServer->sendSaveData(whichcp, Subcmd::FloatValue | Subcmd::SetValue, num, &key, NULL, NULL, &val);
        (*ans)[num] = val;
    }
}
...
unsigned int eraseSaveString(int whichcp, const string &key, unsigned int index)
{
    ...
    if (index < ans->size()) {
        if (SERVER)
            VSServer->sendSaveData(whichcp, Subcmd::StringValue | Subcmd::EraseValue, index, &key, NULL, NULL, NULL);
        ans->erase(ans->begin() + index);
    }
    return ans->size();
}
...
unsigned int clearSaveString(int whichcp, const string &key)
{
    ...
    if (!ret)
        return 0;
    if (SERVER)
        VSServer->sendSaveData(whichcp, Subcmd::StringValue | Subcmd::EraseValue, -1, &key, NULL, NULL, NULL);
    ans->clear();
    return ret;
}
...
void saveStringList(int playernum, const string &mykey, const vector<string> &names)
{
    ...
    for (vector<string>::const_iterator i = names.begin(); i != names.end(); ++i) {
        if (SERVER)
            VSServer->sendSaveData(playernum, Subcmd::StringValue | Subcmd::SetValue, ans.size(), &mykey, NULL, &*i, NULL);
        ans.push_back(*i);
    }
}
...
void saveDataList(int whichcp, const string &key, const vector<float> &values)
{
    ...
    for (vector<float>::const_iterator i = values.begin(); i != values.end(); ++i) {
        if (SERVER)
            VSServer->sendSaveData(whichcp, Subcmd::FloatValue | Subcmd::SetValue, ans.size(), &key, NULL, NULL, &*i);
        ans.push_back(*i);
    }
}
