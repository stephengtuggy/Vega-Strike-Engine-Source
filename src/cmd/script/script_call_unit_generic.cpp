...
extern bool                  PlanetHasLights(std::shared_ptr<Unit> un);

#if 0
NEVER NEVER NEVER use std::shared_ptr<Unit> to save a unit across frames
extern std::shared_ptr<Unit> player_unit;
BAD BAD BAD
...
static std::shared_ptr<Unit> getIthUnit(un_iter uiter, int i);
...
varInst *Mission::call_unit(missionNode *node, int mode)
{
    ...
    if (method_id == CMT_UNIT_getUnit) {
        ...
        QVector pos(getFloatArg(node, mode, 6), getFloatArg(node, mode, 7), getFloatArg(node, mode, 8));
        ...
        if (mode == SCRIPT_RUN) {
            ...
            cf.fg = Flightgroup::newFlightgroup(
                name_string, type_string, faction_string, ai_string, nr_of_ships, nr_of_waves, logo_tex, logo_alp, this);
            ...
            fflush(stderr);
#endif
            std::shared_ptr<Unit> tmp = call_unit_launch(&cf, clstyp, destinations);
        ...
        if (method_id == CMT_UNIT_getContainer) {
            ...
            std::shared_ptr<Unit> res_unit = NULL;
            if (mode == SCRIPT_RUN)
            ...
            std::shared_ptr<Unit> ret_unit = NULL;
            if (mode == SCRIPT_RUN) {
            ...
            if (mode == SCRIPT_RUN) {
                printf("upgrading %s %s %d %d %s\n",
                       ...
                       mountoffset,
                       subunitoffset,
                       loop_through_mounts ? "true" : "false");
                fflush(stdout);
            ...
            if (mode == SCRIPT_RUN) {
                ...
                fg = Flightgroup::newFlightgroup(
                    arg, type, FactionUtil::GetFaction(my_unit->faction), order, nr_ships, nr_waves_left, "", "", this);
...
std::shared_ptr<Unit> Mission::call_unit_launch(CreateFlightgroup *fg, int type, const string &destinations)
{
    ...
    for (u = 0; u < fg->nr_ships; u++) {
        ...
        if (type == PLANETPTR) {
            ...
            if (bsrc[0] != '\0')
                s = parse_alpha(bsrc);
            my_unit = UnitFactory::createPlanet(QVector(0, 0, 0),
                                                QVector(0, 0, 0),
                                                0,
                                                Vector(0, 0, 0),
                                                0,
            ...
            my_unit = UnitFactory::createUnit(
                fg->fg->type.c_str(), false, faction_nr, string(""), fg->fg, u + fg->fg->nr_ships - fg->nr_ships, NULL, getUniqueSerial());
        }
        units[u] = my_unit;
    }
...
static std::shared_ptr<Unit> getIthUnit(un_iter uiter, int unit_nr)
{
