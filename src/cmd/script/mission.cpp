...
void Mission::terminateMission()
{
    ...
    if (f != Mission_delqueue.end()) {
        VSFileSystem::vs_dbg(1) << boost::format("Not deleting mission twice: %1%") % this->mission_name << std::endl;
    }
    ...
    if (g_game.vsdebug >= 1) {
        ...
        for (vector<Mission *>::iterator i = active_missions->begin(); i != active_missions->end(); ++i) {
            if ((*i)->player_num == player_num) {
                ++misnum;
                VSFileSystem::vs_dbg(1) << boost::format("   Mission #%1%: %2%") % misnum % (*i)->mission_name << std::endl;
            }
        }
    }
    ...
    VSFileSystem::vs_dbg(1) << boost::format("Terminating mission %1% #%2%") % this->mission_name % queuenum << std::endl;
    if (queuenum >= 0) {
        ...
        VSFileSystem::vs_dbg(1) << boost::format("Terminating mission #%1% - got %2% scripts") % queuenum % scripts->size() << std::endl;
        if (num < scripts->size())
        ...
        VSFileSystem::vs_dbg(1) << boost::format("Terminating mission #%1% - got %2% missions") % queuenum % missions->size() << std::endl;
        if (num < missions->size())
            missions->erase(missions->begin() + num);
        VSFileSystem::vs_dbg(1) << boost::format("Terminating mission #%1% - %2% scripts remain") % queuenum % scripts->size() << std::endl;
        VSFileSystem::vs_dbg(1) << boost::format("Terminating mission #%1% - %2% missions remain") % queuenum % missions->size()
                                << std::endl;
    }
