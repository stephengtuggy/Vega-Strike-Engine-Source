...
Nebula *UnitFactory::createNebula(const char *unitfile, bool SubU, int faction, Flightgroup *fg, int fg_snumber, ObjSerial netcreate)
{
    _Universe->netLock(true);
...
{
    _Universe->netLock(true);
    Missile *un = new Missile(filename, faction, modifications, damage, phasedamage, time, radialeffect, radmult, detonation_radius);
    _Universe->netLock(false);
    if (netcreate)
        un->SetSerial(netcreate);
    /*
                                  ...
                                  ObjSerial                         netcreate)
{
     ...
     *                       // Send a packet to clients in order to make them create this unit
     *
     *                       addPlanetBuffer( netbuf, x, y, vely, rotvel, pos, gravity, radius, filename, sr, ds, dest, orbitcent, parent,
     * ourmat, ligh, faction, fullname, inside_out, netcreate); endBuffer( netbuf ); VSServer->broadcast( netbuf, 0,
     * _Universe->activeStarSystem()->GetZone(), CMD_ENTERCLIENT, true);
     *               }
     *               VSServer->invalidateSnapshot();
     */
    return p;
...
Enhancement *
UnitFactory::createEnhancement(const char *filename, int faction, const string &modifications, Flightgroup *flightgrp, int fg_subnumber)
{
...
Building *UnitFactory::createBuilding(
    Terrain *parent, bool vehicle, const char *filename, bool SubUnit, int faction, const std::string &unitModifications, Flightgroup *fg)
{
    return NULL;
}

Asteroid *
UnitFactory::createAsteroid(const char *filename, int faction, Flightgroup *fg, int fg_snumber, float difficulty, ObjSerial netcreate)
{
    _Universe->netLock(true);
