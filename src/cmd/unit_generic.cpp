...
bool Unit::AutoPilotTo(std::shared_ptr<Unit> un, bool automaticenergyrealloc)
{
    std::string tmp;
...
Vector Unit::GetWarpRefVelocity() const
{
    ...
    {
        const std::shared_ptr<Unit> vr = computer.velocity_ref.GetConstUnit();
        if (vr)
...
Vector Unit::GetWarpVelocity() const
{
    Vector VelocityRef(0, 0, 0);
    {
        std::shared_ptr<Unit> vr = const_cast<UnitContainer *>(&computer.velocity_ref)->GetUnit();
        if (vr)
            VelocityRef = vr->cumulative_velocity;
    }
...
bool Unit::InRange(const std::shared_ptr<Unit> target, double &mm, bool cone, bool cap, bool lock) const
{
...
std::shared_ptr<Unit> Unit::Target()
{
    return computer.target.GetUnit();
}

const std::shared_ptr<Unit> Unit::Target() const
{
    return computer.target.GetConstUnit();
}

std::shared_ptr<Unit> Unit::VelocityReference()
{
    return computer.velocity_ref.GetUnit();
}

const std::shared_ptr<Unit> Unit::VelocityReference() const
{
    return computer.velocity_ref.GetConstUnit();
}

std::shared_ptr<Unit> Unit::Threat()
{
...
bool flickerDamage(std::shared_ptr<Unit> un, float hullpercent)
{
...
bool CrashForceDock(std::shared_ptr<Unit> thus, std::shared_ptr<Unit> dockingUn, bool force)
{
    std::shared_ptr<Unit> un            = dockingUn;
...
void Unit::reactToCollision(
    std::shared_ptr<Unit> smalle, const QVector &biglocation, const Vector &bignormal, const QVector &smalllocation, const Vector &smallnormal, float dist)
{
    ...
    if (crash_dock_unit) {
        std::shared_ptr<Unit> dockingun = smalle;
        std::shared_ptr<Unit> thus      = this;
    ...
    if (!jumpReactToCollision(smalle)) {
        ...
        // Damage distribution (NOTE: currently arbitrary - no known good model for calculating how much energy object endures as a result
        // of the collision)
        ...
        if (Network != NULL) {
            ...
            if ((smalle->isUnit() != MISSILEPTR) && isnotplayerorhasbeenmintime) {
                ...
                float maxForce =
                    maxForceMultiplier * (smalle->limits.forward + smalle->limits.retro + smalle->limits.lateral + smalle->limits.vertical);
        ...
        static int upgradefac = XMLSupport::parse_bool(vs_config->getVariable("physics", "cargo_deals_collide_damage", "false"))
                                    ? -1
        ...
        if (_Universe->AccessCamera()) {
            ...
            if (thcp == NULL && smcp == NULL) {
                if (smallmag > collision_hack_distance + this->rSize() && thismag > collision_hack_distance) {
                    static float front_collision_hack_angle = cos(
                        3.1415926536f *
                        XMLSupport::parse_float(vs_config->getVariable("physics", "front_collision_avoidance_hack_angle", "40")) / 180.0f);
        ...
        if (dealdamage) {
            if (faction != upgradefac) {
                smalle->ApplyDamage(
                    biglocation.Cast(), bignormal, small_damage, smalle, GFXColor(1, 1, 1, 2), this->owner != NULL ? this->owner : this);
            }
            if (smalle->faction != upgradefac) {
                this->ApplyDamage(smalllocation.Cast(),
                                  smallnormal,
                                  large_damage,
                                  this,
                                  GFXColor(1, 1, 1, 2),
                                  smalle->owner != NULL ? smalle->owner : smalle);
            }
        }
    }
}
...
static list<std::shared_ptr<Unit> >                Unitdeletequeue;
static Hashtable<long, Unit, 2095> deletedUn;
int                                deathofvs = 1;
void                               CheckUnit(std::shared_ptr<Unit> un)
{
...
void Unit::Init()
{
    ...
    shield.shield2fb.front = shield.shield2fb.back = shield.shield2fb.frontmax = shield.shield2fb.backmax = armor.frontrighttop =
        armor.backrighttop = armor.frontlefttop = armor.backlefttop = armor.frontrightbottom = armor.backrightbottom =
            armor.frontleftbottom = armor.backleftbottom = 0;
    hull                                                 = 1; // 10;
    maxhull                                              = 1; // 10;
    shield.number                                        = 0;
...
bool CheckAccessory(std::shared_ptr<Unit> tur)
{
    bool accessory = tur->name.get().find("accessory") != string::npos;
    if (accessory) {
        tur->SetAngularVelocity(tur->DownCoordinateLevel(
            Vector(tur->GetComputerData().max_pitch_up, tur->GetComputerData().max_yaw_right, tur->GetComputerData().max_roll_right)));
    }
    return accessory;
}
...
{
    static bool client_side_fire = XMLSupport::parse_bool(vs_config->getVariable("network", "client_side_fire", "true"));
...
float CloseEnoughCone(std::shared_ptr<Unit> me)
{
...
bool CloseEnoughToAutotrack(std::shared_ptr<Unit> me, std::shared_ptr<Unit> targ, float &cone)
{
    if (targ) {
...
float Unit::cosAngleTo(std::shared_ptr<Unit> targ, float &dist, float speed, float range, bool turnmargin) const
{
    ...
    float turnangle =
        SIMULATION_ATOM * tmpmax(turnlimit,
...
float Unit::cosAngleFromMountTo(std::shared_ptr<Unit> targ, float &dist) const
{
    float retval = -1;
    dist         = FLT_MAX;
    float  tmpcos;
...
void Unit::Threaten(std::shared_ptr<Unit> targ, float danger)
{
    if (!targ) {
...
void disableSubUnits(std::shared_ptr<Unit> uhn)
{
    std::shared_ptr<Unit> un;
...
{
    if (flightgroup) {
        std::shared_ptr<Unit> leader = flightgroup->leader.GetUnit();
...
float Unit::getRelation(const std::shared_ptr<Unit> targ) const
{
...
extern signed char ComputeAutoGuarantee(std::shared_ptr<Unit> un);
extern float       getAutoRSize(std::shared_ptr<Unit> orig, std::shared_ptr<Unit> un, bool ignore_friend = false);
                            ...
                            if (whereto.length() == 1 + i->first.length() + j->first.length()) {
                                if (whereto.substr(0, i->first.length()) == i->first && whereto.substr(i->first.length() + 1) == j->first) {
                                    static float SystemWarpTargetBonus =
                                        XMLSupport::parse_float(vs_config->getVariable("physics", "target_distance_to_warp_bonus", "1.33"));
...
void Unit::UpdatePhysics(
    const Transformation &trans, const Matrix &transmat, const Vector &cum_vel, bool lastframe, UnitCollection *uc, std::shared_ptr<Unit> superunit)
{
    ...
    std::shared_ptr<Unit> target             = Unit::Target();
    ...
    if (target && cloaking < 0 /*-1 or -32768*/) {
        ...
        if (!computer.force_velocity_ref && activeStarSystem) {
            std::shared_ptr<Unit> nextVelRef = activeStarSystem->nextSignificantUnit();
            if (nextVelRef) {
    ...
    for (int i = 0; (int)i < GetNumMounts(); ++i) {
        ...
        if (mounts[i].processed == Mount::FIRED) {
            ...
            t1.to_matrix(m1);
            int autotrack = 0;
            if ((0 != (mounts[i].size & weapon_info::AUTOTRACKING)) && ((Network != NULL && !SERVER) || TargetTracked()))
            ...
            for (unsigned int locind = 0; locind < Unit::NUM_COLLIDE_MAPS; ++locind)
                hint[locind] = (!is_null(superunit->location[locind])) ? superunit->location[locind]
                                                                       : _Universe->activeStarSystem()->collidemap[locind]->begin();
            if (!mounts[i].PhysicsAlignedFire(this,
                                              t1,
                                              m1,
                                              cumulative_velocity,
                                              (!isSubUnit() || owner == NULL) ? this : owner,
                                              target,
                                              autotrack,
                                              trackingcone,
                                              hint)) {
    ...
    if (warp_is_interstellar && (curr_physical_state.position.MagnitudeSquared() > howFarToJump() * howFarToJump() && !isSubUnit())) {
        ...
        if (!jumpDirect) {
            _Universe->activeStarSystem()->JumpTo(
                this, NULL, NearestSystem(_Universe->activeStarSystem()->getFileName(), curr_physical_state.position), true, true);
        }
    }
...
void Unit::UpdateSubunitPhysics(
    const Transformation &trans, const Matrix &transmat, const Vector &cum_vel, bool lastframe, UnitCollection *uc, std::shared_ptr<Unit> superunit)
{
    if (!SubUnits.empty()) {
            ...
            if (this->sim_atom_multiplier && su->sim_atom_multiplier) {
                ...
                if (((su->last_processed_sqs < su->cur_sim_queue_slot) && (cur_sim_frame >= su->cur_sim_queue_slot))
                    ...
                    Unit::UpdateSubunitPhysics(
                        su, cumulative_transformation, cumulative_transformation_matrix, cumulative_velocity, lastframe, uc, superunit);
                }
            }
...
void Unit::UpdateSubunitPhysics(std::shared_ptr<Unit> subunit,
...
float CalculateNearestWarpUnit(const std::shared_ptr<Unit> thus, float minmultiplier, std::shared_ptr<Unit> *nearest_unit, bool count_negative_warp_units)
{
    ...
    std::shared_ptr<Unit> planet;
    std::shared_ptr<Unit> testthis = NULL;
    {
    ...
    for (un_fiter iter = _Universe->activeStarSystem()->gravitationalUnits().fastIterator(); (planet = *iter) || testthis; ++iter) {
...
float Unit::GetMaxWarpFieldStrength(float rampmult) const
{
    ...
    std::shared_ptr<Unit> nearest_unit  = NULL;
    ...
    if (graphicOptions.InWarp == 1 || graphicOptions.RampCounter != 0) {
        ...
        if (graphicOptions.RampCounter != 0) {
            ...
            if (graphicOptions.InWarp == 1 && graphicOptions.RampCounter > warprampuptime)
                graphicOptions.RampCounter = warprampuptime;
            rampmult = (graphicOptions.InWarp)
                           ? 1.0 - ((graphicOptions.RampCounter / warprampuptime) * (graphicOptions.RampCounter / warprampuptime))
                           : (graphicOptions.RampCounter / warprampdowntime) * (graphicOptions.RampCounter / warprampdowntime);
        }
...
static QVector RealPosition(const std::shared_ptr<Unit> un)
{
    if (un->isSubUnit())
        return un->Position();
...
static QVector AutoSafeEntrancePoint(const QVector start, float rsize, const std::shared_ptr<Unit> goal)
{
...
std::string GenerateAutoError(std::shared_ptr<Unit> me, std::shared_ptr<Unit> targ)
{
    if (UnitUtil::isAsteroid(targ)) {
        static std::string err =
            XMLSupport::escaped_string(vs_config->getVariable("graphics", "hud", "AsteroidsNearMessage", "#ff0000Asteroids Near#000000"));
        return err;
    }
    if (targ->isPlanet()) {
    ...
    if (targ->getRelation(me) < 0) {
        static std::string err =
            XMLSupport::escaped_string(vs_config->getVariable("graphics", "hud", "EnemyNearMessage", "#ff0000Enemy Near#000000"));
        return err;
    }
    static std::string err =
        XMLSupport::escaped_string(vs_config->getVariable("graphics", "hud", "StarshipNearMessage", "#ff0000Starship Near#000000"));
    return err;
}

bool Unit::AutoPilotToErrorMessage(const std::shared_ptr<Unit> target, bool ignore_energy_requirements, std::string &failuremessage, int recursive_level)
{
    ...
    if (target->isUnit() == PLANETPTR) {
        const std::shared_ptr<Unit> targ = *(target->viewSubUnits());
    ...
    static float atd_no_enemies =
        XMLSupport::parse_float(vs_config->getVariable("physics",
                                                       "auto_pilot_termination_distance_no_enemies",
    ...
    float totpercent = 1;
    if (totallength > 1) {
        float apt = (target->isUnit() == PLANETPTR) ? (autopilot_term_distance + target->rSize() * UniverseUtil::getPlanetRadiusPercent())
            ...
            for (un_iter i = ss->getUnitList().createIterator(); (un = *i) != NULL; ++i) {
                ...
                if ((!(un->isUnit() == PLANETPTR && canflythruplanets)) && un->isUnit() != NEBULAPTR && (!UnitUtil::isSun(un))) {
                    if (un != this && un != target) {
                        ...
                        if ((nedis < trad * autopilot_no_enemies_multiplier || tdis <= trad * autopilot_no_enemies_multiplier) &&
    ...
    if (this != target) {
        if ((end - start).MagnitudeSquared() < rSize() * rSize()) {
            failuremessage =
                XMLSupport::escaped_string(vs_config->getVariable("graphics", "hud", "AlreadyNearMessage", "#ff0000Already Near#000000"));
            return false;
        }
        ...
        if ((cp = _Universe->isPlayerStarship(this)) != NULL) {
            ...
            if (getFlightgroup() != NULL) {
                std::shared_ptr<Unit> other = NULL;
...
bool Unit::jumpReactToCollision(std::shared_ptr<Unit> smalle)
{
    ...
    bool        SPEC_interference = (NULL != _Universe->isPlayerStarship(smalle))
    ...
    if (!GetDestinations().empty()) {
        ...
        if ((!SPEC_interference && (smalle->GetJumpStatus().drive >= 0 && // we have power
            ...
            std::shared_ptr<Unit> jumppoint = this;
            if (SERVER)
    ...
    if (!smalle->GetDestinations().empty()) {
        ...
        if ((!SPEC_interference &&
             (GetJumpStatus().drive >= 0 && (warpenergy >= GetJumpStatus().energy || (ai_jump_cheat && cp == NULL)))) ||
            smalle->pImage->forcejump) {
            warpenergy -= GetJumpStatus().energy;
            DeactivateJumpDrive();
            std::shared_ptr<Unit> jumppoint = smalle;
            if (SERVER) {
...
{
    // torque is a normal
    return torque * (Vector(copysign(limits.pitch, torque.i), copysign(limits.yaw, torque.j), copysign(limits.roll, torque.k)) * torque);
}
...
Vector Unit::ClampVelocity(const Vector &velocity, const bool afterburn)
{
    ...
    float        limit = afterburn ? (abfuelclamp * (computer.max_ab_speed() - computer.max_speed()) + (fuelclamp * computer.max_speed()))
...
Vector Unit::MaxThrust(const Vector &amt1)
{
    // amt1 is a normal
    return amt1 *
           (Vector(copysign(limits.lateral, amt1.i), copysign(limits.vertical, amt1.j), amt1.k > 0 ? limits.forward : -limits.retro) *
            amt1);
}
...
Vector Unit::ClampThrust(const Vector &amt1, bool afterburn)
{
    ...
    float ablimit = afterburn ? ((limits.afterburn - limits.forward) * abfuelclamp + limits.forward * fuelclamp) : limits.forward;
    if (amt1.k > ablimit)
        Res.k = ablimit;
    ...
    if (afterburntype == 2) {
        ...
        warpenergy -=
            afterburnenergy * GetFuelUsage(afterburn) * SIMULATION_ATOM * Res.Magnitude() * FMEC_exit_vel_inverse / Lithium6constant;
    }
    if (3 == afterburntype || afterburntype == 1) {
        // fuel-burning overdrive - uses afterburner efficiency. In NO_AFTERBURNER case, "afterburn" will always be false, so can reuse
        // code. HACK this forces the reaction to be Li-6+Li-6 fusion with efficiency governed by the getFuelUsage function
        fuel -= ((afterburn && finegrainedFuelEfficiency) ? afterburnenergy : GetFuelUsage(afterburn)) * SIMULATION_ATOM * Res.Magnitude() *
...
float WARPENERGYMULTIPLIER(std::shared_ptr<Unit> un)
{
...
float currentTotalShieldVal(const Shield &shield)
{
    ...
    case 4:
        ...
        break;
    case 8:
        maxshield = shield.shield8.frontrighttop + shield.shield8.backrighttop + shield.shield8.frontlefttop + shield.shield8.backlefttop +
                    shield.shield8.frontrightbottom + shield.shield8.backrightbottom + shield.shield8.frontleftbottom +
...
float Unit::MaxShieldVal() const
{
    ...
    case 4:
        maxshield =
            .25 * (shield.shield4fbrl.frontmax + shield.shield4fbrl.backmax + shield.shield4fbrl.leftmax + shield.shield4fbrl.rightmax);
        break;
    case 8:
        maxshield = .125 * (shield.shield8.frontrighttopmax + shield.shield8.backrighttopmax + shield.shield8.frontlefttopmax +
                            shield.shield8.backlefttopmax + shield.shield8.frontrightbottommax + shield.shield8.backrightbottommax +
                            shield.shield8.frontleftbottommax + shield.shield8.backleftbottommax);
        break;
    }
...
void Unit::RegenShields()
{
    ...
    if (shield.number) {
        ...
        if (shields_in_spec || !graphicOptions.InWarp) {
            energy -= shield.recharge * VSD / (100 * (shield.efficiency ? shield.efficiency : 1)) / shieldenergycap * shield.number *
                      shield_maintenance_cost * SIMULATION_ATOM * ((apply_difficulty_shields) ? g_game.difficulty : 1);
            if (energy < 0) {
                velocity_discharge = true;
                energy             = 0;
            }
        }
        rec = (velocity_discharge) ? 0
        ...
        if (_Universe->isPlayerStarship(this)) {
            if (rand() < .00005 * RAND_MAX) {
                UniverseUtil::IOmessage(
                    0, "	game", "all", "**Warning** Power Supply Overdrawn: downgrade shield or purchase reactor capacitance!");
            }
        }
    }
...
Vector Unit::ResolveForces(const Transformation &trans, const Matrix &transmat)
{
    ...
    if (air_res_coef || lateral_air_res_coef) {
        ...
        Vector AirResistance =
            Velocity * (air_res_coef * velmag / GetMass()) * (corner_max.i - corner_min.i) * (corner_max.j - corner_min.j);
        if (AirResistance.Magnitude() > velmag) {
            ...
            if (lateral_air_res_coef) {
                ...
                AirResistance     = lateralVel * (lateral_air_res_coef * velmag / GetMass()) * (corner_max.i - corner_min.i) *
    ...
    const Vector &pnt, const Vector &normal, float amt, std::shared_ptr<Unit> affectedUnit, const GFXColor &color, float phasedamage)
{
    ...
    if (SERVER && (ppercentage > 0 || spercentage > 0)) {
      ...
      // If server side, we send the unit serial + serialized shields + shield recharge + shield leak + ...
        ...
        VSServer->sendDamages(
            this->serial, this->getStarSystem()->GetZone(), hull, shield, armor, ppercentage, spercentage, amt, netpnt, netnormal, col);
...
std::shared_ptr<Unit> findUnitInStarsystem(const void *unitDoNotDereference)
{
    std::shared_ptr<Unit> un;
    for (un_iter i = _Universe->activeStarSystem()->getUnitList().createIterator(); (un = *i) != NULL; ++i)
        if (un == reinterpret_cast<const std::shared_ptr<Unit> >(unitDoNotDereference))
            return un;
    return NULL;
}
...
void Unit::ApplyDamage(const Vector &  pnt,
                       const Vector &  normal,
                       float           amt,
                       std::shared_ptr<Unit> affectedUnit,
                       ...
                       float           phasedamage)
{
    ...
    if (!Network && cp) {
        ...
        Threaten(reinterpret_cast<std::shared_ptr<Unit> >(ownerDoNotDereference), 10);
    } else if (!Network) {
    ...
    if (hull < 0) {
        ...
        if (!mykilled) {
            if (cp) {
                ScoreKill(cp, reinterpret_cast<std::shared_ptr<Unit> >(ownerDoNotDereference), this);
            } else {
        ...
        std::shared_ptr<Unit> computerai = NULL;
        std::shared_ptr<Unit> player     = NULL;
        if (cp == NULL) {
...
void Unit::DamageRandSys(float dam, const Vector &vec, float randnum, float degrees)
{
    ...
    if (degrees >= 35 && degrees < 60) {
        ...
        static float warpenergy_damage_prob =
            fuel_damage_prob - XMLSupport::parse_float(vs_config->getVariable("physics", "warpenergy_damage_prob", "0.05"));
        ...
        static float cargovolume_damage_prob =
            ab_damage_prob - XMLSupport::parse_float(vs_config->getVariable("physics", "cargovolume_damage_prob", ".15"));
        static float upgradevolume_damage_prob =
            cargovolume_damage_prob - XMLSupport::parse_float(vs_config->getVariable("physics", "upgradevolume_damage_prob", ".1"));
        static float cargo_damage_prob =
            upgradevolume_damage_prob - XMLSupport::parse_float(vs_config->getVariable("physics", "cargo_damage_prob", "1"));
    ...
    if (degrees >= 120 && degrees < 150) {
        ...
        if (randnum >= .9) {
            static char max_shield_leak =
                (char)mymax(0.0, mymin(100.0, XMLSupport::parse_float(vs_config->getVariable("physics", "max_shield_leak", "90"))));
            static char min_shield_leak =
                (char)mymax(0.0, mymin(100.0, XMLSupport::parse_float(vs_config->getVariable("physics", "max_shield_leak", "0"))));
...
void Unit::Kill(bool erasefromsave, bool quitting)
{
    ...
    if (docked & (DOCKING_UNITS)) {
        ...
        vector<std::shared_ptr<Unit> > dockedun;
        unsigned int   i;
...
float Unit::FShieldData() const
{
    ...
    case 8: {
        if (shield.shield8.frontrighttopmax != 0 || shield.shield8.frontrightbottommax != 0 || shield.shield8.frontlefttopmax != 0 ||
            shield.shield8.frontleftbottommax != 0) {
...
float Unit::BShieldData() const
{
    ...
    case 8: {
        if (shield.shield8.backrighttopmax != 0 || shield.shield8.backrightbottommax != 0 || shield.shield8.backlefttopmax != 0 ||
            shield.shield8.backleftbottommax != 0) {
...
float Unit::LShieldData() const
{
    ...
    case 8: {
        if (shield.shield8.backlefttopmax != 0 || shield.shield8.backleftbottommax != 0 || shield.shield8.frontlefttopmax != 0 ||
            shield.shield8.frontleftbottommax != 0) {
...
float Unit::RShieldData() const
{
    ...
    case 8: {
        if (shield.shield8.backrighttopmax != 0 || shield.shield8.backrightbottommax != 0 || shield.shield8.frontrighttopmax != 0 ||
            shield.shield8.frontrightbottommax != 0) {
...
#endif
        std::shared_ptr<Unit> mydeleter = Unitdeletequeue.back();
...
std::shared_ptr<Unit> makeBlankUpgrade(string templnam, int faction)
{
    std::shared_ptr<Unit> bl = UnitFactory::createServerSideUnit(templnam.c_str(), true, faction);
...
const std::shared_ptr<Unit> makeFinalBlankUpgrade(string name, int faction)
{
    ...
    free(unitdir);
    const std::shared_ptr<Unit> lim = UnitConstCache::getCachedConst(StringIntKey(limiternam, faction));
    if (!lim)
...
const std::shared_ptr<Unit> makeTemplateUpgrade(string name, int faction)
{
    char * unitdir    = GetUnitDir(name.c_str());
...
const std::shared_ptr<Unit> loadUnitByCache(std::string name, int faction)
{
    const std::shared_ptr<Unit> temprate = UnitConstCache::getCachedConst(StringIntKey(name, faction));
    if (!temprate)
        ...
        if (system_damage_on_armor || did_hull_damage) {
            ...
            if (numCargo() > 0) {
                if (DestroySystem(hull, maxhull, numCargo())) {
                    ...
                    if (GetCargo(which).GetCategory().find("upgrades/") == 0 && GetCargo(which).GetCategory().find(DamagedCategory) != 0 &&
                        ...
                        static bool NotActuallyDowngrade =
                            XMLSupport::parse_bool(vs_config->getVariable("physics", "separate_system_flakiness_component", "false"));
                        if (!NotActuallyDowngrade) {
                            const std::shared_ptr<Unit> downgrade = loadUnitByCache(GetCargo(which).content, FactionUtil::GetFactionIndex("upgrades"));
                            if (downgrade) {
...
bool withinShield(const ShieldFacing &facing, float theta, float rho)
{
    ...
    return rho >= facing.rhomin && rho < facing.rhomax &&
           ((theta >= facing.thetamin && theta < facing.thetamax) || (theta360 >= facing.thetamin && theta360 < facing.thetamax));
}
...
void Unit::TargetTurret(std::shared_ptr<Unit> targ)
{
    if (!SubUnits.empty()) {
        std::shared_ptr<Unit> su;
...
bool Unit::TargetLocked(const std::shared_ptr<Unit> checktarget) const
{
...
bool Unit::TargetTracked(const std::shared_ptr<Unit> checktarget)
{
...
void Unit::Target(std::shared_ptr<Unit> targ)
{
    if (targ == this)
        return;
    ObjSerial oldtarg = 0;
            ...
            if (jump.drive != -1) {
                ...
                for (un_iter i = _Universe->activeStarSystem()->getUnitList().createIterator(); (u = *i) != NULL; ++i)
                    if (!u->GetDestinations().empty()) {
                        if (std::find(u->GetDestinations().begin(), u->GetDestinations().end(), targ->activeStarSystem->getFileName()) !=
                            u->GetDestinations().end()) {
                            Target(u);
                            ActivateJumpDrive(0);
                            found = true;
                        }
...
void Unit::VelocityReference(std::shared_ptr<Unit> targ)
{
    computer.force_velocity_ref = !!targ;
    computer.velocity_ref.SetUnit(targ);
}

void Unit::SetOwner(std::shared_ptr<Unit> target)
{
    owner = target;
}
...
template <bool FORWARD> class WeaponComparator
{
  public:
    ...
    {
        if (a.size() == b.size()) {
            for (WeaponGroup::const_iterator iterA = a.begin(), iterB = b.begin(); iterA != a.end() && iterB != b.end(); ++iterA, ++iterB) {
                if ((*iterA) < (*iterB))
                    return FORWARD;
                else if ((*iterB) < (*iterA))
                    return !FORWARD;
    ...
    static bool checkmount(std::shared_ptr<Unit> un, int i, bool missile)
    {
    ...
    static void ToggleWeaponSet(std::shared_ptr<Unit> un, bool missile)
    {
        if (un->mounts.size() == 0) {
            std::shared_ptr<Unit> tur = NULL;
            ...
            if ((*iter).size() && notSpecial(un->mounts[(*((*iter).begin()))])) {
                ...
                for (iter2 = mypairset.begin(); iter2 != mypairset.end(); ++iter2)
                    if ((*iter2).size() && notSpecial(un->mounts[(*((*iter2).begin()))])) {
                        WeaponGroup myGroup;
                        set_union((*iter).begin(), (*iter).end(), (*iter2).begin(), (*iter2).end(), inserter(myGroup, myGroup.begin()));
                        myset.insert(myGroup);
                    }
            }
...
void Unit::SetRecursiveOwner(std::shared_ptr<Unit> target)
{
    owner = target;
    std::shared_ptr<Unit> su;
    for (un_iter iter = getSubUnits(); (su = *iter); ++iter)
...
void Unit::SetCollisionParent(std::shared_ptr<Unit> name)
{
...
bool Unit::EndRequestClearance(std::shared_ptr<Unit> targ)
{
    std::vector<std::shared_ptr<Unit> >::iterator lookcleared;
...
bool Unit::RequestClearance(std::shared_ptr<Unit> dockingunit)
{
...
extern void SwitchUnits(std::shared_ptr<Unit> , std::shared_ptr<Unit> );
    ...
    for (unsigned int i = 0; i < pImage->dockedunits.size(); ++i) {
        ...
        if (un == _Universe->AccessCockpit()->GetParent()) {
            ...
            for (unsigned int i = 0; i < pImage->clearedunits.size(); ++i)
                // this is a hack because we don't have an interface to say "I want to buy a ship"  this does it if you press shift-c in the
                // base
...
std::set<std::shared_ptr<Unit> > arrested_list_do_not_dereference;
...
void UpdateMasterPartList(std::shared_ptr<Unit> );
int  Unit::ForceDock(std::shared_ptr<Unit> utdw, unsigned int whichdockport)
{
    ...
    std::set<std::shared_ptr<Unit> >::iterator arrested = arrested_list_do_not_dereference.find(this);
...
int Unit::Dock(std::shared_ptr<Unit> utdw)
{
...
int Unit::CanDockWithMe(std::shared_ptr<Unit> un, bool force)
{
            ...
            for (unsigned int j = 0; j < un->pImage->dockingports.size(); ++j) {
                if (insideDock(pImage->dockingports[i],
                               InvTransform(GetTransformation(),
                                            Transform(un->GetTransformation(), un->pImage->dockingports[j].GetPosition().Cast())),
...
bool Unit::IsCleared(const std::shared_ptr<Unit> DockingUnit) const
{
...
bool Unit::isDocked(const std::shared_ptr<Unit> d) const
{
    if (!d)
        return false;
...
bool Unit::UnDock(std::shared_ptr<Unit> utdw)
{
    unsigned int i = 0;
...
static int UpgradeFloat(double &result,
                        ...
                        double (*computepercentage)(double oldvar, double upgrador, double newvar),
                        double &percentage,
                        bool    forcedowngrade,
                        bool    usetemplate,
...
bool Unit::UpgradeMounts(const std::shared_ptr<Unit> up, int mountoffset, bool touchme, bool downgrade, int &numave, const std::shared_ptr<Unit> templ, double &percentage)
{
    int  j;
    int  i;
    ...
    for (i = 0, j = mountoffset; i < up->GetNumMounts() && i < GetNumMounts() /*i should be GetNumMounts(), s'ok*/; ++i, ++j)
        ...
        if (up->mounts[i].status == Mount::ACTIVE || up->mounts[i].status == Mount::INACTIVE) {
            ...
            bool ismissiletype = (0 != (up->mounts[i].type->size &
            ...
            if (!downgrade) {
                ...
                if (up->mounts[i].type->weapon_name.find("_UPGRADE") == string::npos) {
                    ...
                    if ((unsigned int)(up->mounts[i].type->size) == (up->mounts[i].type->size & mounts[jmod].size)) {
                        if (up->mounts[i].type->weapon_name != mounts[jmod].type->weapon_name || mounts[jmod].status == Mount::DESTROYED ||
                            mounts[jmod].status == Mount::UNCHOSEN) {
                    ...
                    for (unsigned int k = 0; k < (unsigned int)GetNumMounts(); ++k) {
                        ...
                        if (strcasecmp(mounts[jkmod].type->weapon_name.c_str(), up->mounts[i].type->weapon_name.c_str()) == 0) {
                            ...
                            if (isammo && mounts[jkmod].ammo <= 0)
                                // whether it's gun ammo or a missile, you can't remove ammo from an infinite source, and you can't remove
                                // ammo if there isn't any
                                continue;
                            else
                                found = true;
...
std::shared_ptr<Unit> CreateGenericTurret(std::string tur, int faction)
{
    return new Unit(tur.c_str(), true, faction, "", 0, 0);
}

bool Unit::UpgradeSubUnits(const std::shared_ptr<Unit> up, int subunitoffset, bool touchme, bool downgrade, int &numave, double &percentage)
{
    return UpgradeSubUnitsWithFactory(up, subunitoffset, touchme, downgrade, numave, percentage, &CreateGenericTurret);
}

bool Unit::UpgradeSubUnitsWithFactory(const std::shared_ptr<Unit> up,
                                      int         subunitoffset,
                                      bool        touchme,
                                      bool        downgrade,
                                      int &       numave,
                                      ...
                                      std::shared_ptr<Unit> (*createupgradesubunit)(std::string s, int faction))
{
    ...
    for (upturrets = up->viewSubUnits(); ((*upturrets) != NULL) && ((*ui) != NULL); ++ui, ++upturrets) {
        hasAnyTurrets = true;
        const std::shared_ptr<Unit> addtome;
        ...
        if (foundthis) {
            if (touchme) {
                ...
                if (!downgrade) {
                    std::shared_ptr<Unit> addToMeNew                = (*createupgradesubunit)(addtome->name, addtome->faction);
                    ...
                    std::shared_ptr<Unit> un; // make garbage unit
...
bool Unit::canUpgrade(const std::shared_ptr<Unit> upgrador,
                      int         mountoffset,
                      int         subunitoffset,
                      int         additive,
                      bool        force,
...
bool Unit::Upgrade(const std::shared_ptr<Unit> upgrador,
                   int         mountoffset,
                   int         subunitoffset,
                   int         additive,
                   bool        force,
...
bool Unit::canDowngrade(
    const std::shared_ptr<Unit> downgradeor, int mountoffset, int subunitoffset, double &percentage, const std::shared_ptr<Unit> downgradelimit, bool gen_downgrade_list)
{
    return UpAndDownGrade(
        downgradeor, NULL, mountoffset, subunitoffset, false, true, false, true, percentage, downgradelimit, false, gen_downgrade_list);
}
...
{
    return UpAndDownGrade(
        downgradeor, NULL, mountoffset, subunitoffset, true, true, false, true, percentage, downgradelimit, false, gen_downgrade_list);
}
...
double       Unit::Upgrade(const std::string &file, int mountoffset, int subunitoffset, bool force, bool loop_through_mounts)
{
    ...
    const std::shared_ptr<Unit> up         = UnitConstCache::getCachedConst(StringIntKey(file, upgradefac));
    if (!up)
        up = UnitConstCache::setCachedConst(StringIntKey(file, upgradefac), UnitFactory::createUnit(file.c_str(), true, upgradefac));
    ...
    const std::shared_ptr<Unit> templ    = UnitConstCache::getCachedConst(StringIntKey(templnam, this->faction));
    if (templ == NULL) {
    ...
    if (up->name != "LOAD_FAILED") {
        for (int i = 0; percentage == 0; ++i) {
            if (!this->Unit::Upgrade(up,
                                     mountoffset + i,
                                     subunitoffset + i,
                                     GetModeFromName(file.c_str()),
                                     force,
                                     percentage,
...
#define STDUPGRADE_SPECIFY_DEFAULTS(my, oth, temp, noth, dgradelimer, dgradelimerdefault, clamp, value_to_lookat)                          \
    do {                                                                                                                                   \
        retval = (UpgradeFloat(resultdoub,                                                                                                 \
                               my,                                                                                                         \
                               oth,                                                                                                        \
                               (templ != NULL) ? temp : 0,                                                                                 \
                               Adder,                                                                                                      \
...
#define STDUPGRADECLAMP(my, oth, temp, noth)                                                                                               \
    do {                                                                                                                                   \
        STDUPGRADE_SPECIFY_DEFAULTS(my, oth, temp, noth, downgradelimit->my, blankship->my, !force_change_on_nothing, this->my);           \
    } while (0)

bool Unit::UpAndDownGrade(const std::shared_ptr<Unit> up,
                          const std::shared_ptr<Unit> templ,
                          int         mountoffset,
                          int         subunitoffset,
                          bool        touchme,
                          ...
                          bool        gen_downgrade_list)
{
    ...
    if (Network && !_Universe->netLocked() && touchme) {
        int playernum = _Universe->whichPlayerStarship(this);
        if (playernum >= 0) {
            ObjSerial buySerial = downgrade ? 0 : serial, sellSerial = downgrade ? serial : 0;
            Network[playernum].cargoRequest(buySerial, sellSerial, up->name, 0, mountoffset, subunitoffset);
        }
        return false;
    }
    if (SERVER && touchme && !_Universe->netLocked() && getStarSystem()) {
        ...
        ObjSerial buySerial = downgrade ? 0 : serial, sellSerial = downgrade ? serial : 0;
        VSServer->BroadcastCargoUpgrade(
            serial, buySerial, sellSerial, up->name, 0, 0, 0, false, 0, mountoffset, subunitoffset, getStarSystem()->GetZone());
    }
    ...
    double       temppercent;
    static std::shared_ptr<Unit> blankship     = NULL;
    ...
    if (!csv_cell_null_check || force_change_on_nothing ||
        cell_has_recursive_data(upgrade_name,
                                up->faction,
                                "Heat_Sink_Rating|Repair_Droid|Hold_Volume|Upgrade_Storage_Volume|Equipment_Space|Hidden_Hold_Volume|ECM_"
                                "Rating|Primary_Capacitor|Warp_Capacitor")) {
    ...
    if (!csv_cell_null_check || force_change_on_nothing ||
        cell_has_recursive_data(
            upgrade_name,
            up->faction,
            "Maneuver_Yaw|Maneuver_Pitch|Maneuver_Roll|Left_Accel|Top_Accel|Retro_Accel|Forward_Accel|Afterburner_Accel|Default_Speed_"
            "Governor|Afterburner_Speed_Governor|Yaw_Governor|Pitch_Governor|Roll_Speed_Governor")) {
    ...
    if (UpgradeCockpitDamage) {
        STDUPGRADE(pImage->fireControlFunctionality,
                   up->pImage->fireControlFunctionality,
                   templ->pImage->fireControlFunctionality,
                   (unittable ? 0 : 1));
        STDUPGRADE(pImage->fireControlFunctionalityMax,
                   up->pImage->fireControlFunctionalityMax,
                   templ->pImage->fireControlFunctionalityMax,
                   (unittable ? 0 : 1));
        STDUPGRADE(
            pImage->SPECDriveFunctionality, up->pImage->SPECDriveFunctionality, templ->pImage->SPECDriveFunctionality, (unittable ? 0 : 1));
        STDUPGRADE(pImage->SPECDriveFunctionalityMax,
                   up->pImage->SPECDriveFunctionalityMax,
                   templ->pImage->SPECDriveFunctionalityMax,
                   (unittable ? 0 : 1));
        STDUPGRADE(pImage->CommFunctionality, up->pImage->CommFunctionality, templ->pImage->CommFunctionality, (unittable ? 0 : 1));
        STDUPGRADE(
            pImage->CommFunctionalityMax, up->pImage->CommFunctionalityMax, templ->pImage->CommFunctionalityMax, (unittable ? 0 : 1));
        STDUPGRADE(pImage->LifeSupportFunctionality,
                   up->pImage->LifeSupportFunctionality,
                   templ->pImage->LifeSupportFunctionality,
                   (unittable ? 0 : 1));
        STDUPGRADE(pImage->LifeSupportFunctionalityMax,
                   up->pImage->LifeSupportFunctionalityMax,
        ...
        for (unsigned int upgr = 0; upgr < upgrmax; upgr++)
            STDUPGRADE(
                pImage->cockpit_damage[upgr], up->pImage->cockpit_damage[upgr], templ->pImage->cockpit_damage[upgr], (unittable ? 0 : 1));
        for (unsigned int upgr = 0; upgr < upgrmax; ++upgr)
        ...
        if (shield.number == up->shield.number) {
            ...
            case 8:
                ...
                STDUPGRADE(shield.shield8.frontrightbottommax,
                           ...
                           0);
                STDUPGRADE(
                    shield.shield8.backrightbottommax, up->shield.shield8.backrightbottommax, templ->shield.shield8.backrightbottommax, 0);
                STDUPGRADE(
                    shield.shield8.frontleftbottommax, up->shield.shield8.frontleftbottommax, templ->shield.shield8.frontleftbottommax, 0);
                STDUPGRADE(
                    shield.shield8.backleftbottommax, up->shield.shield8.backleftbottommax, templ->shield.shield8.backleftbottommax, 0);
    ...
    // Radar stuff
    if (!csv_cell_null_check || force_change_on_nothing ||
        cell_has_recursive_data(upgrade_name, up->faction, "Radar_Range|Radar_Color|ITTS|Can_Lock|Max_Cone|Lock_Cone|Tracking_Cone")) {
        if (!csv_cell_null_check || force_change_on_nothing || cell_has_recursive_data(upgrade_name, up->faction, "Radar_Range")) {
            STDUPGRADECLAMP(
                computer.radar.maxrange, up->computer.radar.maxrange, use_template_maxrange ? templ->computer.radar.maxrange : FLT_MAX, 0);
        }
        ...
        if (!csv_cell_null_check || force_change_on_nothing || cell_has_recursive_data(upgrade_name, up->faction, "ITTS")) {
            computer.itts =
                UpgradeBoolval(computer.itts, up->computer.itts, touchme, downgrade, numave, percentage, force_change_on_nothing);
        }
        if (!csv_cell_null_check || force_change_on_nothing || cell_has_recursive_data(upgrade_name, up->faction, "Can_Lock")) {
            computer.radar.canlock = UpgradeBoolval(
                computer.radar.canlock, up->computer.radar.canlock, touchme, downgrade, numave, percentage, force_change_on_nothing);
        }
    ...
    if (downgrade) {
        ...
        // NOTE: Afterburner type 1 (gas)
        // NOTE: Afterburner type 0 (pwr)
        if (((afterburnenergy > up->afterburnenergy || (afterburntype != up->afterburntype && up->afterburnenergy != 32767)) &&
...
bool Unit::ReduceToTemplate()
{
    ...
    const std::shared_ptr<Unit> temprate = makeFinalBlankUpgrade(name, faction);
    bool        success  = false;
    double      pct      = 0;
...
int Unit::RepairUpgrade()
{
    ...
    if (ComponentBasedUpgrades) {
        ...
        const std::shared_ptr<Unit> maxrecharge = makeTemplateUpgrade(name.get() + ".template", faction);
        ...
        for (unsigned int i = 0; i < mpl->numCargo(); ++i)
            if (mpl->GetCargo(i).GetCategory().find("upgrades") == 0) {
                const std::shared_ptr<Unit> up = loadUnitByCache(mpl->GetCargo(i).content, upfac);
                // now we analyzify up!
...
bool Unit::RepairUpgradeCargo(Cargo *item, std::shared_ptr<Unit> baseUnit, float *credits)
{
    ...
    if (isWeapon(item->category)) {
        const std::shared_ptr<Unit> upgrade = getUnitFromUpgradeName(item->content, this->faction);
        ...
        if (notadditive || item->GetCategory().find(DamagedCategory) == 0) {
            ...
            const std::shared_ptr<Unit> un       = getUnitFromUpgradeName(item->content, this->faction);
            if (un) {
...
vector<CargoColor> &Unit::FilterDowngradeList(vector<CargoColor> &mylist, bool downgrade)
{
    ...
    const std::shared_ptr<Unit> downgradelimit = NULL;
    ...
    for (unsigned int i = 0; i < mylist.size(); ++i) {
        ...
        if (mode != 2 || (!downgrade)) {
            const std::shared_ptr<Unit> NewPart = UnitConstCache::getCachedConst(StringIntKey(mylist[i].cargo.GetContent().c_str(), upgrfac));
            if (!NewPart) {
                NewPart = UnitConstCache::setCachedConst(StringIntKey(mylist[i].cargo.GetContent(), upgrfac),
                                                         UnitFactory::createUnit(mylist[i].cargo.GetContent().c_str(), false, upgrfac));
            }
            if (NewPart->name == string("LOAD_FAILED")) {
                const std::shared_ptr<Unit> NewPart = UnitConstCache::getCachedConst(StringIntKey(mylist[i].cargo.GetContent().c_str(), faction));
                if (!NewPart) {
            ...
            if (NewPart->name != string("LOAD_FAILED")) {
                ...
                if (!downgrade) {
                    ...
                    if (templ == NULL) {
                        templ = UnitConstCache::setCachedConst(StringIntKey(templnam, faction),
                                                               UnitFactory::createUnit(templnam.c_str(), true, this->faction));
                    }
                    ...
                    if (downgradelimit == NULL) {
                        downgradelimit = UnitConstCache::setCachedConst(StringIntKey(limiternam, faction),
...
extern int SelectDockPort(std::shared_ptr<Unit> , std::shared_ptr<Unit> parent);
...
void Unit::EjectCargo(unsigned int index)
{
    ...
    if (tmp) {
        ...
        if (tmp->quantity > 0) {
            ...
            std::shared_ptr<Unit> cargo = NULL;
            ...
            float        arot = 0;
            static float grot =
                XMLSupport::parse_float(vs_config->getVariable("graphics", "generic_cargo_rotation_speed", "1")) * 3.1415926536 / 180;
            if (!cargo) {
                static float crot =
                    XMLSupport::parse_float(vs_config->getVariable("graphics", "cargo_rotation_speed", "60")) * 3.1415926536 / 180;
                static float erot =
                    XMLSupport::parse_float(vs_config->getVariable("graphics", "eject_rotation_speed", "0")) * 3.1415926536 / 180;
                if (tmpcontent == "eject") {
                    if (isplayer) {
                        Flightgroup *fg        = this->getFlightgroup();
                        int          fgsnumber = 0;
                    ...
                    if (isplayer) {
                        ...
                        if (cargotime == 0.0) {
                            ...
                            cargo = UnitFactory::createMissile("eject", fac, "", 0, 0, ejectcargotime, 1, 1, 1, getUniqueSerial());
                        }
                    }
                    arot = erot;
                    cargo->PrimeOrders();
                    ...
                    int upgrfac = FactionUtil::GetUpgradeFaction();
                    cargo       = UnitFactory::createMissile(tmpnam.c_str(), upgrfac, "", 0, 0, cargotime, 1, 1, 1, getUniqueSerial());
                    arot        = rot;
                }
            }
            if (cargo->name == "LOAD_FAILED") {
                cargo->Kill();
                cargo = UnitFactory::createMissile(
                    "generic_cargo", FactionUtil::GetUpgradeFaction(), "", 0, 0, cargotime, 1, 1, 1, getUniqueSerial());
                arot = grot;
            }
...
int Unit::RemoveCargo(unsigned int i, int quantity, bool eraseZero)
{
    ...
    if (SERVER && !_Universe->netLocked() && getStarSystem()) {
        VSServer->BroadcastCargoUpgrade(this->serial,
                                        0,
                                        this->serial,
                                        carg->GetContent(),
                                        carg->price,
...
{
    const std::shared_ptr<Unit> thus = this;
...
std::string Unit::GetManifest(unsigned int i, std::shared_ptr<Unit> scanningUnit, const Vector &oldspd) const
{
...
bool Unit::SellCargo(unsigned int i, int quantity, float &creds, Cargo &carg, std::shared_ptr<Unit> buyer)
{
...
bool Unit::SellCargo(const std::string &s, int quantity, float &creds, Cargo &carg, std::shared_ptr<Unit> buyer)
{
    Cargo tmp;
    tmp.content                     = s;
    vector<Cargo>::iterator mycargo = std::find(pImage->cargo.begin(), pImage->cargo.end(), tmp);
...
bool Unit::BuyCargo(unsigned int i, unsigned int quantity, std::shared_ptr<Unit> seller, float &creds)
{
...
bool Unit::BuyCargo(const std::string &cargo, unsigned int quantity, std::shared_ptr<Unit> seller, float &creds)
{
    unsigned int i;
...
{
    std::shared_ptr<Unit> un      = (std::shared_ptr<Unit> )mythis;
    float       mass    = un->Mass;
    static bool usemass = XMLSupport::parse_bool(vs_config->getVariable("physics", "use_cargo_mass", "true"));
...
std::string Unit::shieldSerializer(const XMLType &input, void *mythis)
{
    ...
    case 8:
        return string("\" frontrighttop=\"") + tostring(un->shield.shield8.frontrighttop) + string("\" backrighttop=\"") +
               tostring(un->shield.shield8.backrighttop) + string("\" frontlefttop=\"") + tostring(un->shield.shield8.frontlefttop) +
               ...
               tostring(un->shield.shield8.frontleftbottom) + string("\" backleftbottom=\"") + tostring(un->shield.shield8.backleftbottom);

    case 4:
    default:
        return tostring(un->shield.shield4fbrl.frontmax) + string("\" back=\"") + tostring(un->shield.shield4fbrl.backmax) +
               string("\" left=\"") + tostring(un->shield.shield4fbrl.leftmax) + string("\" right=\"") +
...
std::string CargoToString(const Cargo &cargo)
{
    ...
    return string("\t\t\t<Cargo mass=\"") + XMLSupport::tostring((float)cargo.mass) + string("\" price=\"") +
           ...
           string("\" quantity=\"") + XMLSupport::tostring((int)cargo.quantity) + string("\" file=\"") + cargo.GetContent() + missioncargo +
           string("\"/>\n");
}
            ...
            if (numcargo > 0) {
                ...
                if (carg->GetCategory().find("upgrades/") == 0 && carg->GetCategory().find(DamagedCategory) != 0 &&
                    ...
                    if (pImage->next_repair_time == -FLT_MAX) {
                        ...
                        const std::shared_ptr<Unit> up    = getUnitFromUpgradeName(carg->content, upfac);
...
std::map<string, std::shared_ptr<Unit> > MeshAnimation::Units;

MeshAnimation::MeshAnimation(std::shared_ptr<Unit> _unitDst)
    : animatedMesh(true),
      activeAnimation(0),
      timeperframe(3.0),
      done(true),
...
bool MeshAnimation::Init(const char *filename, int faction, Flightgroup *flightgrp, const char *animationExt)
{
    ...
    if (meshes->size() != 0) {
        ...
        if (!animationExt)
            animationName = string(count); // if there is no extension given, the animations are called by their load order, 1, 2 ,3
                                           // ....10..
        else
        ...
        cerr << "Animation data loaded for unit: " << string(filename) << ", named " << uniqueUnitName << " - with: " << numFrames
             << " frames." << endl;
        return true;
    } else {
        delete meshes;
...
{
    std::map<string, std::shared_ptr<Unit> >::iterator pos;
