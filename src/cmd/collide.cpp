...
collideTrees::collideTrees(const std::string &hk, csOPCODECollider *cT, csOPCODECollider *cS) : hash_key(hk), colShield(cS)
{
...
csOPCODECollider *collideTrees::colTree(std::shared_ptr<Unit> un, const Vector &othervelocity)
{
    ...
    static unsigned int max_collide_trees =
        static_cast<unsigned int>(XMLSupport::parse_int(vs_config->getVariable("physics", "max_collide_trees", "16384")));
...
{
    return lc.Mini.i < tmp.Maxi.i && lc.Mini.j < tmp.Maxi.j && lc.Mini.k < tmp.Maxi.k && lc.Maxi.i > tmp.Mini.i && lc.Maxi.j > tmp.Mini.j &&
