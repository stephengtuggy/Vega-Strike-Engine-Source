...
{
    if (ABS(other.MinX() - MaxX()) < SMALL_EPSILON || ABS(other.MaxX() - MinX()) < SMALL_EPSILON) {
...
{
    if (ABS(other.MinY() - MaxY()) < SMALL_EPSILON || ABS(other.MaxY() - MinY()) < SMALL_EPSILON) {
...
{
    if (ABS(other.MinZ() - MaxZ()) < SMALL_EPSILON || ABS(other.MaxZ() - MinZ()) < SMALL_EPSILON) {
        if (MaxX() < other.MinX() || MinX() > other.MaxX())
            return false;
        if (MaxY() < other.MinY() || MinY() > other.MaxY())
            return false;
...
void csBox3::GetConvexOutline(const csVector3 &pos, csVector3 *ar, int &num_array, bool bVisible) const
{
    const csVector3 &bmin = Min();
    const csVector3 &bmax = Max();
    int              idx;
...
bool csBox3::Between(const csBox3 &box1, const csBox3 &box2) const
{
    ...
    if (((maxbox.x >= box1.minbox.x && minbox.x <= box2.maxbox.x) || (maxbox.x >= box2.minbox.x && minbox.x <= box1.maxbox.x)) &&
        ((maxbox.y >= box1.minbox.y && minbox.y <= box2.maxbox.y) || (maxbox.y >= box2.minbox.y && minbox.y <= box1.maxbox.y)) &&
        ((maxbox.z >= box1.minbox.z && minbox.z <= box2.maxbox.z) || (maxbox.z >= box2.minbox.z && minbox.z <= box1.maxbox.z))) {
...
{
    return csBox3(MIN(box1.minbox.x, box2.minbox.x),
...
{
    return csBox3(MIN(box.minbox.x, point.x),
...
csBox3 operator*(const csBox3 &box1, const csBox3 &box2)
{
    return csBox3(MAX(box1.minbox.x, box2.minbox.x),
