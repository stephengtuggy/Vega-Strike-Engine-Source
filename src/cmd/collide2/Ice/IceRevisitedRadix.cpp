...
/**
 *	Main sort routine.
 *	This one is for integer values. After the call, mRanks contains a list of indices in sorted order, i.e. in the order you may process
 *your data. \param		input	[in] a list of integer values to sort
 *	\param		nb		[in] number of values to sort, must be < 2^31
 *	\param		hint	[in] RADIX_SIGNED to handle negative values, RADIX_UNSIGNED if you know your input buffer only contains
 *positive values \return		Self-Reference
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (udword j = 0; j < 4; j++) {
        ...
        if (PerformPass) {
            ...
            if (j != 3 || hint == RADIX_UNSIGNED) {
                ...
                //				for(udword i=1;i<128;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];	// 1
                // to 128 for positive numbers
                for (i = 1; i < 128; i++)
...
/**
 *	Main sort routine.
 *	This one is for floating-point values. After the call, mRanks contains a list of indices in sorted order, i.e. in the order you may
 *process your data. \param		input			[in] a list of floating-point values to sort
 *	\param		nb				[in] number of values to sort, must be < 2^31
            ...
            if (PerformPass) {
                ...
                if (INVALID_RANKS) {
                    for (udword i = 0; i < nb; i++) {
                        ...
                        // Number is positive, same as above 						else
