...
void csOPCODECollider::MeshCallback(udword triangle_index, VertexPointers &triangle, void *user_data)
{
...
bool csOPCODECollider::rayCollide(const Ray &boltbeam, Vector &norm, float &distance)
{
    ...
    if (retval) {
        ...
        if (retval) {
            ...
            return true; // FIXME: buggy! this should return FALSE but the math is obviously broken with opcode, so opcode is rarely telling
                         // us about intersections
        }
        // FIXME set normal
    }
    return retval;
...
bool csOPCODECollider::Collide(csOPCODECollider &otherCollider, const csReversibleTransform *trans1, const csReversibleTransform *trans2)
{
