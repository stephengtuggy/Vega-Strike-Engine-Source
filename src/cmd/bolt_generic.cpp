...
Bolt::Bolt(const weapon_info *typ, const Matrix &orientationpos, const Vector &shipspeed, void *owner, CollideMap::iterator hint)
...
bool Bolt::Update(Collidable::CollideRef index)
{
    ...
    cur_position += ((ShipSpeed + drawmat.getR() * speed /
                                      ((type->type == weapon_info::BALL) * type->Radius + (type->type != weapon_info::BALL) * type->Length))
                         .Cast() *
                     SIMULATION_ATOM);
...
bool Bolt::CollideAnon(Collidable::CollideRef b, std::shared_ptr<Unit> un)
{
