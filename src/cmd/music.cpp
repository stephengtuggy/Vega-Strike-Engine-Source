...
int Music::SelectTracks(int layer)
{
    ...
    static size_t maxrecent = XMLSupport::parse_int(vs_config->getVariable("audio", "shuffle_songs.history_depth", MAX_RECENT_HISTORY));
...
#endif
        {
            ...
            if (foundcache) {
                *me->music_load_info = wherecache->second;
                ...
                VSFileSystem::vs_dbg(1) << boost::format("Failed to load music file \"%1%\"") % songname << std::endl;
            }
        }
    ...
    if (game_options.Music) {
        if (whichsong != NOLIST && whichlist != NOLIST && whichlist < (int)playlist.size() && whichsong < (int)playlist[whichlist].size()) {
            if (muzak[(layer >= 0) ? layer : 0].lastlist != whichlist) {
                static bool clear =
                    XMLSupport::parse_bool(vs_config->getVariable("audio", "shuffle_songs.clear_history_on_list_change", "true"));
                if (clear) {
                    std::list<std::string> &recent = muzak[(layer >= 0) ? layer : 0].recent_songs;
                    recent.clear();
                }
    ...
    if (this != NULL) {
        if (whichlist != NOLIST && whichlist >= 0 && whichlist < (int)playlist.size()) {
            ...
            if (playlist[whichlist].size())
                GotoSong(whichlist,
                         random ? randInt(playlist[whichlist].size()) : playlist[whichlist].counter++ % playlist[whichlist].size(),
                         true,
                         layer);
            else
