...
template <class UnitType> unsigned int GameUnit<UnitType>::nummesh() const
{
...
template <class UnitType>
void GameUnit<UnitType>::addHalo(
    const char *filename, const Matrix &trans, const Vector &size, const GFXColor &col, std::string halo_type, float halo_speed)
{
...
template <class UnitType> void GameUnit<UnitType>::UpdateHudMatrix(int whichcam)
{
    ...
    _Universe->AccessCamera(whichcam)->SetPosition(
            ...
            if (mahnt->xyscale != 0 && mahnt->zscale != 0) {
                ...
                if (gun && mahnt->status != Mount::UNCHOSEN) {
                    ...
                    if (GFXSphereInFrustum(mountLocation.position, gun->rSize() * vlpqrScaleFactor) > 0) {
                        ...
                        float lod = gun->rSize() * g_game.detaillevel *
...
template <class UnitType> void GameUnit<UnitType>::Draw(const Transformation &parent, const Matrix &parentMatrix)
{
        ...
        }
        {
            ...
            for (un_iter iter = this->getSubUnits(); (un = *iter); ++iter) {
                ...
                interpolation_blend_factor =
                    calc_blend_factor(saved_interpolation_blend_factor, un->sim_atom_multiplier, un->cur_sim_queue_slot, cur_sim_frame);
                (un)->Draw(*ct, *ctm);
    ...
    for (i = 0; (int)i < nummounts; i++) {
            ...
            if (mahnt->xyscale != 0 && mahnt->zscale != 0) {
                ...
                if (gun && mahnt->status != Mount::UNCHOSEN) {
                    ...
                    if (GFXSphereInFrustum(mountLocation.position, gun->rSize() * avgscale) > 0) {
                        float d = (mountLocation.position - _Universe->AccessCamera()->GetPosition()).Magnitude();
                        float pixradius =
                            gun->rSize() * perspectiveFactor((d - gun->rSize() < g_game.znear) ? g_game.znear : d - gun->rSize());
                        float lod = pixradius * g_game.detaillevel;
                        if (lod > 0.5 && pixradius > 2.5) {
        ...
        if (this->mounts[i].type->type == weapon_info::BEAM)
            if (this->mounts[i].ref.gun)
                this->mounts[i].ref.gun->Draw(
                    *ct,
                    wmat,
                    ((this->mounts[i].size & weapon_info::AUTOTRACKING) && (this->mounts[i].time_to_lock <= 0) && Unit::TargetTracked())
                        ? Unit::Target()
                        : NULL,
    ...
    if (On_Screen && !UnitType::graphicOptions.NoDamageParticles && !(this->docked & (UnitType::DOCKED | UnitType::DOCKED_INSIDE))) {
        if (damagelevel < .99 && this->nummesh() > 0 && this->GetHull() > 0) {
            unsigned int switcher = (damagelevel > .8) ? 1 : (damagelevel > .6) ? 2 : (damagelevel > .4) ? 3 : (damagelevel > .2) ? 4 : 5;
...
{
    if (s.empty()) {
        VSFileSystem::vs_dbg(1) << boost::format("WARNING: invalid float: %1%") % s << std::endl;
        return 0.f;
    } else {
...
template <class UnitType> Matrix GameUnit<UnitType>::WarpMatrix(const Matrix &ctm) const
{
    if (this->GetWarpVelocity().MagnitudeSquared() <
            (game_options.warp_stretch_cutoff * game_options.warp_stretch_cutoff * game_options.game_speed * game_options.game_speed) ||
        ...
        ScaleMatrix(k,
                    Vector(1,
                           1,
                           1 + (speed > (game_options.warp_stretch_max_region0_speed * game_options.game_speed) ? stretchlength
