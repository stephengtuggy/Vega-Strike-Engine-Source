...
void Bolt::Draw()
{
    ...
    float                          pixel_angle =
        2 * sin(g_game.fov * M_PI / 180.0 / (g_game.y_resolution > g_game.x_resolution ? g_game.y_resolution : g_game.x_resolution)) *
                        ...
                        for (j = i->begin(); j != i->end(); j++) {
                            ...
                            if (distance * pixel_angle < bolt_size) {
                                ...
                                if (game_options.StretchBolts > 0)
                                    ScaleMatrix(drawmat,
                                                Vector(1, 1, bolt.type->Speed * etime * game_options.StretchBolts / bolt.type->Length));
