...
template <class UnitType> void GameUnit<UnitType>::Split(int level)
{
    ...
    std::shared_ptr<CSVRow> unit_stats(LookupUnitRow(this->name, fac));
        ...
        if (percent) {
            ...
            if (game_options.ai_sound)
                AUDPlay(this->sound->shield,
                        this->ToWorldCoordinates(pnt).Cast() + this->cumulative_transformation.position,
                        this->Velocity,
                        1);
        }
    } else {
    ...
    if (this->pImage->pExplosion == NULL && this->pImage->timeexplode == 0) {
        ...
        if (this->isUnit() != MISSILEPTR) {
            _Universe->activeStarSystem()->AddMissileToQueue(
                new MissileEffect(this->Position(),
                                  this->MaxShieldVal(),
                                  0,
        ...
        if (!sub) {
            ...
            if (this->isUnit() == UNITPTR) {
                if (rand() < RAND_MAX * game_options.percent_shockwave && (!this->isSubUnit())) {
                    ...
                    unsigned int which =
                        AddAnimation(this->Position(), this->ExplosionRadius(), true, shockani, game_options.shockwave_growth);
                ...
                if (un) {
                    ...
                    if (!BaseInterface::CurrentBase) {
                        ...
                        if (newtime - lasttime > game_options.time_between_music ||
                            (_Universe->isPlayerStarship(this) && this->isUnit() != MISSILEPTR && this->faction != upgradesfaction)) {
