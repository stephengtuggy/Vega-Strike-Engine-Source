...
static void UpgradeUnit(Unit* un, const std::string& upgrades)
{
    ...
    while ((when = upgrades.find('{', ofs)) != string::npos) {
        ...
        double percent = 1.0;
        un->Unit::Upgrade(upgradee, mountoffset, subunitoffset, GetModeFromName(upgrade.c_str()), true, percent, NULL);
    }
}
    ...
    while ((where = docks.find('{', ofs)) != string::npos) {
        if ((when = docks.find('}', where + 1)) != string::npos) {
            ...
            for (int i = 0; i < overlap; i++)
                thus->pImage->dockingports.push_back(DockingPorts(
                    pos.Cast() * xml.unitscale, size * xml.unitscale, minsize * xml.unitscale, DockingPorts::Type::Value(type)));
        } else {
            ofs = string::npos;
        }
    }
...
{
    std::shared_ptr<CSVTable> table = row.getParent();
    Unit::XML                 xml;
    ...
    AddMeshes(
        xml.meshes, xml.randomstartframe, xml.randomstartseconds, xml.unitscale, OPTIM_GET(row, table, Mesh), faction, getFlightgroup());
    ...
    shieldcount +=
        AssignIf(OPTIM_GET(row, table, Shield_Front_Top_Right), two.shield2fb.front, four.shield4fbrl.front, eight.shield8.frontrighttop);
    shieldcount +=
        AssignIf(OPTIM_GET(row, table, Shield_Front_Top_Left), two.shield2fb.front, four.shield4fbrl.front, eight.shield8.frontlefttop);
    shieldcount +=
        AssignIf(OPTIM_GET(row, table, Shield_Back_Top_Left), two.shield2fb.back, four.shield4fbrl.back, eight.shield8.backlefttop);
    shieldcount +=
        AssignIf(OPTIM_GET(row, table, Shield_Back_Top_Right), two.shield2fb.back, four.shield4fbrl.back, eight.shield8.backrighttop);
    shieldcount += AssignIf(
        OPTIM_GET(row, table, Shield_Front_Bottom_Left), two.shield2fb.front, four.shield4fbrl.left, eight.shield8.frontleftbottom);
    shieldcount += AssignIf(
        OPTIM_GET(row, table, Shield_Front_Bottom_Right), two.shield2fb.front, four.shield4fbrl.right, eight.shield8.frontrightbottom);
    shieldcount +=
        AssignIf(OPTIM_GET(row, table, Shield_Back_Bottom_Left), two.shield2fb.back, four.shield4fbrl.left, eight.shield8.backleftbottom);
    shieldcount += AssignIf(
        OPTIM_GET(row, table, Shield_Back_Bottom_Right), two.shield2fb.back, four.shield4fbrl.right, eight.shield8.backrightbottom);
    ...
    graphicOptions.RecurseIntoSubUnitsOnCollision =
        stob(OPTIM_GET(row, table, Collide_Subunits), graphicOptions.RecurseIntoSubUnitsOnCollision ? true : false) ? 1 : 0;
    jump.energy      = ::stof(OPTIM_GET(row, table, Outsystem_Jump_Cost));
    jump.insysenergy = ::stof(OPTIM_GET(row, table, Warp_Usage_Cost));
    if (WCfuelhack)
        fuel = warpenergy = warpenergy + jump.energy * 0.1f; // this is required to make sure we don't trigger the "globally out of fuel" if
                                                             // we use all warp charges -- save some afterburner for later!!!
    afterburnenergy = ::stof(OPTIM_GET(row, table, Afterburner_Usage_Cost), 32767);
    afterburntype   = ::stoi(OPTIM_GET(
        row,
        table,
        Afterburner_Type)); // type 1 == "use fuel", type 0 == "use reactor energy", type 2 ==(hopefully) "use jump fuel" 3: NO AFTERBURNER
    ...
    limits.lateral =
        .5 * (::stof(OPTIM_GET(row, table, Left_Accel)) + ::stof(OPTIM_GET(row, table, Right_Accel))) * game_accel * game_speed;
    limits.vertical =
        .5 * (::stof(OPTIM_GET(row, table, Top_Accel)) + ::stof(OPTIM_GET(row, table, Bottom_Accel))) * game_accel * game_speed;
    ...
    computer.radar.canlock       = stob(OPTIM_GET(row, table, Can_Lock), true);
    {
        ...
        if ((iffval.empty()) || (iffval == "FALSE") || (iffval == "0")) {
            ...
            computer.radar.capability = Computer::RADARLIM::Capability::IFF_BUBBLE | Computer::RADARLIM::Capability::IFF_FRIEND_FOE |
    ...
    // Begin the Pow-w-w-war Zone Collide Tree Generation
    {
        ...
        if (!this->colTrees) {
            ...
            if (xml.hasColTree) {
                csrc = getCollideTree(Vector(1, 1, 1), xml.rapidmesh ? &polies : NULL);
            }
...
Unit* Unit::makeMasterPartList()
{
    ...
    std::shared_ptr<CSVTable> table = loadCSVTableList(mpl, VSFileSystem::UnknownFile, false);
