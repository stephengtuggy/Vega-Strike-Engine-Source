                         ...
                         std::shared_ptr<Unit> superunit)
{
    ...
    if (curthick <= 0) {
        ...
        if (TableLocationChanged(CollideInfo, tmpMini, tmpvec) || (curthick > 0 && CollideInfo.object.b == NULL)) {
            ...
            CollideInfo.hhuge =
                (((CollideInfo.Maxi.i - CollideInfo.Mini.i) / coltableacc) * ((CollideInfo.Maxi.j - CollideInfo.Mini.j) / coltableacc) *
        ...
        if (tractor || repulsor) {
            ...
            if (fp || target->isTractorable(Unit::tractorPush)) {
                ...
                if (relspeed < maxrelspeed) {
                    ...
                    target->ApplyForce(direction * (appldam / sqrt((target->sim_atom_multiplier > 0) ? target->sim_atom_multiplier : 1.0) *
                                                    mymin(1, target->GetMass())));
                }
            }
                            ...
                            if (_Universe->isPlayerStarship(un)) {
                                ...
                                if (tmp && tmp->owner == un) {
                                    ...
                                    static int tractor_onboard_fromturret = AUDCreateSoundWAV(
                                        vs_config->getVariable("unitaudio", "player_tractor_cargo_fromturret", "tractor_onboard.wav"));
