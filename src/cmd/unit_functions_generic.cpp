...
const std::shared_ptr<Unit> getUnitFromUpgradeName(const string &upgradeName, int myUnitFaction = 0)
{
    ...
    if (partUnit->name == "LOAD_FAILED") {
        ...
        if (!partUnit)
            partUnit =
                UnitConstCache::setCachedConst(StringIntKey(name, myUnitFaction), UnitFactory::createUnit(name, true, myUnitFaction));
    }
    return partUnit;
}
...
void SetShieldZero(std::shared_ptr<Unit> un)
{
    ...
    case 8:
        ...
        break;
    case 4:
        un->shield.shield4fbrl.front = un->shield.shield4fbrl.back = un->shield.shield4fbrl.left = un->shield.shield4fbrl.right = 0;
        break;
    case 2:
