...
/// This set inherits from the STL multiset, with a slight variation: The value is nonconst--that means you are allowed to change things but
/// must not alter the key. This set inherits from the STL multiset, with a slight variation: You are allowed to update the key of a
/// particular iterator that you have obtained.
/** Note: T is the type that each element is pointing to. */
template <class T, class _Compare = std::less<MutableShell<T>>> class KeyMutableSet : public std::multiset<MutableShell<T>, _Compare>
{
  ...
  public:
    ...
    void
    changeKey(typename SUPER::iterator &iter, const T &newKey, typename SUPER::iterator &templess, typename SUPER::iterator &rettempmore)
    {
