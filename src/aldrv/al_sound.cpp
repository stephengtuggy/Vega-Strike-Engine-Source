    ...
    if (ogg.size() > 4) {
        if (ogg[0] == 'O' && ogg[1] == 'g' && ogg[2] == 'g' && ogg[3] == 'S') {
            ...
            if (ov_open_callbacks(&ff, &vf, NULL, 0, callbacks)) {
                ...
                int signedvalue = 1;
                int bitstream   = 0;
                while ((bytesread = ov_read(
                            &vf, &converted[converted.size() - segmentsize], segmentsize, 0, samples / 8, signedvalue, &bitstream)) > 0) {
                    int numtoerase = 0;
...
static int LoadSound(ALuint buffer, bool looping, bool music)
{
    ...
    if (!dirtysounds.empty()) {
        ...
        if (sounds[i].buffer != (ALuint)0) {
            VSFileSystem::vs_dbg(3) << boost::format("using claimed buffer %1%") % sounds[i].buffer << std::endl;
        }
...
void AUDPlay(const int sound, const QVector &pos, const Vector &vel, const float gain)
{
    ...
    if ((tmp = AUDQueryAudability(sound, pos.Cast(), vel, gain)) != 0) {
        if (AUDReclaimSource(sound, pos == QVector(0, 0, 0))) {
            ...
            if (tmp != 2) {
                VSFileSystem::vs_dbg(3) << boost::format("AUDPlay sound %1% %2%") % sounds[sound].source % sounds[sound].buffer
                                        << std::endl;
                AUDAddWatchedPlayed(sound, pos.Cast());
            } else {
                VSFileSystem::vs_dbg(3) << boost::format("AUDPlay stole sound %1% %2%") % sounds[sound].source % sounds[sound].buffer
                                        << std::endl;
                alSourceStop(sounds[sound].source);
            }
            alSourcePlay(sounds[sound].source);
