...
void NetServer::broadcastUnit(std::shared_ptr<Unit> un, unsigned short zone)
{
...
void NetServer::sendNewUnitQueue()
{
    ...
    for (ZoneMap::const_iterator zoneiter = zonemgr->zones.begin(); zoneiter != zonemgr->zones.end(); ++zoneiter) {
        ...
        for (int verind = 0; verind < 4; verind += 2) {
            ...
            NetBuffer      netbuf;
            if (maxver != 65535)
                netbuf.setVersion(maxver);
            bool added = false;
            for (std::vector<UnitContainer>::iterator iter = newUnits.begin(); iter != newUnits.end(); ++iter) {
                std::shared_ptr<Unit> un = (*iter).GetUnit();
                if (un) {
                    ...
                    if (sys) {
                        ...
                        if (unzone == zone) {
                            ...
                            if (verind == 0)
                                displayUnitInfo(un, string(), (" * CREATED " + un->getStarSystem()->getFileName()).c_str());
                        }
                    }
                } else {
            ...
            if (added) {
                ...
                Packet p2;
                p2.bc_create(CMD_ENTERCLIENT, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(66));
...
void NetServer::addClient(ClientPtr clt)
{
    ...
    if (!dockedUnit) {
        ...
        cerr << "PLAYER DOCKED TO " << dockedUnit->getFullname() << " - STARTING DOCKED AT POSITION : x=" << safevec.i << ",y=" << safevec.j
             << ",z=" << safevec.k << endl;
    }
    ...
    if (sts) {
        ...
        packet2.bc_create(
            CMD_ENTERCLIENT, un->GetSerial(), netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1311));
        COUT << "<<< SEND ENTERCLIENT(" << un->GetSerial() << ") TO OTHER CLIENT IN THE ZONE------------------------------------------"
             << endl;
    ...
    netbuf.addSerial(0);
    p2.bc_create(CMD_SNAPCARGO, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(179));
    zonemgr->broadcast(un->activeStarSystem->GetZone(), un->GetSerial(), &p2, true);

    COUT << "ADDED client n " << un->GetSerial() << " in ZONE " << un->activeStarSystem->GetZone() << " at STARDATE "
...
void NetServer::serverTimeInitUDP(ClientPtr clt, NetBuffer &netbuf)
{
    ...
    p2.send(CMD_SERVERTIME,
            0,
            timeBuf.getData(),
            timeBuf.getDataLength(),
            SENDANDFORGET,
            &clt->cltudpadr,
            *clt->lossy_socket,
            __FILE__,
...
void NetServer::removeClient(ClientPtr clt)
{
    ...
    packet2.bc_create(CMD_EXITCLIENT, un->GetSerial(), NULL, 0, SENDRELIABLE, __FILE__, PSEUDO__LINE__(1311));
    zonemgr->broadcast(clt, &packet2, true);
}
 ...
 */
ClientState
aim_assist(ClientState cs, ClientState ocs /*old*/, QVector realtargetpos, Vector realtargetvel, Vector targetpos, Vector targetvel)
{
    float       t              = 0;
    ...
    if (targetperpmag > .001 && realtargetperpmag > .001 && targetvel.MagnitudeSquared() > .25 && realtargetvel.MagnitudeSquared() > .25 &&
        ...
        realtargetperp =
            realtargetperp *
            (1. / realtargetperpmag); // compute a unit direction perpendicular to the plane made by the target and its relative velocity
        ...
        if (fabs(t) < velaimlim) {
            ...
            VelocityAimValid   = FINITE(fvel.i) && FINITE(fvel.j) && FINITE(fvel.k) && FINITE(forient.s) && FINITE(forient.v.i) &&
                               FINITE(forient.v.j) && FINITE(forient.v.k);
    ...
    }
    {
        ...
        OrientationAimValid = FINITE(fvel.i) && FINITE(fvel.j) && FINITE(fvel.k) && FINITE(forient.s) && FINITE(forient.v.i) &&
                              FINITE(forient.v.j) && FINITE(forient.v.k);
...
ClientState aim_assist(ClientState cs, ClientState ocs /*old*/, std::shared_ptr<Unit> target, Vector targetpos, Vector targetvel)
{
...
void NetServer::posUpdate(ClientPtr clt)
{
    ...
    if (cp)
        ...
        COUT << "ERROR: Client has NULL cockpit. Does he have two clients ingame? ser=" << un->GetSerial() << ", callsign=" << clt->callsign
             << endl;
    snapchanged = 1;
}
...
void NetServer::disconnect(ClientPtr clt, const char *debug_from_file, int debug_from_line)
{
    COUT << "enter " << __PRETTY_FUNCTION__ << endl
         ...
         << " *** disconnecting " << clt->callsign << " because of " << clt->_disconnectReason << endl;
