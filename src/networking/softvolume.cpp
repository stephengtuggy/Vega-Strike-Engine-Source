...
class shape_sampler
{
  public:
    ...
    {
        return interpolateF16F16(
            shape[SHAPE_SAMPLE_SAFEPOS(this->pos >> 16)], shape[SHAPE_SAMPLE_SAFEPOS(this->pos >> 16) + 1], this->pos & 0xFFFF);
    }
...
inline unsigned short mpyUS16F16(unsigned short a, unsigned long f16)
{
    return (unsigned short)min((SDL_UINT64)US16_MAX,
                               max((SDL_UINT64)US16_MIN, (SDL_UINT64)(((((((SDL_INT64)a) - 0x8000) * f16) + rng16.get()) >> 16) + 0x8000)));
}
...
inline unsigned short mpyUS16F16(unsigned short a, unsigned long f16)
{
    return (unsigned short)min(US16_MAX,
                               max(US16_MIN, (unsigned long)(((((((signed long)a) - 0x8000) * (f16 >> 8)) + rng16.get()) >> 8) + 0x8000)));
}
