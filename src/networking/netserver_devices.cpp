...
void NetServer::BroadcastTarget(ObjSerial serial, ObjSerial oldtarget, ObjSerial target, unsigned short zone)
{
    ...
    if (clt)
        p.send(
            CMD_TARGET, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, PSEUDO__LINE__(24));
    netbuf.Reset();
    netbuf.addSerial(0);
    clt = this->getClientFromSerial(oldtarget);
    if (clt)
        p.send(
            CMD_TARGET, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, PSEUDO__LINE__(34));
...
void NetServer::BroadcastUnfire(ObjSerial serial, const vector<int> &weapon_indicies, unsigned short zone)
{
    ...
    p.bc_create(CMD_UNFIREREQUEST, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1695));
    zonemgr->broadcast(zone, serial, &p, true); // NETFIXME: Should unfire be TCP?
}

// In BroadcastFire we must use the provided serial because it may not be the client's serial
// but may be a turret serial
void NetServer::BroadcastFire(
    ObjSerial serial, const vector<int> &weapon_indicies, ObjSerial missile_serial, float player_energy, unsigned short zone)
{
    Packet    p;
    NetBuffer netbuf;
    // bool found = false;
    ...
    for (unsigned int i = 0; i < weapon_indicies.size(); i++)
        netbuf.addInt32(weapon_indicies[i]);
    p.bc_create(CMD_FIREREQUEST, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1710));
                                      ...
                                      unsigned short     zone)
{
    ...
    p.bc_create(CMD_CARGOUPGRADE, sender, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(87));
...
void NetServer::sendCargoSnapshot(ObjSerial cltser, const UnitCollection &list)
{
    ...
    netbuf.addSerial(0);
    p2.send(CMD_SNAPCARGO, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, PSEUDO__LINE__(196));
}

void NetServer::sendSaveData(
    int cp, unsigned short type, int pos, const string *key, Mission *miss, const string *strValue, const float *floatValue)
{
    ...
    p2.send(CMD_SAVEDATA, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, PSEUDO__LINE__(164));
}
...
void NetServer::sendMission(int cp, unsigned short packetType, string mission, int pos)
{
    ...
    p.send(CMD_MISSION, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, PSEUDO__LINE__(186));
}
                            ...
                            GFXColor &     color)
{
    ...
    if (lastserial != serial || curtime - timestamp > mintime) {
        ...
        p.bc_create(CMD_DAMAGE, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1729));
...
void NetServer::sendCustom(int cp, const string &command, const string &args, const string &id)
{
    ...
    p2.send(CMD_CUSTOM, un->GetSerial(), netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, __LINE__);
}
...
void NetServer::sendMessage(const string &from, const string &to, const string &message, float delay)
{
    ...
    if (to[0] == 'p') {
        ...
        if (playerto != -1 && (unsigned int)playerto < _Universe->numPlayers()) {
            ...
            if (!clt)
                return;
            p2.send(CMD_TXTMESSAGE,
                    0,
                    netbuf.getData(),
                    netbuf.getDataLength(),
                    SENDRELIABLE,
                    NULL,
                    clt->tcp_sock,
                    __FILE__,
        ...
        p2.bc_create(CMD_TXTMESSAGE, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(229));
        if (to == "broadcast") {
            for (ZoneMap::const_iterator iter = zonemgr->zones.begin(); iter != zonemgr->zones.end(); ++iter)
...
void NetServer::sendCommunication(std::shared_ptr<Unit> from, std::shared_ptr<Unit> to, const class CommunicationMessage *c)
{
    ...
    Packet p2;
    p2.send(CMD_COMM, from->GetSerial(), netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, clt->tcp_sock, __FILE__, __LINE__);
}
...
void NetServer::sendKill(ObjSerial serial, unsigned short zone)
{
    ...
    p.bc_create(CMD_KILL, serial, NULL, 0, SENDRELIABLE, __FILE__, PSEUDO__LINE__(1771));
    // WARNING : WE WILL SEND THE INFO BACK TO THE CLIENT THAT HAS FIRED
...
void NetServer::sendJump(std::shared_ptr<Unit> un, std::shared_ptr<Unit> dst, const string &dststr)
{
    ...
    if (clt && (cpnum = _Universe->whichPlayerStarship(un)) >= 0 && un != NULL) {
        ...
        /*
         *  savestr = cp->savegame->WriteSaveGame
         * (fn.c_str(),un->LocalPosition(),cp->credits,cp->unitfilename,-1,FactionUtil::GetFactionName(un->faction), false); csvstr =
         * un->WriteUnitString();
         */
...
void NetServer::sendDockAuthorize(ObjSerial serial, ObjSerial utdw_serial, int docknum, unsigned short zone)
{
    ...
    p.bc_create(CMD_DOCK, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(118));
    zonemgr->broadcast(zone, serial, &p, true);
...
void NetServer::sendUnDock(ObjSerial serial, ObjSerial utdwserial, unsigned short zone)
{
    ...
    p.bc_create(CMD_UNDOCK, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(134));
    zonemgr->broadcast(zone, serial, &p, true);
