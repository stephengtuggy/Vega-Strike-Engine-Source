...
#ifdef VSNET_DEBUG
    if (!e)
        COUT << "_sq " << (_sq.empty() ? "empty" : "not empty") << ", _sq_current " << (_sq_current.empty() ? "empty" : "not empty")
             << endl;
#endif
    _sq_mx.unlock();
    return !e;
...
bool VsnetTCPSocket::lower_selected(int datalen)
{
    ...
    if (get_nonblock() == false && datalen == -1)
        endless = false;
    do {
        if ((_incomplete_header > 0) || (_incomplete_header == 0 && _incomplete_packet == 0)) {
            ...
            if (ret <= 0) {
                if (ret == 0 || vsnetEConnAborted() || vsnetEConnReset()) {
                    ...
                    COUT << "recv returned " << ret << " errno " << vsnetGetLastError() << " = " << vsnetLastError() << endl;
