...
SocketSet::SocketSet(bool blockmainthread) : VSThread(false), _blockmain(blockmainthread), _blockmain_pending(0)
{
...
int SocketSet::wait(timeval *tv)
{
    ...
    if (_blockmain_pending == 0) {
        ...
        for (int i = 0; i < _blockmain_pending; i++)
            if (FD_ISSET(i, &_blockmain_set))
                ostr << " " << i;
        COUT << "something pending for sockets:" << ostr.str() << " (" << _blockmain_pending << ")" << endl;
#endif
...
void SocketSet::add_pending(int fd)
{
    if (_blockmain) {
        ...
        if (fd >= _blockmain_pending)
            _blockmain_pending = fd + 1;
        _blockmain_mx.unlock();
    }
}
...
int SocketSet::private_select(timeval *timeout)
{
    ...
    for (Set::iterator it = _autoset.begin(); it != _autoset.end(); it++) {
        ...
        if (fd >= 0) {
            ...
            if (b->need_test_writable() && !wrote_on_negative) {
                private_addset(b->get_write_fd(), write_set_select, max_sock_select);
            }
...
#ifndef USE_NO_THREAD
    private_addset(_thread_wakeup.getread(), read_set_select, max_sock_select);
#endif
    ...
    int ret = ::select(max_sock_select, &read_set_select, &write_set_select, 0, timeout);
    if (ret == -1) {
        ...
        private_test_dump_active_sets(
            max_sock_select, debug_copy_of_read_set_select, read_set_select, debug_copy_of_write_set_select, write_set_select);
#endif
        for (Set::iterator it = _autoset.begin(); it != _autoset.end(); it++) {
            VsnetSocketBase *b  = (*it);
            int              fd = b->get_fd();
...
// printf("reconnecting?\n");
#ifdef VSNET_DEBUG
                    COUT << "saw activity on " << b->get_write_fd() << " but main file descriptor is " << b->get_fd() << endl;
#endif
                    b->lower_clean_sendbuf();
                }
            }
...
void SocketSet::private_test_dump_active_sets(int maxfd, fd_set &read_before, fd_set &read_after, fd_set &write_before, fd_set &write_after)
{
#if 0
