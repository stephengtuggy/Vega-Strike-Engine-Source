...
ClientState::ClientState(const std::shared_ptr<Unit> un)
{
    ...
    if (un->graphicOptions.RampCounter != 0 || un->graphicOptions.InWarp) {
        this->SPECRampCounter    = (un->graphicOptions.InWarp ? 1 : -1) * un->graphicOptions.RampCounter;
...
{
    ostr << "SERIAL=" << this->client_serial << " - Position=" << pos.position.i << "," << pos.position.j << "," << pos.position.k
         << " - Orientation=" << pos.orientation.v.i << "," << pos.orientation.v.j << "," << pos.orientation.v.k
         << " - Velocity=" << veloc.i << "," << veloc.j << ","
         << veloc.k
...
int ClientState::operator==(const ClientState &ctmp) const
{
    ...
    return this->client_serial == ctmp.client_serial && this->pos.position == ctmp.pos.position &&
           this->pos.orientation == ctmp.pos.orientation;
    // return 0;
}
...
{
    ostr << "(clnt addr " << c.cltadr << " tcpsock=" << c.tcp_sock << ", lossysock=";
    if (c.lossy_socket)
        ostr << *c.lossy_socket;
    else
        ostr << "NULL";
