...
VsnetHTTPSocket::VsnetHTTPSocket(const AddressIP &remote_ip, const std::string &host, const std::string &path, SocketSet &set)
...
VsnetHTTPSocket::VsnetHTTPSocket(const std::string &uri, SocketSet &set)
    : VsnetSocketBase(-1, "http", set), _incompleteheadersection(0), _content_length(0), _send_more_data(false), sendDataPos(0)
{
    numRetries        = 0;
    timeToNextRequest = 0;
    readHeader        = false;
...
int        VsnetHTTPSocket::lower_sendbuf()
{
    ...
    if (!(_send_more_data || _content_length || _incompleteheader.length() || _header.size())) {
    ...
    while (_incompleteheadersection >= 2) {
        ...
        if (_incompleteheadersection > 2 || !isspace(rcvchr)) {
            if (_header.empty()) {
                ...
                _header.insert(std::pair<std::string, std::string>("Status", _incompleteheader.substr(sp1 + 1 /*, sp2-sp1-1*/)));
            } else {
                ...
                do
                    colonsp++;
                while (colonsp < _incompleteheader.length() && isspace(_incompleteheader[colonsp]));
                _header.insert(std::pair<std::string, std::string>(_incompleteheader.substr(0, colon), _incompleteheader.substr(colonsp)));
            }
            _incompleteheader = "";
        }
        break; // always break
...
bool VsnetHTTPSocket::lower_selected(int datalen)
{
    ...
    while (datalen != 0) {
        ...
        if (ret > 0)
            cout << "READ DATA: " << std::string(rcvbuf, ret) << endl; // DELETEME!
        if (ret <= 0)
            cout << "recv returned " << ret << endl;
        if (ret == 0) {
            // It is not an error if closed without a Content-Length header.
            if (_content_length >= 0 &&
                ((readHeader == false /*set to false on success return*/ && dataToReceive.length() == 0) || readHeader == true)) {
                // incomplete transfer...
        ...
        if (datalen > 0 || datalen < 0) {
                ...
                for (bufpos = 0; bufpos < ret; bufpos++) {
                    ...
                    if (parseHeaderByte(rcvchr)) {
                        ...
                        if ((*iter).second.find("100") != std::string::npos) {
                            ...
                            if ((*iter).second.find("301") != std::string::npos || (*iter).second.find("302") != std::string::npos ||
                                (*iter).second.find("303") != std::string::npos || (*iter).second.find("307") != std::string::npos) {
            ...
            while (bufpos < ret) {
                for (; bufpos < ret && readingchunked; bufpos++) {
                    ...
                    if (rcvchr == ';')
                        chunkedchar = ';'; // this means extension follow and we shouldn't add numbers
                }
