...
namespace Client
{
    ...
    Manager::Manager(SocketSet &sets, const char **local_search_paths) : _set(sets)
    {
    ...
    Manager::Manager(SocketSet &sets) : _set(sets)
    {
        COUT << "Enter " << __PRETTY_FUNCTION__ << endl;
    ...
    bool Manager::private_lower_test_access(Item *i)
    {
        ...
        for (vector<string>::const_iterator it = _local_search_paths.begin(); it != _local_search_paths.end(); it++) {
            string path = *it + "/" + file;
...
namespace Server
{
    ...
    DownloadItem::DownloadItem(SOCKETALT sock, bool error, const string &file) : _sock(sock), _error(error), _file(file)
    {
    }
    ...
    DownloadItemFile::DownloadItemFile(SOCKETALT sock, const string &file)
        : DownloadItem(sock, true, file), _handle(NULL), _size(0), _offset(0)
    {
    }

    DownloadItemFile::DownloadItemFile(SOCKETALT sock, const string &file, VSFileSystem::VSFile *f, size_t sz)
        : DownloadItem(sock, false, file), _handle(f), _size(sz), _offset(0)
    {
    }
    ...
    DownloadItemBuf::DownloadItemBuf(SOCKETALT sock, const string &file, const char *buf, size_t sz)
        : DownloadItem(sock, false, file), _size(sz), _offset(0)
    {
        _buf = new char[sz];
    ...
    LOCALCONST_DEF(Manager, int, _packetWorkahead, 5) Manager::Manager(SocketSet &sets, const char **local_search_paths) : _set(sets)
    {
        COUT << "Enter " << __PRETTY_FUNCTION__ << endl;
