...
VsnetSocket::VsnetSocket(int sock, const AddressIP &remote_ip, const char *socktype, SocketSet &sets)
    : VsnetSocketBase(sock, socktype, sets), _remote_ip(remote_ip)
{
}
...
VsnetSocketBase::VsnetSocketBase(int fd, const char *socktype, SocketSet &sets) : _fd(fd), _set(&sets), _noblock(-1)
{
    MAKE_VALID
