...
Packet::Packet(const void *buffer, size_t sz)
{
    ...
    if (sz >= header_length) {
        ...
        if (h.data_length > sz) {
            ...
            if (packet_uncompress(_packet, (const unsigned char *)buffer, h.data_length, h) == false)
                display(__FILE__, __LINE__NOMSC);
        } else {
            ...
            COUT << "Parsed a packet with"
                 ...
                 << " ser=" << h.serial << " ts=" << h.timestamp << " len=" << h.data_length << endl;
#endif
        }
    } else {
...
#ifdef HAVE_ZLIB_H
            if (packet_uncompress(_packet, (const unsigned char *)buffer.getConstBuf(), h.data_length, h) == false)
                display(__FILE__, __LINE__NOMSC);
#else  /* HAVE_ZLIB_H */
...
void Packet::create(Cmd cmd, ObjSerial nserial, const char *buf, unsigned int length, int prio, const char *caller_file, int caller_line)
{
    CHECK_VALID
    ...
    COUT << "enter " << __PRETTY_FUNCTION__ << endl
         ...
         << "    *** create " << cmd << " ser=" << nserial << ", " << length << "    *** curtime " << curtime << " microtime " << microtime
...
#ifdef VSNET_DEBUG
                COUT << "Created a compressed packet of length " << h.data_length + header_length << " for sending" << endl;
#endif
...
#ifdef VSNET_DEBUG
                COUT << "Compressing " << cmd << " packet refused - bigger than original" << std::endl;
#endif
...
void Packet::display(const char *file, int line)
{
    CHECK_VALID
    cout << "*** " << file << ":" << line << " " << endl;
    cout << "*** Packet display -- Command : " << Cmd(h.command) << " - Serial : " << h.serial << " - Flags : " << h.flags << endl;
...
int Packet::send(SOCKETALT dst_s, const AddressIP *dst_a)
{
    ...
    if (dst_a == NULL)
        ...
        COUT << "sending " << Cmd(h.command) << " through " << dst_s << " to " << *dst_a << endl;
#endif
...
bool Packet::packet_uncompress(PacketMem &outpacket, const unsigned char *src, size_t sz, Header &header)
{
    ...
    if (zlib_errcode != Z_OK) {
        COUT << "Compressed packet not correctly received, "
             << "decompression failed with zlib errcode " << zlib_errcode << endl;
        return false;
        ...
        COUT << "Parsed a compressed packet with"
             ...
             << " ser=" << header.serial << " ts=" << header.timestamp << " len=" << header.data_length << endl;
#endif
        return true;
    }
}
