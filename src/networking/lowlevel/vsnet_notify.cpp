...
namespace Client
{
    ...
    NotifyMe::NotifyMe() : _state(Idle), _error(Ok), _total(0), _offset(0)
    {
    }
    ...
    Item::Item(SOCKETALT sock, const string &filename, VSFileSystem::VSFileType ft, NotifyPtr notify)
        : _filetype(ft), _sock(sock), _filename(filename), _state(Idle), _error(Ok), _notify(notify)
    {
    }
    ...
    File::File(SOCKETALT sock, const string &filename, string localbasepath, VSFileSystem::VSFileType ft, NotifyPtr notify)
        : Item(sock, filename, ft, notify), _destfile(filename), _localbasepath(localbasepath), _of(NULL), _len(0), _offset(0)

    {
    }

    File::File(
        const string &destfile, SOCKETALT sock, const string &filename, string localbasepath, VSFileSystem::VSFileType ft, NotifyPtr notify)
        : Item(sock, filename, ft, notify), _destfile(destfile), _localbasepath(localbasepath), _of(NULL), _len(0), _offset(0)
    ...
    NoteFile::NoteFile(SOCKETALT sock, const std::string &filename, VSFileSystem::VSFileType ft, std::string localbasepath)
        : File(sock, filename, localbasepath, ft, NotifyPtr()), _me(new NotifyMe)
    {
        protected_replace_notifier(_me);
    }

    NoteFile::NoteFile(SOCKETALT sock, const std::string &filename, VSFileSystem::VSFileType ft)
        : File(sock, filename, VSFileSystem::homedir, ft, NotifyPtr()), _me(new NotifyMe)
    {
        protected_replace_notifier(_me);
    }

    NoteFile::NoteFile(const std::string &destfile, SOCKETALT sock, const std::string &filename, VSFileSystem::VSFileType ft)
        : File(destfile, sock, filename, VSFileSystem::homedir, ft, NotifyPtr()), _me(new NotifyMe)
    {
        protected_replace_notifier(_me);
    }
    ...
    Buffer::Buffer(SOCKETALT sock, const string &filename, VSFileSystem::VSFileType ft)
        : Item(sock, filename, ft, NotifyPtr()), _me(new NotifyMe), _len(0), _offset(0)
    {
        protected_replace_notifier(_me);
    }
    ...
    TestItem::TestItem(SOCKETALT sock, const string &filename) : Item(sock, filename, VSFileSystem::UnknownFile, NotifyPtr(this))
    {
    ...
    FileSet::NotifyConclusion::NotifyConclusion(FileSet *f, std::string s) : _fileset(f), _file(s)
    {
    }
    ...
    FileSet::FileSet(boost::shared_ptr<Manager> mgr, SOCKETALT sock, std::list<std::string> filenames, std::string path) : _to_go(0)
    {
    ...
    Notify_f::Notify_f(std::string filename, NotifyFunction fun) : _filename(filename), _fun(fun), _total(0), _offset(0)
    {
    }
