...
enum Cmd {
    ...
    CMD_DOWNLOAD = 0x1E, // May be identical to CMD_ASKFILE in the end
                         // CMD_unused_0x1F = 0x1F ,		// This means a webcam-shot just arrived (from client to server and from
                         // server to concerned clients)

    // 2* Misc. commands

    CMD_SOUNDSAMPLE = 0x20,  // This means a sound sample just arrived (from client to server and from server to concerned clients or from
                             // client to clients)
    ...
    CMD_SECMESSAGE   = 0x24, // This means a sound sample just arrived (from client to server and from server to concerned clients or from
                             // client to clients)
    CMD_SECSNDSAMPLE = 0x25, // This means a sound sample just arrived (from client to server and from server to concerned clients or from
                             // client to clients)
