...
std::shared_ptr<Unit> NetClient::enterClient(NetBuffer &netbuf, ObjSerial cltserial)
{
    ...
    if (!isLocalSerial(cltserial)) {
        ...
        if (cltname.length()) {
            ...
            if (savestr.length() == 0) {
                if (this->lastsave.size() && this->lastsave[0].length() && this->callsign == cltname)
...
ClientPtr NetClient::AddClientObject(std::shared_ptr<Unit> un, ObjSerial cltserial)
{
        ...
        cltserial = un->GetSerial();
    {
        ...
        if (fg)
            fgname = ", fg " + fg->name;
        COUT << " >>> ENTERING CLIENT =( " << cltserial << fgname << " )= -----------------" << endl;
    }
...
void NetClient::sendAlive()
{
    ...
    if (!un)
        return;
    Packet p;
    p.send(CMD_PING, un->GetSerial(), (char *)NULL, 0, SENDANDFORGET, NULL, *this->lossy_socket, __FILE__, PSEUDO__LINE__(414));
    //}
}
