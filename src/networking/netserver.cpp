...
void NetServer::start(int argc, char **argv)
{
    ...
    string tmp;
    acctserver = XMLSupport::parse_bool(
        vs_config->getVariable("server", "useaccountserver", vs_config->getVariable("network", "use_account_server", "false")));
        ...
        if (local) {
            ...
            for (int i = 0; i < 5 && localaddr[i]; i++) {
                ...
                if (ipaddr.substr(0, 4) == "127.") {
                    ...
                    else if (ipaddr.substr(0, 8) == "192.168." || ipaddr.substr(0, 3) == "10.")
...
bool NetServer::updateTimestamps(ClientPtr cltp, Packet &p)
{
    ...
    if (int_ts < clt->getLatestTimestamp()) {
        ...
        //{
        ret =
            !(p.getCommand() == CMD_SNAPSHOT || p.getCommand() == CMD_POSUPDATE ||
              p.getCommand() ==
                  CMD_PING); // only invalidates updates if its a snapshot or posupdate--same reason it updates the timestamps to begin with
    }
    //}
    /*
     *  else if( clt->isTcp())
...
void NetServer::processPacket(ClientPtr clt, unsigned char cmd, const AddressIP &ipadr, Packet &p)
{
    ...
    case CMD_CONNECT: {
        ...
        psend.send(CMD_CONNECT,
                   0,
                   netnewbuf.getData(),
                   netnewbuf.getDataLength(),
                   SENDRELIABLE,
                   &ipadr,
                   clt->tcp_sock,
                   __FILE__,
    ...
    case CMD_TXTMESSAGE: {
        ...
        p2.bc_create(
            CMD_TXTMESSAGE, un->GetSerial(), netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1293));
    ...
    case CMD_SAVEACCOUNTS:
        COUT << "Received a save request for " << clt->callsign << " (" << packet_serial << ")..." << endl;
        un = clt->game_unit.GetUnit();
        if (un) {
    ...
    case CMD_SHIPDEALER: {
        ...
        if (type == Subcmd::BuyShip) {
            ...
            if (cargIndex == UINT_MAX || !cargptr)
                break;
            if (cargptr->price > cp->credits)
                break;
            /* // Do the transaction.
    ...
    case CMD_FIREREQUEST:
        ...
        if (un == NULL) {
            ...
            vector<Mount>::iterator i =
                un->mounts
                    .begin(); // note to self: if vector<Mount *> is ever changed to vector<Mount> remove the const_ from the const_iterator
    ...
    case CMD_CARGOUPGRADE: {
        ...
        if (!cargptr) {
            ...
            if (!cargptr) {
                fprintf(stderr,
                        ...
                        sender ? sender->GetSerial() : -1,
                        cargoName.c_str(),
                        buyer ? buyer->GetSerial() : -1,
                        seller ? seller->GetSerial() : -1);
                // Return the credits.
        ...
        if (didMoney) {
            ...
            if (!upgrade) {
                BroadcastCargoUpgrade(sender->GetSerial(),
                                      buyer_ser,
                                      seller->GetSerial(),
                                      cargoName,
                                      carg.GetPrice(),
    ...
    case CMD_TARGET:
            ...
            break;
        {
            ...
            if (!ss) {
                COUT << "StarSystem for client " << clt->callsign << ", " << unclt->GetSerial() << " not found!" << endl;
                break;
            }
    ...
    case CMD_STARTNETCOMM: {
        ...
        p2.bc_create(
            packet.getCommand(), packet_serial, packet.getData(), packet.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1293));
        // Send to concerned clients
    ...
    case CMD_STOPNETCOMM:
        ...
        p2.bc_create(
            packet.getCommand(), packet_serial, packet.getData(), packet.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1302));
        // Send to concerned clients
        zonemgr->broadcast(un->getStarSystem()->GetZone(), packet_serial, &p2, true);
        break;
    case CMD_SOUNDSAMPLE:
        ...
        p2.bc_create(
            packet.getCommand(), packet_serial, packet.getData(), packet.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(1341));
...
{
    Packet p;
    p.bc_create(command, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, __FILE__, PSEUDO__LINE__(902));
