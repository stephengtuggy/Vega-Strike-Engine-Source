...
void Prediction::InitInterpolation(std::shared_ptr<Unit> un, const ClientState &last_packet_state, double elapsed_since_last_packet, double deltatime)
{
    ...
    if (elapsed_since_last_packet > 0.)
        AB = (last_packet_state.getVelocity() * last_packet_state.getSpecMult() - VB) / ((float)elapsed_since_last_packet);
    ...
    // cerr << un->old_state<<", B="<<B.i<<",,"<<B.k<<", VB="<<VB.i<<",,"<<VB.k<<", delay="<<delay<<",
...
Transformation LinearPrediction::Interpolate(std::shared_ptr<Unit> un, double deltatime) const
{
    ...
    if (deltatime > this->deltatime || this->deltatime == 0) {
        ...
        // cerr << "Using OLD Pos "<<un->GetSerial()<<": A2=("<<A2.i<<",,"<<A2.k<<"), delay="<<(deltatime/this->deltatime)<<",
        // A0=("<<A0.i<<",,"<<A0.k<<")"<<endl;
