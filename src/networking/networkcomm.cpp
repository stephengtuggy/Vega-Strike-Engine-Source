...
void NetworkCommunication::SendMessage(SOCKETALT &send_sock, ObjSerial serial, string message)
{
    ...
    if (method == ServerUnicast) {
        ...
        if (this->secured && use_secured) {
            ...
            p.send(
                CMD_TXTMESSAGE, serial, encrypted.c_str(), message.length(), SENDRELIABLE, NULL, send_sock, __FILE__, PSEUDO__LINE__(244));
        } else
#endif
        {
            p.send(CMD_SECMESSAGE, serial, message.c_str(), message.length(), SENDRELIABLE, NULL, send_sock, __FILE__, PSEUDO__LINE__(244));
        }
