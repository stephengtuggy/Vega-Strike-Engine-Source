...
class NetServer
{
  ...
  public:
    ...
    // void	sendDamages( ObjSerial serial, int zone, string shields, float recharge, char leak, unsigned short ab, unsigned short af,
    // unsigned short al, unsigned short ar, float ppercentage, float spercentage, float amt, Vector & pnt, Vector & normal, GFXColor &
    // color);

    bool saveAccount(int cpnum);

    void sendCustom(int cp, const string &cmd, const string &args, const string &id);
    void sendDamages(ObjSerial      serial,
                     unsigned short zone,
                     float          hull,
