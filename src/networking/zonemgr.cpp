...
void displayUnitInfo(std::shared_ptr<Unit> un, const string callsign, const char *type)
{
    ...
    if (!un) {
        ...
        if (fg)
            cout << ", \'" << fg->name << "\' " << un->getFgSubnumber();
...
void ZoneMgr::broadcast(ClientPtr fromcltw, Packet *pckt, bool isTcp, unsigned short minver, unsigned short maxver)
{
    ...
    for (LI i = lst->begin(); i != lst->end(); i++) {
        ...
        if ((isTcp || clt->ingame) && clt->netversion >= minver && clt->netversion <= maxver &&
            ...
            COUT << "BROADCASTING " << pckt->getCommand() << " to client #";
            if (un2)
                cout << un2->GetSerial();
            else
                cout << "dead";
...
void ZoneMgr::broadcastSample(int zone, ObjSerial serial, Packet *pckt, float frequency)
{
    ...
    for (LI i = lst->begin(); i != lst->end(); i++) {
        ...
        if (clt->ingame && clt->comm_freq != -1 && clt->portaudio && clt->comm_freq == frequency && ((!un) || (un->GetSerial() != serial)))
            pckt->bc_send(clt->cltadr, clt->tcp_sock);
    }
}
...
void ZoneMgr::broadcastSnapshots(bool update_planets)
{
    ...
    for (ZoneMap::iterator iter = zones.begin(); iter != zones.end(); ++iter) {
        ...
        if (true) {
            ...
            // COUT << "BROADCAST SNAPSHOTS = "<<zone_clients[i]<<" clients in zone "<<i<<" time now: "<<queryTime()<<"; frame time:
            // "<<getNewTime() << endl; Loop for all the zone's clients
                ...
                while (*iter) {
                    ...
                    for (; (unit = *iter) != NULL; ++iter) {
                        ...
                        if (unit->GetSerial() != 0 && unit != cltk->game_unit.GetUnit()) {
                            ...
                            if ((unit->isUnit() == UNITPTR || unit->isUnit() == ASTEROIDPTR || unit->isUnit() == MISSILEPTR) ||
