...
NetClient::~NetClient()
{
    ...
    if (clt_tcp_sock)
        delete clt_tcp_sock; // UDP sockets don't seem to like being deleted.
...
void        NetClient::Respawn(ObjSerial newserial)
{
    ...
    cp->savegame->ParseSaveGame("", mysystem, "", pos, setplayerXloc, cp->credits, packedInfo, whichcp, lastsave[0], false);
...
int         NetClient::recvMsg(Packet* outpacket, timeval* timeout)
{
    ...
    if (true) {
        ...
        case CMD_EXITCLIENT:
            if (nostarsystem)
                break;
            COUT << ">>> " << local_serial << " >>> EXITING CLIENT =( serial #" << packet_serial
                 << " )= --------------------------------------" << endl;
        ...
        case CMD_ADDEDYOU:
            ...
            if (nostarsystem)
                break;
            COUT << ">>> " << local_serial << " >>> ADDED IN GAME =( serial #" << packet_serial
                 << " )= --------------------------------------" << endl;
        ...
        case CMD_DISCONNECT:
            ...
            COUT << ">>> " << local_serial << " >>> DISCONNECTED -> Client killed =( serial #" << packet_serial
                 << " )= --------------------------------------" << endl;
        ...
        case CMD_FIREREQUEST: {
            ...
            if (un != NULL) {
                ...
                vector<Mount>::iterator i = un->mounts.begin(); // note to self: if vector<Mount *> is ever changed to vector<Mount> remove
                                                                // the const_ from the const_iterator
        ...
        case CMD_SNAPDAMAGE: {
            ...
            for (unsigned int i = 0; i < nbupdates; i++) {
                ...
                if (noffset == offset) {
                    COUT << "ERROR Premature end of Snapshot buffer " << std::hex << std::string(netbuf.getData(), netbuf.getSize())
                         << std::dec << std::endl;
                    break;
                }
                offset   = noffset;
...
Transformation NetClient::Interpolate(Unit* un, double addtime)
{
    ...
    if (clt) {
        ...
        // cerr << "  *** interpolate "<<un->getFullname()<<","<<un->GetSerial()<<" (" << trans.position.i << ", " << trans.position.j << ",
        // " << trans.position.k << ")"<<endl; cerr << "  *** INTERPOLATE (" << un->curr_physical_state.position.i << ", " <<
        // un->curr_physical_state.position.j << ", " << un->curr_physical_state.position.k << "): next deltatime=" <<
        // clt->getNextDeltatime() << ", deltatime=" << clt->getDeltatime() << ", this-deltatime=" << this->deltatime << ", elapsed since
        // packet=" << clt->elapsed_since_packet << "\n        =>        (" << trans.position.i << ", " << trans.position.j << ", " <<
        // trans.position.k << ")        Vel =    (" << un->Velocity.i << ", " << un->Velocity.j << ", " << un->Velocity.k << ")" <<
        // std::endl;
    } else {
