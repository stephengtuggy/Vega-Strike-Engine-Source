...
class Prediction
{
  ...
  public:
    ...
    virtual QVector        InterpolatePosition(std::shared_ptr<Unit> un, double deltatime) const    = 0;
    virtual Quaternion     InterpolateOrientation(std::shared_ptr<Unit> un, double deltatime) const = 0;
    virtual Transformation Interpolate(std::shared_ptr<Unit> un, double deltatime) const;
};
