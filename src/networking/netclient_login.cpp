...
int NetClient::authenticate()
{
    ...
    if (str_callsign.length() && str_passwd.length()) {
        ...
        packet2.send(
            CMD_LOGIN, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, *this->clt_tcp_sock, __FILE__, PSEUDO__LINE__(165));
        COUT << "Send login for player <" << str_callsign
             << ">:<*******"
                "> - buffer length : "
             << packet2.getDataLength() << " (+" << packet2.getHeaderLength() << " header len" << endl;
    } else {
...
int NetClient::loginAuth(string str_callsign, string str_passwd, string &error)
{
    ...
    packet2.send(
        CMD_LOGIN, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, *this->clt_tcp_sock, __FILE__, PSEUDO__LINE__(316));
...
vector<string> &NetClient::loginAcctLoop(string str_callsign, string str_passwd)
{
    ...
    if (lastsave.empty() || lastsave[0] != "") {
        ...
        COUT << "Trying to connect to game server..." << endl << "\tIP=" << _serverip << ":" << _serverport << endl;
    }
    return lastsave;
}
...
void NetClient::loginAccept(Packet &p1)
{
    ...
    cerr << "WE ARE ON STARDATE " << datestr << " - converted : " << _Universe->current_stardate.GetFullTrekDate() << endl;
...
void NetClient::respawnRequest()
{
    ...
    // No data.
    packet2.send(
        CMD_RESPAWN, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, *this->clt_tcp_sock, __FILE__, PSEUDO__LINE__(165));
}
...
void NetClient::SetConfigServerAddress(string &addr, unsigned short &port)
{
    ...
    if (use_acctserver) {
        ...
        this->_serverip = addr = vs_config->getVariable("network", "account_server_url", "http://localhost/cgi-bin/accountserver.py");
...
{
    COUT << " enter " << __PRETTY_FUNCTION__ << " with " << addr << endl;
...
{
    if (clt_tcp_sock && clt_tcp_sock->valid())
        clt_tcp_sock->disconnect("NC_init_tcp");
    if (clt_udp_sock && clt_udp_sock->valid())
    ...
    COUT << " enter " << __PRETTY_FUNCTION__ << " with " << addr << ":" << port << endl;
    ...
    *this->clt_tcp_sock = NetUITCP::createSocket(addr, port, _sock_set);
     ...
     */
    Packet join;
    join.send(CMD_CONNECT, CLIENT_NETVERSION, "", 0, SENDRELIABLE, NULL, *this->clt_tcp_sock, __FILE__, PSEUDO__LINE__(407));
    this->enabled = 1;
...
void NetClient::synchronizeTime(SOCKETALT *udpsock)
{
    ...
    while (i < NUM_TIMES && timeout < UDP_TIMEOUT) {
        ...
        packet.send(CMD_SERVERTIME,
                    0,
                    outData.getData(),
                    outData.getDataLength(), // No data.
                    SENDRELIABLE,
                    NULL,
                    *this->clt_tcp_sock,
                    __FILE__,
...
bool NetClient::selectShip(unsigned int ship)
{
    if (lastsave.empty() || lastsave[0] == "") {
        ...
        Packet p;
        p.send(
            CMD_CHOOSESHIP, 0, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE, NULL, *clt_tcp_sock, __FILE__, PSEUDO__LINE__(628));
        string err;
        int    ret = loginLoop(err);
        if (ret != 1 || lastsave.size() < 2 || lastsave[0] == "") {
            cout << "Error in CHOOSEHIP: " << err << "choice=" << ship << "(" << shipname << "), max=" << ret << endl;
            return false;
        }
    }
    return true;
