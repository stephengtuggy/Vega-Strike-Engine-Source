...
AccountServer::AccountServer() : _sock_set(true)
{
    ...
    if ((recvcount = sock.recvbuf(&p, &ipadr)) > 0) {
        ...
        case CMD_NEWSUBSCRIBE: {
            ...
            if (!found) {
                ...
                if (packet2.send(packet.getCommand(), packet.getSerial(), (char *)NULL, 0, SENDRELIABLE, NULL, sock, __FILE__, __LINE__) <
                    0) {
                    cout << "ERROR sending authorization" << endl;
                    exit(1);
                }
...
void AccountServer::sendAuthorized(SOCKETALT sock, Account *acct)
{
    ...
    if (acct->isNew()) {
        ...
        Packet packet2;
        if (packet2.send(
                LOGIN_ACCEPT, serial, netbuf.getData(), netbuf.getDataLength(), SENDRELIABLE | COMPRESSED, NULL, sock, __FILE__, __LINE__) <
            0) {
            cout << "ERROR sending authorization" << endl;
            exit(1);
        }
