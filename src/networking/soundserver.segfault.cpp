    ...
    void Play(float fadeout, float fadein, Music &oldmusic)
    {
        ...
        if (fadeout) {
            ...
            if (!e_already_sent) {
                ...
                if (!e_already_sent) // race condition I know  I know--not much we can do bout it..besides this ole girl crashes when she
                                     // wishes.
                    oldmusic.Stop();
            }
...
void TakeLotsOfPriority()
{
    ...
    // Disable timesharing
    {
        ...
        error =
            thread_policy_set(mach_thread_self(), THREAD_EXTENDED_POLICY, (thread_policy_t)&extendedPolicy, THREAD_EXTENDED_POLICY_COUNT);
        ...
        error                       = thread_policy_set(
            mach_thread_self(), THREAD_PRECEDENCE_POLICY, (thread_policy_t)&precedencePolicy, THREAD_PRECEDENCE_POLICY_COUNT);
        if (error != KERN_SUCCESS) {
#if DEBUG
