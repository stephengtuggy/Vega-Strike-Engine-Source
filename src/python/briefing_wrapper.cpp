...
{
    return mission->briefing->AddStarship(name.c_str(), FactionUtil::GetFactionIndex(faction), vec);
}
...
voidWRAPPED1(removeShip, int, ship) voidWRAPPED3(enqueueOrder, int, ship, Vector, dest, float, time)
    voidWRAPPED3(replaceOrder, int, ship, Vector, dest, float, time) WRAPPED1(Vector, getShipPosition, int, ship, (50, 75, 100))
        voidWRAPPED2(setShipPosition, int, ship, Vector, vec) voidWRAPPED0(terminate) voidWRAPPED1(setCamPosition, Vector, pos)
            voidWRAPPED3(setCamOrientation, Vector, p, Vector, q, Vector, r) voidWRAPPED2(setCloak, int, ship, float, amount)
