...
extern std::shared_ptr<VegaConfig>createVegaConfig(const char *file);
...
/* Debug level is a character, 1, 2, or 3.   1 is most important warnings only. 2 is warnings that are important but not hazardous to
 * gameplay 3 is for developer type messages, more akin to informational output but not relevant to anything an end user would care about.
...
template <class A, class B, class C, class D, class E> int vs_fscanf(FILE *fp, const char *format, A *a, B *b, C *c, D *d, E *e)
{
 ...
 *                                                                                                               // Note : may only change
 * ONE level of directory or it breaks
 *  void vs_cdup();											// Goes up one level and changes the
 * direcrtory
 * accordingly void vs_mkdir (const std::string &nam);					// Creates a directory void vs_setdir (const char *
 * path);						// Sets the current directory
...
class VSFile
{
  ...
  public:
    ...
    template <class A, class B> int Fscanf(const char *format, A *a, B *b)
    {
        ...
        if (!UseVolumes[this->alt_type]) {
            ...
            if (q_volume_format == vfmtPK3) {
                ...
                delete[] newformat;
                std::cerr << "FSCANF : sscanf read " << readbytes << " bytes - OFFSET=" << offset << " VALUES : a=" << (*a)
                          << ", b=" << (*b) << std::endl;
                this->offset += readbytes;
                this->GoAfterEOL();
            } else if (q_volume_format == vfmtVSR) {
    ...
    template <class A, class B, class C, class D, class E, class F> int Fscanf(const char *format, A *a, B *b, C *c, D *d, E *e, F *f)
    {
        int ret = -1;
        if (!UseVolumes[this->alt_type]) {
