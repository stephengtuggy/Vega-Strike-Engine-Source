...
float GetStarSpreadScale() {
    return configuration().graphics.star_spreading_flt * configuration().physics.game_speed_flt;
}

float ScaleOrbitDist(bool autogenerated) {
    if (autogenerated) {
        return configuration().physics.autogen_compactness_flt * configuration().physics.runtime_compactness_flt;
    }
    return configuration().physics.runtime_compactness_flt;
}
...
void StarSystem::LoadXML(const string filename, const Vector &centroid, const float timeofyear) {
    ...
    if (configuration().physics.game_speed_affects_autogen_systems) {
        autogenerated = false;
    }
    ...
    xml->scale *= configuration().physics.star_system_scale_flt;
    ...
    xml->reflectivity = configuration().graphics.reflectivity_flt;
    xml->unitlevel = 0;
