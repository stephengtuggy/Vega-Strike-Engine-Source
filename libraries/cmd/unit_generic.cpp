...
bool Unit::InRange(const Unit *target, double &mm, bool cone, bool cap, bool lock) const {
    ...
    if (cone && radar.max_cone > -.98) {
        const QVector delta(target->Position() - Position());
        mm = delta.Magnitude();
        if ((!lock) || (!TargetLocked(target))) {
            double tempmm = mm - target->rSize();
            if (tempmm > 0.0001) {
                if ((ToLocalCoordinates(delta)).k / tempmm < radar.max_cone
                        && cone) {
                    return false;
                }
            }
    ...
    for (i = 0; i < utdw->pImage->dockedunits.size(); ++i) {
        if (utdw->pImage->dockedunits[i]->uc.GetUnit() == this) {
            ...
            if (launch_speed.get() > 0) {
                computer.set_speed = launch_speed.get();
            }
...
bool Unit::UpAndDownGrade(const Unit *up,
    ...
    if (downgrade) {
        ...
        //we are upgrading!
        if (touchme) {
            for (const auto & cargo : up->cargo) {
                if (CanAddCargo(cargo)) {
                    AddCargo(cargo, false);
                }
            }
        }
    }
...
int Unit::RepairUpgrade() {
    ...
    if (ComponentBasedUpgrades) {
        for (unsigned int i = 0; i < numCargo(); ++i) {
            if (GetCargo(i).GetCategory().find(DamagedCategory) == 0) {
                ++success;
                static size_t dam_len = strlen(DamagedCategory);
                GetCargo(i).SetCategory("upgrades/" + GetCargo(i).GetCategory().substr(dam_len));
            }
        }
    }
    return success;
    ...
    if (filtercargoprice) {
        ...
        if (cp) {
            for (auto & cargo_color : mylist) {
                if (cargo_color.cargo.GetPrice() > cp->credits) {
...
void Unit::Repair() {
        ...
        if (numcargo > 0) {
            ...
            if (carg->GetCategory().find("upgrades/") == 0
                ...
                if (next_repair_time == -FLT_MAX) {
                    next_repair_time =
                            UniverseUtil::GetGameTime() + repairtime.get() * (1 - percentoperational) / repair_bot.Get();
                } else {
    ...
    float ammt_repair = simulation_atom_var / repairtime.get() * repair_bot.Get();
    ...
    if (!mounts.empty()) {
        ...
        if (mounts[i].functionality < mounts[i].maxfunctionality) {
            mounts[i].functionality += mount_repair_quantity.get();
            if (mounts[i].functionality > mounts[i].maxfunctionality) {
...
void Unit::UpdatePhysics3(const Transformation &trans,
    ...
    if (EXTRA_CARGO_SPACE_DRAG.get() > 0) {
        ...
        if ((this->faction == upgfac) || (this->name == "eject") || (this->name == "Pilot")) {
            Velocity = Velocity * (1 - EXTRA_CARGO_SPACE_DRAG.get());
        }
    }
    ...
    if (SPACE_DRAG.get() > 0) {
        Velocity = Velocity * (1 - SPACE_DRAG.get());
    }
