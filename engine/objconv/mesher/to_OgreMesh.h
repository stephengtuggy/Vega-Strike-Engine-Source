...
void *Init(const char *inputfile,
        const char *matfile);     //creates an output context, and returns it - preloaded with the contents of inputfile, which must be an OgreMesh
void Add(void *outputcontext, const XML &memfile);     //adds input to the output context
void Dump(void *outputcontext,
        const char *outputfile,
        const char *materialfile);     //de-initializes the output context, and dumps its contents into outputfile (if it's not null)
void SetName(void *outputcontext,
        const char *name);     //sets the name of the mesh - actually, only used to create unique material names
void SetBasePath(void *outputcontext,
        const char *name);     //sets the name of the mesh - actually, only used to create unique material names
...
void AutoLOD(void *outputcontext,
        ...
        float refDistance);     //autogenerates LOD levels - if force==true, will discard current LOD data
