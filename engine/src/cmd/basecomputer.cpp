...
//These should probably be in a header file somewhere.
extern void SwitchUnits(UnitPtr ol, UnitPtr nw);
extern void TerminateCurrentBase(void);
extern void CurrentBaseUnitSet(UnitPtr un);
//For ships stats.
extern string MakeUnitXMLPretty(std::string, UnitPtr);
//For Options menu.
...
int basecargoassets(UnitPtr base, string cargoname);
...
//CONSTRUCTOR.
BaseComputer::BaseComputer(UnitPtr player, UnitPtr base, const std::vector<DisplayMode> &modes) :
...
void BaseComputer::recalcTitle() {
    ...
    //Base name.
    UnitPtr baseUnit = m_base.GetUnit();
    string baseName;
    if (baseUnit) {
    ...
    switch (m_currentDisplay) {
        ...
        case UPGRADE:
            //Fall through.
        case CARGO: {
            UnitPtr playerUnit = m_player.GetUnit();
            if (playerUnit) {
                const float emptyVolume = m_currentDisplay
                        == CARGO ? playerUnit->getEmptyCargoVolume()
                        : playerUnit->getEmptyUpgradeVolume();
...
bool BaseComputer::scrollToItem(Picker *picker, const Cargo &item, bool select, bool skipFirstCategory) {
    ...
    assert(cells != NULL);
    PickerCell *cell = cells->cellWithId(item.GetContent());
    picker->setMustRecalc();
    if (!cell) {
        //Item is not here.
        ...
        int i = 0;
        for (; i < count; i++) {
            if (item.GetContent() < cells->cellAt(i)->id()) {
                break;
            }
        }
        if (i == count) {
...
bool BaseComputer::configureUpgradeCommitControls(const Cargo &item, TransactionType trans) {
    ...
    if (trans == BUY_UPGRADE) {
            ...
            if (!isWeapon(item.GetCategory())) {
                ...
                for (unsigned int i = 0; i < numc; ++i) {
                    ...
                    if (c->GetCategory().find("upgrades/") == 0 && !isWeapon(c->GetCategory())) {
                        float po = UnitUtil::PercentOperational(player, c->GetContent(), c->GetCategory(), false);
                        if (po > .02 && po < .98) {
        ...
        bool unhidden = true;
        if (m_player.GetUnit()
                && UnitUtil::PercentOperational(m_player.GetUnit(), item.GetContent(), item.GetCategory(), false) < 1) {
            if (m_base.GetUnit()) {
                if (RepairPrice(UnitUtil::PercentOperational(m_player.GetUnit(),
                                item.GetContent(), item.GetCategory(), false),
                        m_base.GetUnit()->PriceCargo(item.GetContent()))
        ...
        switch (tlist->transaction) {
            case BUY_CARGO:
                ...
                if (item.GetCategory().find("My_Fleet") != string::npos) {
                    ...
                    tempString = (boost::format("Price: #b#%1$.2f#-b#n#")
                            % baseUnit->PriceCargo(item.GetContent())).str();
                ...
                break;
            case BUY_UPGRADE:
                if (item.GetContent() == BASIC_REPAIR_NAME) {
                    ...
                    tempString = (boost::format("Price: #b#%1$.2f#-b#n1.5#") % baseUnit->PriceCargo(item.GetContent()))
                            .str();
                }
                descString += tempString;
                if (item.GetDescription() == "" || item.GetDescription()[0] != '#') {
            ...
            case BUY_SHIP:
                ...
                if (item.GetCategory().find("My_Fleet") != string::npos) {
                    ...
                    PRETTY_ADDN("", baseUnit->PriceCargo(item.GetContent()), 2);
            ...
            case SELL_CARGO:
                ...
                if (item.mission) {
                    ...
                    tempString = (boost::format("Value: #b#%1$.2f#-b, purchased for %2$.2f#n#")
                            % baseUnit->PriceCargo(item.GetContent())
                            % item.price)
                            .str();
                }
                descString += tempString;
                ...
                descString += tempString;
                if (!item.mission) {
                    tailString = buildCargoDescription(item, *this, baseUnit->PriceCargo(item.GetContent()));
                }
                break;
            case SELL_UPGRADE:
                ...
                //********************************************************************************************
            {
                double percent_working = m_player.GetUnit() ? UnitUtil::PercentOperational(
                        m_player.GetUnit(), item.GetContent(), item.GetCategory(), false) : 0.0;
                if (percent_working < 1) {
                    //IF DAMAGED
                    tempString = (boost::format("Damaged and Used value: #b#%1$.2f#-b, purchased for %2$.2f#n1.5#")
                            % SellPrice(percent_working, baseUnit->PriceCargo(item.GetContent()))
                            % item.price)
                            .str();
                    descString += tempString;
                    ...
                    tempString = (boost::format("Percent Working: #b#%1$.2f#-b, Repair Cost: %2$.2f#n1.5#")
                            % (percent_working * 100)
                            % RepairPrice(percent_working, baseUnit->PriceCargo(item.GetContent())))
                            .str();
                    descString += tempString;
                } else {
                    tempString = (boost::format("Used value: #b#%1$.2f#-b, purchased for %2$.2f#n1.5#")
                            % usedValue(baseUnit->PriceCargo(item.GetContent())) % item.price).str();
                    descString += tempString;
                }
                if (damaged_mode) {
                    descString +=
...
bool UpgradeAllowed(const Cargo &item, UnitPtr playerUnit) {
    ...
    while (prohibited_upgrades.length()) {
        ...
        if (item.GetContent() == content || (0 == item.GetCategory().find(content))) {
            if (quantity == 0) {
                color_prohibited_upgrade_flag = true;
                return false;
            }
            unsigned int i = 0;
            Cargo *numUpgrades = playerUnit->GetCargo(item.GetContent(), i);
            if (numUpgrades) {
            ...
            for (i = 0; i < limit; ++i) {
                ...
                if (numUpgrades && (0 == numUpgrades->GetCategory().find(content))) {
...
void BaseComputer::loadListPicker(TransactionList &tlist,
    ...
    for (size_t i = 0; i < tlist.masterList.size(); i++) {
        ...
        string itemName = beautify(UniverseUtil::LookupUnitStat(item.GetContent(), "upgrades", "Name"));
        string originalName = itemName;
        if (itemName.empty()) {
            itemName = beautify(item.GetContent());
        }
        if (item.quantity > 1) {
...
//*******************************************************************************
        ...
        if (transType == SELL_UPGRADE && m_player.GetUnit()) {
            ...
            double percent_working =
                    UnitUtil::PercentOperational(m_player.GetUnit(), item.GetContent(), item.GetCategory(), false);
        ...
        SimplePickerCell *cell = new SimplePickerCell(itemName, item.GetContent(), final_color, i);
...
extern int SelectDockPort(UnitPtr utdw, UnitPtr parent);
...
//if no docking port AND no permission, no ships (different from above for VS compatibility)
    ...
    loadMasterList(m_player.GetUnit(),
            vector<string>(),
            donttakethis,
            true,
...
void BaseComputer::loadMasterList(UnitPtr un,
...
static void eliminateZeroCargo(UnitPtr un) {
...
bool BaseComputer::buySelectedCargo(int requestedQuantity) {
    ...
    if (item) {
        ...
        playerUnit->BuyCargo(item->GetContent(), quantity, baseUnit, _Universe->AccessCockpit()->credits);
...
bool BaseComputer::sellSelectedCargo(int requestedQuantity) {
    ...
    if (item) {
        ...
        if (item->mission) {
            ...
            playerUnit->SellCargo(item->GetContent(), quantity, _Universe->AccessCockpit()->credits, sold, baseUnit);
        }
        eliminateZeroCargo(playerUnit);
        //Reload the UI -- inventory has changed.  Because we reload the UI, we need to
        //find, select, and scroll to the thing we bought.  The item might be gone from the
...
void BaseComputer::loadMissionsMasterList(TransactionList &tlist) {
    ...
    UnitPtr unit = _Universe->AccessCockpit()->GetParent();
    ...
    for (size_t i = 0; i < stringCount; i++) {
        ...
        if (lastCategorySep != string::npos) {
            //We have a category.
            c.cargo.SetContent(originalName.substr(lastCategorySep + 1));
            c.cargo.SetCategory(originalName.substr(0, lastCategorySep));
        } else {
            //No category in name.
            c.cargo.SetContent(originalName);
            c.cargo.SetCategory("");
        }
        c.cargo.SetContent(c.cargo.GetContent());
        //Description gets name at the top.
        c.cargo.SetDescription("#b#" + beautify(c.cargo.GetContent()) + ":#-b#n1.75#"
    ...
    if (!active_missions.empty()) {
        for (unsigned int i = 1; i < active_missions.size(); ++i) {
            CargoColor amission;
            amission.cargo.SetContent(XMLSupport::tostring(i) + " " + active_missions[i]->mission_name);
            ...
            for (auto & objective : active_missions[i]->objectives) {
...
void BaseComputer::loadBuyUpgradeControls(void) {
    ...
    //Add Basic Repair.
    CargoColor repair;
    repair.cargo.SetContent(BASIC_REPAIR_NAME);
...
void BaseComputer::loadSellUpgradeControls() {
    ...
    UnitPtr partListUnit{&GetUnitMasterPartList()};
    ...
    if (clearDowngrades) {
        ...
        for (unsigned int i = 0; i < tlist.masterList.size(); ++i) {
            if (downgradeMap.find(tlist.masterList[i].cargo.GetContent()) == downgradeMap.end()) {
...
static void BasicRepair(UnitPtr parent) {
    if (parent) {
...
protected:
    ...
    const UnitPtr m_downgradeLimiter;
};
...
void BaseComputer::BuyUpgradeOperation::start() {
    ...
    Cargo *part = baseUnit->GetCargo(m_selectedItem.GetContent(), offset);     //Whether the base has any of these.
...
bool BaseComputer::BuyUpgradeOperation::checkTransaction() {
    UnitPtr playerUnit = m_parent.m_player.GetUnit();
    if (!playerUnit) {
        finish();
...
void BaseComputer::BuyUpgradeOperation::concludeTransaction() {
    UnitPtr playerUnit = m_parent.m_player.GetUnit();
    UnitPtr baseUnit = m_parent.m_base.GetUnit();
    if (!(playerUnit && baseUnit)) {
        finish();
    ...
    double percent;
    int numleft = basecargoassets(baseUnit, m_part.GetContent());
    while (numleft > 0
        ...
        if (_Universe->AccessCockpit()->credits >= price) {
            ...
            unsigned int index;
            baseUnit->GetCargo(m_part.GetContent(), index);
        ...
        numleft = basecargoassets(baseUnit, m_part.GetContent());
    }
    updateUI();
...
int basecargoassets(UnitPtr baseUnit, string cargoname) {
    unsigned int dummy;
...
void BaseComputer::SellUpgradeOperation::selectMount(void) {
    ...
    for (int i = 0; i < playerUnit->getNumMounts(); i++) {
        ...
        if (playerUnit->mounts[i].status == Mount::ACTIVE || playerUnit->mounts[i].status == Mount::INACTIVE) {
            ...
            const UnitPtr partUnit =
                    UnitConstCache::getCachedConst(StringIntKey(m_part.GetContent(), FactionUtil::GetUpgradeFaction()));
            string ammoexp;
            ...
            } else if (matchCargoToWeapon(m_part.GetContent(), unitName)) {
                selectable = true;
                selectableCount++;
                mount = i;
            }
...
//Finish the transaction.
void BaseComputer::SellUpgradeOperation::concludeTransaction() {
    UnitPtr playerUnit = m_parent.m_player.GetUnit();
    UnitPtr baseUnit = m_parent.m_base.GetUnit();
    if (!(playerUnit && baseUnit)) {
        finish();
    ...
    const double price = m_part.price * usedValue(percent);
    //Adjust the money.
    _Universe->AccessCockpit()->credits += price;
    //Change the ship.
    if (playerUnit->Downgrade(m_newPart, m_selectedMount, m_selectedTurret, percent, m_downgradeLimiter)) {
        ...
        m_part.quantity = 1;
        m_part.price = baseUnit->PriceCargo(m_part.GetContent());
        ...
        if (!isWeapon(item->GetCategory())) {
            if (playerUnit) {
                ...
                if (baseUnit) {
                    ...
                    playerUnit->BuyCargo(item->GetContent(), quantity, baseUnit, _Universe->AccessCockpit()->credits);
                    playerUnit->Upgrade(item->GetContent(), 0, 0, true, false);
                    refresh();
                }
            }
            return true;
...
bool BaseComputer::fixUpgrade(const EventCommandId &command, Control *control) {
    ...
    if (baseUnit && playerUnit && item) {
        ...
        if (playerUnit->RepairUpgradeCargo(item, baseUnit, credits)) {
            if (UnitUtil::PercentOperational(playerUnit, item->GetContent(), "upgrades/", false) < 1.0) {
...
Cargo CreateCargoForOwnerStarship(const Cockpit *cockpit, const UnitPtr base, int i) {
    Cargo cargo;
    cargo.quantity = 1;
    cargo.volume = 1;
    cargo.price = 0;
    ...
    cargo.SetContent(cockpit->GetUnitFileName(i));
...
Cargo CreateCargoForOwnerStarshipName(const Cockpit *cockpit, const UnitPtr base, std::string name, int &index) {
...
void SwapInNewShipName(Cockpit *cockpit, UnitPtr base, const std::string &newFileName, int swappingShipsIndex) {
...
bool sellShip(UnitPtr baseUnit, UnitPtr playerUnit, std::string shipname, BaseComputer *bcomputer) {
    Cockpit *cockpit = _Universe->isPlayerStarship(playerUnit);
...
bool BaseComputer::sellShip(const EventCommandId &command, Control *control) {
    ...
    return ::sellShip(baseUnit, playerUnit, item->GetContent(), this);
}

bool buyShip(UnitPtr baseUnit,
        UnitPtr playerUnit,
        std::string content,
        bool myfleet,
...
bool BaseComputer::showPlayerInfo(const EventCommandId &command, Control *control) {
    ...
    for (; i < numFactions; i++) {
        UnitPtr currentplayer = UniverseUtil::getPlayerX(UniverseUtil::getCurrentPlayer());
        float relation = 0;
...
void showUnitStats(UnitPtr playerUnit, string &text, int subunitlevel, int mode, Cargo &item) {
            ...
            if (!mode) {
                ...
                switch (replacement_mode) {
                    ...
                    case 2:                 //multiplicative
                        PRETTY_ADDU(statcolor + "Increases " + dvol[index] + " Volume by #-c",
                                100.0 * (vol[index] - 1),
                                0,
                                "%");
                        break;
    ...
    switch (num_shields) {
        case 2:
            shield_strings = shield_two_strings;
            break;
        case 4:
            shield_strings = shield_four_strings;
            break;
        case 8:
            shield_strings = shield_eight_strings;
            break;
    }
                ...
                if ((!wi) || (wi->name == "")) {
                    ...
                    switch (wi->type) {
                        ...
                        case WEAPON_TYPE::PROJECTILE:                     //need ammo
                            if (wi->lock_time > 0) {
                                PRETTY_ADDU(statcolor + "   'Fire and Forget' lock time: #-c",
                                        wi->lock_time,
                                        0,
                                        "seconds");
                            } else {
