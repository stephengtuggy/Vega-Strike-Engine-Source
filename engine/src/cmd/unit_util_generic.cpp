...
static const string &getFgDirectiveCR(const UnitPtr my_unit);
...
bool hasDockingUnits(const UnitPtr my_unit) {
    if (!my_unit) {
        return false;
    }
...
int getPhysicsPriority(UnitPtr un) {
    ...
    for (unsigned int i = 0; i < np; ++i) {
        UnitPtr player = _Universe->AccessCockpit(i)->GetParent();
        if (player) {
        ...
        if (cam) {
            ...
            if (dist < cpdist) {
                cpdist = dist;
                UnitPtr parent = _Universe->AccessCockpit(i)->GetParent();
                float lowest_priority_time = SIM_QUEUE_SIZE * SIMULATION_ATOM;
                if (relpos.Dot(relvel) >= 0) {
                    //No need to be wary if they're getting away
                    tooclose =
...
void SetHull(UnitPtr my_unit, float newhull) {
    if (!my_unit) {
        return;
    }
...
UnitPtr getFlightgroupLeader(UnitPtr my_unit) {
    ...
    UnitPtr ret_unit = fg ? fg->leader.GetUnit() : my_unit;
    if (!ret_unit) {
        ret_unit = 0;
    }
    return ret_unit;
...
int getFgSubnumber(UnitPtr my_unit) {
    if (!my_unit) {
        return -1;
    }
...
void RecomputeUnitUpgrades(UnitPtr un) {
    if (un == NULL) {
        return;
    }
    un->ReduceToTemplate();
...
int removeWeapon(UnitPtr my_unit, string name, int mountoffset, bool loop) {
    if (!my_unit) {
        return -1;
    }
...
int hasCargo(const UnitPtr my_unit, string mycarg) {
    if (!my_unit) {
        return 0;
    }
    unsigned int i;
...
string getUnitSystemFile(const UnitPtr un) {
    if (!un) {
...
bool isDockableUnit(const UnitPtr my_unit) {
    ...
    return (
            ...
            && (!my_unit->DockingPortLocations().empty());
}
...
bool isSun(UnitPtr my_unit) {
    ...
    return my_unit->isPlanet() && (vega_dynamic_cast_ptr<Planet>(my_unit.get()))->hasLights();
}
...
void setSpeed(UnitPtr my_unit, float speed) {
    if (my_unit) {
        my_unit->GetComputerData().set_speed = speed;
    }
}

float maxSpeed(const UnitPtr my_unit) {
    if (!my_unit) {
        return 0;
    }
...
float maxAfterburnerSpeed(const UnitPtr my_unit) {
    if (!my_unit) {
        return 0;
    }
...
void setECM(UnitPtr my_unit, int NewECM) {
    //short fix
    if (!my_unit) {
        return;
    }
...
int getECM(const UnitPtr my_unit) {
    //short fix
    if (!my_unit) {
        return 0;
    }
...
static bool ishere(const UnitPtr par, const UnitPtr look) {
    const UnitPtr un;
...
void performDockingOperations(UnitPtr un, UnitPtr unitToDockWith, int actually_dock) {
...
float PercentOperational(UnitPtr un, std::string name, std::string category, bool countHullAndArmorAsFull) {
    if (!un) {
        return 0;
    }
    ...
    const UnitPtr upgrade = getUnitFromUpgradeName(name, un->faction);
    if (!upgrade) {
        return 1.0f;
    }
