...
VertexBuilder<> GetLockingIcon(const QVector &location, const Vector& cam_p,
    ...
    const float bracket_width = configuration().graphics.hud.rotating_bracket_width_flt;

    const float bounded_r_size = std::max(r_size, static_cast<float>(configuration().graphics.hud.min_lock_box_size_flt));
    const float inner_distance = configuration().graphics.hud.min_missile_bracket_size_flt + (configuration().graphics.hud.max_missile_bracket_size_flt - configuration().graphics.hud.min_missile_bracket_size_flt) * lock_percent;
    const float outer_distance = inner_distance + configuration().graphics.hud.rotating_bracket_size_flt;
...
VertexBuilder<> GetAnimatedLockingIcon(const QVector &location, const Vector& cam_p,
    ...
    const float lock_line = configuration().graphics.hud.lock_confirm_line_length_flt;
    const double diamond_size = configuration().graphics.hud.diamond_size_dbl;
    const float theta_speed = configuration().graphics.hud.diamond_rotation_speed_flt;

    const float max = diamond_size * r_size * 0.75 * configuration().graphics.hud.min_missile_bracket_size_flt;
    const float coord = configuration().graphics.hud.min_missile_bracket_size_flt + (configuration().graphics.hud.max_missile_bracket_size_flt - configuration().graphics.hud.min_missile_bracket_size_flt) * lock_percent;
    ...
    const double r_1_size = std::max(r_size * configuration().graphics.hud.rotating_bracket_size_dbl, configuration().graphics.hud.min_lock_box_size_dbl);
...
void SetThickness(ShapeType type) {
    switch(type) {
        case ShapeType::Box:
            GFXLineWidth(configuration().graphics.hud.box_line_thickness_flt);
            break;
        case ShapeType::Diamond:
            GFXLineWidth(configuration().graphics.hud.diamond_line_thickness_flt);
            break;
        case ShapeType::Cross:
            GFXLineWidth(configuration().graphics.hud.nav_cross_line_thickness_flt);
            break;
        default:
            GFXLineWidth(1.0F);
            break;
