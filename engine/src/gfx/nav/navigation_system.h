 ...
 */
#ifndef VEGA_STRIKE_ENGINE_GFX_NAV_NAVIGATION_SYSTEM_H
#define VEGA_STRIKE_ENGINE_GFX_NAV_NAVIGATION_SYSTEM_H
...
public:
    ...
    public:
        SystemIterator(std::string current_system, unsigned int max = 2);
        bool done() const;
        QVector Position();
        std::string operator*();
        SystemIterator &next();
...
//typedef std::pair<string, QVector> SystemInfo;
    ...
    public:
        ...
        CachedSystemIterator(std::string current_system, unsigned max_systems = 2);
        void init(std::string current_system, unsigned max_systems = 2);
...
#endif //VEGA_STRIKE_ENGINE_GFX_NAV_NAVIGATION_SYSTEM_H