...
double calc_blend_factor( double frac, unsigned int priority, unsigned int when_it_will_be_simulated, unsigned int cur_simulation_frame );
...
void StarSystem::Draw( bool DrawCockpit )
{
    ...
    double setupdrawtime = queryTime();
    {
        ...
        float backup = simulation_atom_var;
        //BOOST_LOG_TRIVIAL(trace) << boost::format("StarSystem::Draw(): simulation_atom_var as backed up  = %1%") % simulation_atom_var;
        unsigned int cur_sim_frame = _Universe->activeStarSystem()->getCurrentSimFrame();
        for (int i = 0; i < 2; ++i) {
        ...
        interpolation_blend_factor = saved_interpolation_blend_factor;
        simulation_atom_var = backup;
        //BOOST_LOG_TRIVIAL(trace) << boost::format("StarSystem::Draw(): simulation_atom_var as restored   = %1%") % simulation_atom_var;
...
void StarSystem::createBackground( Star_XML *xml )
{
    ...
    if ( light_map[0]->LoadSuccess() && light_map[0]->isCube() ) {
        ...
        delete light_map[0];
        light_map[0] = new Texture( (xml->backgroundname+"_right.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_POSITIVE_X,
                                  GFXFALSE, game_options.max_cubemap_size );
        light_map[1] = new Texture( (xml->backgroundname+"_left.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_NEGATIVE_X,
                                  GFXFALSE, game_options.max_cubemap_size, GFXFALSE, GFXFALSE, DEFAULT_ADDRESS_MODE,
                                  light_map[0] );
        light_map[2] = new Texture( (xml->backgroundname+"_up.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_POSITIVE_Y,
                                  GFXFALSE, game_options.max_cubemap_size, GFXFALSE, GFXFALSE, DEFAULT_ADDRESS_MODE,
                                  light_map[0] );
        light_map[3] = new Texture( (xml->backgroundname+"_down.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_NEGATIVE_Y,
                                  GFXFALSE, game_options.max_cubemap_size, GFXFALSE, GFXFALSE, DEFAULT_ADDRESS_MODE,
                                  light_map[0] );
        light_map[4] = new Texture( (xml->backgroundname+"_front.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_POSITIVE_Z,
                                  GFXFALSE, game_options.max_cubemap_size, GFXFALSE, GFXFALSE, DEFAULT_ADDRESS_MODE,
                                  light_map[0] );
        light_map[5] = new Texture( (xml->backgroundname+"_back.image").c_str(), 1, TRILINEAR, CUBEMAP, CUBEMAP_NEGATIVE_Z,
                                  GFXFALSE, game_options.max_cubemap_size, GFXFALSE, GFXFALSE, DEFAULT_ADDRESS_MODE,
                                  light_map[0] );
    }
#else
            ...
            try {
                ...
                for (; (unit = *iter); ++iter) {
                    ...
                    if (priority != predprior) {
                        ...
                        BOOST_LOG_TRIVIAL(trace) << "void StarSystem::UpdateUnitPhysics( bool firstframe ): Randomizing unit priority";
...
void StarSystem::Update( float priority, bool executeDirector )
{
        ...
        for (unsigned int j = 0; j < _Universe->numPlayers(); ++j)
            if (_Universe->AccessCockpit( j )->activeStarSystem == this) {
                ...
                if ( _Universe->AccessCockpit( j )->Update() ) {
                    ...
                    //BOOST_LOG_TRIVIAL(trace) << boost::format("void StarSystem::Update( float priority, bool executeDirector ): Msg C: simulation_atom_var as restored   = %1%") % simulation_atom_var;
    ...
    simulation_atom_var = normal_simulation_atom;
    //BOOST_LOG_TRIVIAL(trace) << boost::format("void StarSystem::Update( float priority, bool executeDirector ): Msg D: simulation_atom_var as restored   = %1%") % simulation_atom_var;
    //WARNING cockpit does not get here...
    _Universe->popActiveStarSystem();
}
...
void StarSystem::UpdateMissiles()
{
    ...
    if ( !discharged_missiles.empty() ) {
        if ( discharged_missiles.back()->GetRadius() > 0 ) {           //we can avoid this iterated check for kinetic projectiles even if they "discharge" on hit
            Unit *un;
