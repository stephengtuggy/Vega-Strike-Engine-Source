...
void WriteUnit(
    string tag, string name, string filename, Vector r, Vector s, Vector center, string nebfile, string destination, bool faction)
{
    Tab();
...
void MakePlanet(float radius, int entitytype, bool forceRS, Vector R, Vector S, Vector center)
{
    ...
    Tab();
    fprintf(fp,
            "<Planet name=\"%s\" file=\"%s\" ",
            entitytype == JUMP ? s.c_str() : getRandName(names).c_str(),
            entitytype == JUMP ? "jump.png" : s.c_str());
    ...
    if ((entitytype != JUMP && entitytype != MOON) || grand() < moonofmoonprob) {
        int numu = numun[1] / (nument[PLANET] + nument[GAS]) + grand() * (nument[PLANET] + nument[GAS]) >
                           (numun[1] % (nument[PLANET] + nument[GAS]))
                       ? 1
                       : 0;
        if (entitytype == MOON)
            if (numu > 1)
        ...
        for (unsigned int i = 0; i < numu; i++)
            MakeSmallUnit();
        MakeMoons((entitytype != JUMP && entitytype != MOON) ? .4 * radius : .8 * radius,
                  MOON,
                  entitytype,
...
void MakeMoons(float radius, int entitytype, int callingentitytype, bool forceone)
{
    unsigned int nummoon = nument[entitytype] / nument[callingentitytype] +
                                   (grand() * nument[callingentitytype] < (nument[entitytype] % nument[callingentitytype]))
                               ? 1
                               : 0;
    if (forceone)
        nummoon = 1;
...
void CreateStarSystem()
{
    ...
    assert(starradius[0]);
    fprintf(fp,
            "<system name=\"%s\" background=\"%s\" nearstars=\"%d\" stars=\"%d\" starspread=\"150\">\n",
            systemname.c_str(),
...
int main(int argc, char **argv)
{
    if (argc < 9) {
        fprintf(stderr,
                "Usage: starsysgen <seed> <sector>/<system> <sunradius> <numstars> <numgasgiants> <numrockyplanets> <nummoons> "
