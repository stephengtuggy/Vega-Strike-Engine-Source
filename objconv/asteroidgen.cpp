...
float scale        = 1;
void  determine_centers_and_radii(
     vector<asteroid> &field, const Vector &cube_sides, const float radiusmin, const float radiusmax, const int poly_min, const int poly_max)
{
    for (unsigned int i = 0; i < field.size(); i++) {
        ...
        if (field[i].center.Mag() < safety_zone) {
            ...
            for (unsigned int j = 0; j < i; j++)
                if ((fabs(field[j].center.i - field[i].center.i) < 1.2 * (field[j].radius + field[i].radius) &&
    ...
    for (unsigned int i = 0; i < stacks + 2; i++) {
        ...
        for (unsigned int j = 0; j < slices; j++) {
            ...
            if (i != 0 && i != 1 && i != stacks + 1) {
                ...
                p.push_back(Tri(0, 1 + j, 1 + ((j + 1) % slices)));
            } else if (i == stacks + 1) {
                p.push_back(Tri(1 + (i - 2) * slices + j, 1 + (i - 1) * slices, 1 + (i - 2) * slices + ((j + 1) % slices)));
            }
            // do bottom pyr
        }
    }
