...
static void
ObjToXML(XML &xml, const vector<VTX> &vtxlist, const vector<TEX> &txclist, const vector<NORMAL> &normallist, const vector<FACE> &facelist)
{
    map<IntRef, int> elements;
    for (vector<FACE>::const_iterator fiter = facelist.begin(); fiter != facelist.end(); fiter++) {
        ...
        case 3:
            // Triangle...
            xml.tris.push_back(triangle(e1,
                                        e2,
                                        e3,
                                        xml.vertices[e1].s,
                                        xml.vertices[e1].t,
...
void ObjToXMESH(FILE *obj, FILE *mtl, vector<XML> &xmllist, bool forcenormals)
{
    ...
    while (fgets(buf, osize, obj)) {
        ...
        if (rv >= 2) {
            ...
            case 5:
                // fan - decomposed into tris. Sorry, but after fanification-stripification is done, everything will be allright again.
                {
                    ...
                    for (vector<string>::size_type i = 2; i < splitwhite.size(); i++)
                        facelist.push_back(
                            map_face(FACE(parsePoly(splitwhite[0]), parsePoly(splitwhite[i - 1]), parsePoly(splitwhite[i - 2])),
                                     vtxmap_ii,
                                     txcmap_ii,
                                     normalmap_ii));
                    break;
            ...
            case 4:
                facelist.push_back(
                    map_face(FACE(parsePoly(str), parsePoly(str1), parsePoly(str2), parsePoly(str3)), vtxmap_ii, txcmap_ii, normalmap_ii));
                break;
            case 3:
                facelist.push_back(map_face(FACE(parsePoly(str), parsePoly(str1), parsePoly(str2)), vtxmap_ii, txcmap_ii, normalmap_ii));
                break;
            case 2:
