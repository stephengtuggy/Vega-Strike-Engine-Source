...
// TODO: Make it process #{ifdef()}, #{else} and #{endif} tags, and make it replace $$ with $, and ## with # (and ignore the resulting
// character, to allow output of tags themselves).
static void replaceVars(string &text, map<string, string> variables)
{
    ...
    while ((where = text.find("$(", ip)) != string::npos) {
        if ((when = text.find(')', where + 2)) != string::npos) {
            ...
            string var =
                (whip != string::npos && whip < when) ? text.substr(where + 2, whip - where - 2) : text.substr(where + 2, when - where - 2);
...
void *Init()
{
    ...
    ctxt->top = Ogre::MeshManager::getSingleton().createManual(ctxt->name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
...
static const map<string, string> &LoadTemplates()
{
    ...
    for (unsigned int i = 0; i < sizeof(tpl_fnames) / sizeof(tpl_fnames[0]); i++)
        tpl.insert(
            pair<string, string>(string(tpl_fnames[i]), LoadString(templatePath + "/" + string(tpl_fnames[i]) + ".materialTemplate")));
    return tpl;
}
...
static void AddMesh(void *outputcontext, const XML &xml, const string &matname, bool texcoord, bool normals)
{
    ...
    if (xml.quads.size()) {
        ...
        for (size_t i = 0; i < xml.quads.size(); ++i) {
            unsigned int idx[] = {idxbase + xml.quads[i].indexref[0],
...
static void AutoOrganiseBuffers(Ogre::VertexData *data, Ogre::MeshPtr mesh)
{
    ...
    Ogre::VertexDeclaration *newDcl =
        data->vertexDeclaration->getAutoOrganisedDeclaration(mesh->hasSkeleton(), mesh->hasVertexAnimation() || (mesh->getPoseCount() > 0));
#endif
...
void DoneMeshes(void *outputcontext)
{
    ...
    if (ctxt->sharedvert.size())
        ctxt->top->sharedVertexData =
            CreateVertexBuffers(ctxt->sharedvert, ctxt->sv_usetex, ctxt->sv_usenorm, ctxt->min, ctxt->max, ctxt->maxsqr);
    // Set bounds
...
void Dump(void *outputcontext, const char *outputfile, const char *materialfile)
{
    ...
    if (!fo.fail()) {
        for (map<string, pair<string, string>>::const_iterator it = ctxt->materials.begin(); it != ctxt->materials.end(); ++it)
