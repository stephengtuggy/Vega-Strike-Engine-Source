...
void BFXMToXmeshOrOBJ(FILE *Inputfile, FILE *Outputfile, FILE *OutputObj, FILE *mtl, std::string meshname, char hackfix)
{
    ...
    if (hackfix == 'o') {
        ...
        fprintf(stderr,
                "Something is horribly wrong. Procedure is being called, but output type is neither OBJ nor XMESH. Please contact the "
                "developers\n");
        exit(-1);
    }
    // CHECK MAGIC WORD
    ...
    for (uint32bit recordindex = 0; recordindex < numrecords; recordindex++) {
        ...
        for (uint32bit meshindex = 0; meshindex < nummeshes; meshindex++) {
            ...
            if (isxmesh) {
                fprintf(Outputfile,
                        "<Mesh scale=\"%f\" reverse=\"%d\" forcetexture=\"%d\" sharevert=\"%d\" polygonoffset=\"%f\" blend=\"%s %s\" "
                        "alphatest=\"%f\" ",
                        scale,
                        reverse,
                        forcetexture,
            ...
            for (uint32bit tex = 0; tex < numtextures; tex++) {
                ...
                case TECHNIQUE:
                    ...
                    break;
                default:
                    fprintf(stderr, "Bad case in switch(textype) in file from_BFXM.cpp: type=%d index=%d\n", textype, texindex);
                    if (isxmesh)
