...
const EnumMap::Pair XML::element_names[] = {EnumMap::Pair("UNKNOWN", XML::UNKNOWN),
...
const EnumMap::Pair XML::attribute_names[] = {EnumMap::Pair("UNKNOWN", XML::UNKNOWN),
    ...
    case XML::VERTEX: {
        ...
        case LINE:
            ...
            if (xml->curpolyindex == 1) {
                float x   = xml->vertices[xml->linetemp.indexref[1]].x - xml->vertices[xml->linetemp.indexref[0]].x;
                float y   = xml->vertices[xml->linetemp.indexref[1]].y - xml->vertices[xml->linetemp.indexref[0]].y;
                float z   = xml->vertices[xml->linetemp.indexref[1]].z - xml->vertices[xml->linetemp.indexref[0]].z;
        ...
        case TRIANGLE:
            ...
            if (xml->curpolyindex == 2) {
                ...
                AddNormal(xml->vertices[xml->triangletemp.indexref[0]], temp);
                AddNormal(xml->vertices[xml->triangletemp.indexref[1]], temp);
                AddNormal(xml->vertices[xml->triangletemp.indexref[2]], temp);
            }
            break;
        case QUAD:
            ...
            if (xml->curpolyindex == 3) {
                ...
                AddNormal(xml->vertices[xml->quadtemp.indexref[0]], temp);
                AddNormal(xml->vertices[xml->quadtemp.indexref[1]], temp);
                AddNormal(xml->vertices[xml->quadtemp.indexref[2]], temp);
                AddNormal(xml->vertices[xml->quadtemp.indexref[3]], temp);
            }
            break;
        case LINESTRIP:
        case TRISTRIP:
        ...
        case QUADSTRIP:
            ...
            if (xml->striptemp.points.size() > 2 && (xml->curpolytype != QUADSTRIP || xml->striptemp.points.size() % 2 == 0)) {
                GFXVertex temp;
                bool      rev = ((xml->striptemp.points.size() % 2 == 0) && xml->curpolytype == TRISTRIP) || xml->curpolytype == QUADSTRIP;
                SetNormal(temp,
                ...
                AddNormal(xml->vertices[xml->striptemp.points.back().indexref], temp);
                AddNormal(xml->vertices[xml->striptemp.points[xml->striptemp.points.size() - 2].indexref], temp);
                AddNormal(xml->vertices[xml->striptemp.points[xml->striptemp.points.size() - 3].indexref], temp);
                if (xml->curpolytype == QUADSTRIP)
                    AddNormal(xml->vertices[xml->striptemp.points[xml->striptemp.points.size() - 4].indexref], temp);
            }
            break;
        }
        xml->curpolyindex += 1;
...
uint32bit writesuperheader(XML memfile, FILE *Outputfile)
{
    ...
    runningbytenum += (uint32bit)fwrite(&bytebuf, 1, 1, Outputfile);
    runningbytenum +=
        sizeof(uint32bit) * (uint32bit)fwrite(&versionnumber, sizeof(uint32bit), 1, Outputfile); // VERSION number for BinaryFormattedXMesh
...
uint32bit appendrecordfromxml(XML memfile, FILE *Outputfile, bool forcenormals)
{
    ...
    intbuf = VSSwapHostIntToLittle(
        (uint32bit)(1 + memfile.LODs.size() + memfile.animframes.size())); // Number of meshes = 1 + numLODs + numAnims.
...
uint32bit appendmeshfromxml(XML memfile, FILE *Outputfile, bool forcenormals)
{
    ...
    runningbytenum += sizeof(uint32bit) *
                      (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Length of Variable sized attribute section in bytes
    // Detail texture
    {
        uint32bit namelen = (uint32bit)memfile.detailtexture.name.size();
        intbuf            = VSSwapHostIntToLittle(namelen);
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Length of name of detail texture
        uint32bit nametmp;
    ...
    // Textures
    {
        ...
        for (texnum = 0; texnum < memfile.textures.size(); texnum++) {
            ...
            intbuf            = VSSwapHostIntToLittle(namelen);
            runningbytenum +=
                sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Length of name of texture # texnum
            uint32bit nametmp;
            for (nametmp = 0; nametmp < namelen; nametmp++) {
            ...
            for (nametmp = 0; nametmp < padlength; nametmp++) {
                bytebuf = 0;
                runningbytenum += (uint32bit)fwrite(&bytebuf, sizeof(uchar8bit), 1, Outputfile); // Padded so that next field is word
                                                                                                 // aligned
            }
        }
    }
    ...
    for (size_t logonum = 0; logonum < memfile.logos.size(); logonum++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // logo # logonum: number of references
        for (uint32bit ref = 0; ref < numrefs; ref++) {
            intbuf = VSSwapHostIntToLittle(memfile.logos[logonum].refpnt[ref]);
            runningbytenum +=
                sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // logo # logonum: reference # ref
            floatbuf = VSSwapHostFloatToLittle(memfile.logos[logonum].refweight[ref]);
            runningbytenum += sizeof(float32bit) *
                              (uint32bit)fwrite(&floatbuf, sizeof(float32bit), 1, Outputfile); // logo # logonum: reference # ref weight
        }
    }
    // LODs + Animations
    // LODs
    ...
    // Current VS File format is not compatible with new animation specification - can't test until I fix old files (only 1 at present uses
    // animations)
    ...
    for (size_t anim = 0; anim < memfile.animdefs.size(); anim++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // number of animation mesh offsets
    ...
    for (size_t verts = 0; verts < memfile.vertices.size(); verts++) {
        ...
        float normallen =
            sqrt(memfile.vertices[verts].i * memfile.vertices[verts].i + memfile.vertices[verts].j * memfile.vertices[verts].j +
    ...
    for (size_t ls = 0; ls < memfile.linestrips.size(); ls++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Number of elements in current linestrip
    ...
    for (size_t ts = 0; ts < memfile.tristrips.size(); ts++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Number of elements in current tristrip
    ...
    for (size_t tf = 0; tf < memfile.trifans.size(); tf++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Number of elements in current trifan
    ...
    for (size_t qs = 0; qs < memfile.quadstrips.size(); qs++) {
        ...
        runningbytenum +=
            sizeof(uint32bit) * (uint32bit)fwrite(&intbuf, sizeof(uint32bit), 1, Outputfile); // Number of elements in current quadstrip
