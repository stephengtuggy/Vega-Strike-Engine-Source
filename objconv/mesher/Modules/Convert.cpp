...
#define ADD_OPTION(name)                                                                                                                   \
    ...
    mOptions.insert(name)
#define ADD_ALIAS(alias, original)                                                                                                         \
    ADD_OPTION(alias);                                                                                                                     \
    mAliases.insert(pair<string, string>(alias, original))
            ...
            if (params.size() < 3) {
                ...
                if (phase == 0) {
                    ...
                    for (ConversionImplList::iterator cit = registry.begin();
                         (rc == ConversionImpl::RC_NOT_IMPLEMENTED) && (cit != registry.end());
                         ++cit) {
