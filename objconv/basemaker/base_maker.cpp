...
int TextPlane::Draw(const string &newText, int offset, bool startlower, bool force_highquality, bool automatte)
{
    ...
    GetPos(row, col);
    float rowheight =
        (use_bit) ? ((fnt == GLUT_BITMAP_HELVETICA_12) ? (26. / g_game.y_resolution) : (22. / g_game.y_resolution)) : (myFontMetrics.j);
    if (startlower)
        row -= rowheight;
    glLineWidth(1);
    ...
    while (text_it != newText.end() && row > myDims.j) {
        ...
        if (col + ((text_it + 1 != newText.end())
                       ? (use_bit ? (glutBitmapWidth(fnt, *text_it) / (float)(2 * g_game.x_resolution)) : myFontMetrics.i)
                       : 0) >=
                myDims.i ||
            *text_it == '\n') {
            GetPos(tmp, col);
      ...
      public:
        ...
        class BaseVSSprite : public BaseObj
        {
          public:
            ...
            BaseVSSprite(const char *spritefile, std::string ind) : BaseObj(ind), spr(spritefile, BILINEAR, GFXTRUE)
            {
            }
        };
...
void Base::Room::BaseVSSprite::EndXML(FILE *fp)
{
    ...
    fprintf(fp,
            "Base.Texture (room, '%s', 'bases/%s/%s'+time_of_day+'.spr', %g, %g)\n",
            index.c_str(),
            Base::CurrentBase->basefile.c_str(),
            noextname.c_str(),
            x,
            y);
}
...
bool AddRoomSprite(std::string input, unsigned int room_index, void *dat1, void *dat2, const void *dat3, float x, float y)
{
    ...
    if (input.empty()) {
        // add a ship.
        objs->push_back(
            new Base::Room::BaseShip(0, .34, .93, -1, 0, 0, 0, .93, -.34, QVector((x + tmp_xy.x) / 2, (tmp_xy.y + y) / 2, 4), "my_ship"));
    }
    // FIXME: orientation cannot be changed from editor.
        ...
        if (startdir.empty()) {
            ...
            for (int i = time ? 0 : 3; i < 4; ++i) {
                FILE *fp =
                    fopen(("sprites/bases/" + Base::CurrentBase->basefile + "/" + input + std::string(thetimes[i]) + ".spr").c_str(), "wt");
            ...
            Base::Room::BaseVSSprite *tmp = new Base::Room::BaseVSSprite(
                ("bases/" + Base::CurrentBase->basefile + "/" + input + thetimes[time ? 0 : 3] + ".spr").c_str(), string((char *)dat3));
...
bool LinkStage2(std::string input, unsigned int inputroomindex, void *dat1, void *dat2, const void *dat3, float x, float y)
{
    ...
    if (input == "link") {
         ...
         */
        Input("Which room should this go to?",
              &LinkStage3Goto,
              false,
              inputroomindex,
              &(((Base::Room::Goto *)(links->back()))->index),
              NULL,
              NULL,
              0,
              0);
...
void Base::GotoLink(int linknum)
{
    ...
    if (rooms.size() > linknum && linknum >= 0) {
        ...
        while (rooms.size() <= linknum) {
            ...
            Input("Input Name of Room (Shown at bottom of screen): ",
                  &SetRoomString,
                  false,
                  linknum,
                  &(rooms.back()->deftext),
...
Base::Room::Talk::Talk(unsigned int parind, std::string ind, std::string pythonfile) : Base::Room::Link(parind, ind, pythonfile)
{
    index = -1;
#ifndef BASE_MAKER
    gameMessage         last;
...
Base::Room::Python::Python(unsigned int parind, std::string ind, std::string pythonfile) : Base::Room::Link(parind, ind, pythonfile)
{
}

Base::Base(const char *basefile)
    : curtext(GFXColor(0, 1, 0, 1), GFXColor(0, 0, 0, 1)), othtext(GFXColor(1, 1, .5, 1), GFXColor(0, 0, 0, 1)), time_of_day(false)
{
    CurrentBase  = this;
    curlinkindex = 0;
    float x, y;
