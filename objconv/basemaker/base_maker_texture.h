...
{
  public:
    enum ColorMask { COLOR_MASK_PALETTE = 1, COLOR_MASK_COLOR = 2, COLOR_MASK_ALPHA = 4 };
    enum ColorType {
    ...
    /// Gets the data and information of the open file fp and returns the information into the pointer to the data struct.  Returns true if
    /// successful or else false.
    static bool getTextureData(FILE *fp, FileData *data, TextureTransform tt = defaultTransform);
    /// Gets the data and information of the filename and returns the information into the pointer to the data struct.  Returns true if
    /// successful or else prints an error and returns false.
  ...
  public:
    ...
    /// This calls the opengl bind function.  This does not do anything if handle == last_handle to save time and then sets last_handle to
    /// this handle
    void bind();
