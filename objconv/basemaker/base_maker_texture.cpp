...
static inline bool readPng(FILE *fp, Texture::FileData *data, Texture::TextureTransform tt)
{
     ...
     *               return;
     *       }*/
    // The previous check could only be false if the image was modified after being opened (such as on *nix systems).  In that case, worse
    // things will happen anyways.
    ...
    png_get_IHDR(
        png_ptr, info_ptr, (png_uint_32 *)&data->width, (png_uint_32 *)&data->height, &data->bpp, &ctype, &interlace_type, NULL, NULL);
...
{
    fprintf(stderr, "Bitmap files are not supported due to lack of an alpha channel.  Use PNG files instead to get better compression.\n");
    return false;
}
