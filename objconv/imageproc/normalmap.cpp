...
unsigned char *
ReadPNG(FILE *fp, unsigned int &sizeX, unsigned int &sizeY, int &img_depth, int &img_color_type, unsigned char ***row_pointer_ptr)
{
    png_structp png_ptr;
...
template <typename T> T *readimgA(T *data, int x, int y, int width)
{
    if (x < 0)
        printf("error\n");
    if (y < 0)
...
void ModifyImage(unsigned int sizex, unsigned int sizey, int img_depth, int img_alpha, unsigned char **row_pointers, unsigned char *output)
{
    int    stride                = (img_depth / 8) * (img_alpha ? 4 : 3);
    ...
    *readimg(heightmap, 0, 0, w) = 0.0f;
    ...
    for (int j = 0; j < sizey; ++j)
        for (int i = 0; i < sizex; ++i) {
            ...
            if (j == 0) {
                if (i != 0)
                    *readimg(heightmap, i, j, w) = candidate = *readimg(heightmap, i - 1, j, w) + row_pointers[j][i * stride + 0] - 127;
            } else {
                if (i == 0) {
                    *readimg(heightmap, i, j, w) = candidate = *readimg(heightmap, i, j - 1, w) + row_pointers[j][i * stride + 1] - 127;
                } else {
